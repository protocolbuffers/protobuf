...
module Google
  module Protobuf
    ...
    class Map
      ...
      def frozen?
        ruby_frozen = super
        upb_frozen = Google::Protobuf::FFI.map_frozen?(@map_ptr)
        return true if ruby_frozen and upb_frozen
        # Underlying representation is frozen but _self is not.
        if upb_frozen
          method(:freeze).super_method.call
          return true
        end
        raise RuntimeError.new "Ruby frozen Map with mutable representation" if ruby_frozen
        false
      end
      ...
      def freeze
        ruby_frozen = method(:frozen?).super_method.call
        upb_frozen = Google::Protobuf::FFI.map_frozen?(@map_ptr)
        return self if ruby_frozen and upb_frozen
        if !upb_frozen
        ...
        unless Google::Protobuf::FFI.map_frozen?(@map_ptr)
          raise RuntimeError.new "Underlying representation of map still mutable after freeze"
        end
        unless method(:frozen?).super_method.call
          raise RuntimeError.new "Ruby map still mutable after freeze"
        end
        self
      end
