...
static upb_StringView Convert_StringData(VALUE str, upb_Arena* arena) {
  ...
  if (arena) {
    ...
    // Alias the Ruby String's bytes instead of copying them. Only valid when the
    // result is consumed before anything can allocate: Convert_RubyToUpb may have
    // converted `str` to a temporary that nothing else references, so any GC
    // between here and the read can free or move it. The map lookup paths
    // (Map_index, Map_has_key, Map_delete) satisfy this; insertion paths must
    // pass an arena.
