 ...
 public:
  static void StripFile(const FileDescriptor* old_file,
    ...
    for (int i = file.mutable_message_type()->size() - 1; i >= 0; i--) {
      if (old_file->message_type(i)->options().message_set_wire_format()) {
...
class SchemaAddZeroEnumValue {
 ...
 public:
  SchemaAddZeroEnumValue() : total_added_(0) {}
  ...
  void ScrubEnum(EnumDescriptorProto *enum_type) {
    if (enum_type->value(0).number() != 0) {
      ...
      if (!has_zero) {
        ...
        std::stringstream ss;
        ss << "ADDED_ZERO_VALUE_" << total_added_++;
        enum_type->mutable_value(0)->set_name(ss.str());
      }
    }
