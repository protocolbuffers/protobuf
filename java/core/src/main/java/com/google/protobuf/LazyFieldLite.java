...
public class LazyFieldLite {
   ...
   * instead of at decoding phase (when LazyField is initialized), so we need to record the presence
   * in this field and defer the check until it is actually needed. For example, `builder.mergeFrom`
   * will not trigger the check, but the following `builder.build()` will.
   */
  ...
  /** Returns true if the delayedBytes contains missing required fields. */
  boolean containsMissingRequiredFields() {
    return containsMissingRequiredFields;
  }

  void setContainsMissingRequiredFields(boolean containsMissingRequiredFields) {
  ...
  /**
   * Merges another instance's contents from a ByteString.
   *
   * <p>LazyField is not thread-safe for write access. Synchronizations are needed under read/write
   * situations.
   */
