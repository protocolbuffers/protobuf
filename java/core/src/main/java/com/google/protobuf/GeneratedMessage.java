...
public abstract class GeneratedMessage extends AbstractMessage implements Serializable {
  ...
  public static <ContainingT extends Message, T>
    ...
    checkNotNull(singularType);
    checkNotNull(descriptorOuterClass);
    checkNotNull(extensionName);
    ...
    return new GeneratedExtension<>(
        new CachedDescriptorRetriever() {
          ...
          protected FieldDescriptor loadDescriptor() {
            try {
              ClassLoader classLoader = singularType.getClassLoader();
              if (classLoader == null) {
                // this can happen when the extension is a primitive in which
                // case the ClassLoader is the bootstrap ClassLoader
                classLoader = getClass().getClassLoader();
              }
              Class<?> clazz = classLoader.loadClass(descriptorOuterClass);
      ...
      switch (descriptor.getJavaType()) {
        case MESSAGE:
          if (singularType.isInstance(value)) {
            ...
            if (messageDefaultInstance instanceof MutableMessage) {
              return ((MutableMessage) messageDefaultInstance)
                  .newMessageForType()
                  .mergeFrom((MutableMessage) value);
            }
            return messageDefaultInstance.newBuilderForType().mergeFrom((Message) value).build();
          }
        case ENUM:
          if (getExtensionType() == Extension.ExtensionType.MUTABLE
              && descriptor.isProto1FieldInMutableApi()) {
            // Proto1 enum fields/extensions are downgraded to integers.
    ...
    protected Object singularToReflectionType(final Object value) {
      ...
      switch (descriptor.getJavaType()) {
        case ENUM:
          if (getExtensionType() == Extension.ExtensionType.MUTABLE
              && descriptor.isProto1FieldInMutableApi()) {
            // Proto1 enums fields/extensions are downgraded to integers.
            EnumValueDescriptor result =
                descriptor.getEnumType().findValueByNumber((Integer) value);
            if (result == null) {
              throw new IllegalArgumentException(
                  "Invalid enum value " + value + " for field " + descriptor.getFullName());
            }
            return result;
          }
