...
public final class DebugFormatTest {
  ...
  private static final String UNSTABLE_PREFIX = getUnstablePrefix();
  ...
  private static String getUnstablePrefix() {
  ...
  public void singleLineMessageFormat_returnsSingleLine() {
    ...
    assertThat(result)
        .matches(String.format("%s optional_unredacted_string: \"foo\"", UNSTABLE_PREFIX));
  }
  ...
  public void messageFormat_debugRedactFieldIsRedacted() {
    ...
    assertThat(result)
        .matches(
            String.format("%s\noptional_redacted_string: %s\n", UNSTABLE_PREFIX, REDACTED_REGEX));
  }
