...
#define upb_Atomic_Load(addr, order)                                          \
  _Generic(addr,                                                              \
      UPB_ATOMIC(uintptr_t)*: upb_Atomic_LoadMsc((uint64_t volatile*)(addr)), \
...
#define upb_Atomic_Load(addr, order)                                          \
  _Generic(addr,                                                              \
      UPB_ATOMIC(uintptr_t)*: upb_Atomic_LoadMsc((uint32_t volatile*)(addr)), \
...
static void upb_Atomic_Add(uint64_t volatile* addr, uint64_t val) {
  (void)_InterlockedExchangeAdd64(addr, val);
}
static void upb_Atomic_Sub(uint64_t volatile* addr, uint64_t val) {
  (void)_InterlockedExchangeAdd64(addr, -val);
}

#pragma intrinsic(_InterlockedExchangeAdd)
static void upb_Atomic_Add(uint32_t volatile* addr, uint32_t val) {
  (void)_InterlockedExchangeAdd(addr, val);
}
static void upb_Atomic_Sub(uint32_t volatile* addr, uint32_t val) {
  (void)_InterlockedExchangeAdd(addr, -val);
}
