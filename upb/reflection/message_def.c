...
// Validates whether a message marked with `option map_entry = true` satisfies
// the structural invariants required by UPB's map entry representation.
// Raw descriptor protos generated by fuzzers or custom code may set map_entry =
// true on messages that violate these invariants, causing mini-table build
// errors.
static bool _upb_MessageDef_IsValidMapEntry(const upb_MessageDef* m) {
  // Map entry messages must contain exactly two fields (key and value) and no
  // oneofs.
  // e.g. field_count != 2:
  //   field { name: "key" number: 1 label: LABEL_OPTIONAL }
  // e.g. real_oneof_count != 0:
  //   oneof_decl { name: "foo" }
  ...
  // Ensure both key and value fields exist.
  if (key_field == NULL || val_field == NULL) return false;

  // The first field must be field number 1 (key) and the second must be field
  // number 2 (value).
  // e.g. field { name: "key" number: 3 } field { name: "value" number: 4 }
  if (upb_FieldDef_Number(key_field) != 1 ||
      upb_FieldDef_Number(val_field) != 2) {
    return false;
  }

  // Neither key nor value field can be marked repeated.
  // e.g. field { name: "value" number: 2 label: LABEL_REPEATED }
  if (upb_FieldDef_IsRepeated(key_field) ||
      upb_FieldDef_IsRepeated(val_field)) {
    return false;
  }

  // Neither key nor value field can belong to a oneof group.
  // e.g. field { name: "key" number: 1 oneof_index: 0 }
