...
static void DoTest(const ctx* c) {
  ...
  if (conformance_ConformanceRequest_discard_unknown_fields(c->request)) {
    static const char msg[] = "discard_unknown_fields is not supported";
    conformance_ConformanceResponse_set_skipped(c->response,
                                                upb_StringView_FromString(msg));
    return;
  }
  if (conformance_ConformanceRequest_merge_payload_case(c->request) !=
      conformance_ConformanceRequest_merge_payload_NOT_SET) {
    static const char msg[] = "merge_payload is not supported";
    conformance_ConformanceResponse_set_skipped(c->response,
                                                upb_StringView_FromString(msg));
    return;
  }
