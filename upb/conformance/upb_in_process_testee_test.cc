...
// Kept in sync with upb_in_process_testee.cc: the tests pin the name the
// linked testee reports.
...
// TestAllTypesProto3 { optional_int32: 99 } on the wire.
std::string Int32Is99() { return VarintField(1, 99).str(); }
...
TEST(UpbInProcessTesteeTest, AnswersRequestsWithTheUpbTestee) {
  ...
  std::string serialized_response = runner->RunTest(
      "SomeTest", SerializedRequest(Int32Is99(), conformance_PROTOBUF));
...
TEST(UpbInProcessTesteeTest, AnswersRepeatedRequestsIndependently) {
  ...
  std::string first = runner->RunTest(
      "FirstTest", SerializedRequest(Int32Is99(), conformance_TEXT_FORMAT));
  std::string second = runner->RunTest(
      "SecondTest", SerializedRequest(Int32Is99(), conformance_TEXT_FORMAT));
