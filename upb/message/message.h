...
UPB_INLINE bool upb_Message_NextUnknown2(const upb_Message* msg,
  ...
  if (in) {
    while (i < in->size) {
      ...
      if (upb_TaggedAuxPtr_IsUnknown(tagged_ptr)) {
        ...
        data->value.bytes = *upb_TaggedAuxPtr_UnknownData(tagged_ptr);
        *iter = i;
        return true;
...
upb_Message_DeleteUnknown2(upb_Message* msg, upb_MessageUnknown* data,
