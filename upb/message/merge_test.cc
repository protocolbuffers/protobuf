...
TEST(GeneratedCode, MergeMessageEmptyArrayField) {
  upb_Arena* source_arena = upb_Arena_New();
  protobuf_test_messages_proto3_TestAllTypesProto3* src =
      protobuf_test_messages_proto3_TestAllTypesProto3_new(source_arena);

  // Allocate empty repeated field in src (size = 0).
  protobuf_test_messages_proto3_TestAllTypesProto3_resize_repeated_int32(
      src, 0, source_arena);

  upb_Arena* arena = upb_Arena_New();
  protobuf_test_messages_proto3_TestAllTypesProto3* dst =
      protobuf_test_messages_proto3_TestAllTypesProto3_new(arena);

  // Merge src into empty dst.
  EXPECT_TRUE(upb_Message_MergeFrom(
      UPB_UPCAST(dst), UPB_UPCAST(src),
      &protobuf_0test_0messages__proto3__TestAllTypesProto3_msg_init, 0,
      arena));

  // Destination should still have no elements.
  size_t size = 999;
  const int32_t* values =
      protobuf_test_messages_proto3_TestAllTypesProto3_repeated_int32(dst,
                                                                      &size);
  EXPECT_EQ(size, 0);
  EXPECT_EQ(values, nullptr);

  // If dst already had elements, merging empty array from src should preserve
  // them.
  ASSERT_TRUE(
  ...
  values = protobuf_test_messages_proto3_TestAllTypesProto3_repeated_int32(
      dst, &size);
  EXPECT_EQ(size, 1);
  ASSERT_NE(values, nullptr);
  EXPECT_EQ(values[0], 42);

  // Also test with kUpb_MergeOption_Alias
  protobuf_test_messages_proto3_TestAllTypesProto3* dst_alias =
      protobuf_test_messages_proto3_TestAllTypesProto3_new(arena);
  EXPECT_TRUE(upb_Message_MergeFrom(
      UPB_UPCAST(dst_alias), UPB_UPCAST(src),
...
TEST(GeneratedCode, MergeMessageEmptyMapField) {
  upb_Arena* source_arena = upb_Arena_New();
  protobuf_test_messages_proto3_TestAllTypesProto3* src =
      protobuf_test_messages_proto3_TestAllTypesProto3_new(source_arena);

  // Allocate empty map by setting and deleting a key.
  protobuf_test_messages_proto3_TestAllTypesProto3_map_int32_int32_set(
      src, 1, 2, source_arena);
  protobuf_test_messages_proto3_TestAllTypesProto3_map_int32_int32_delete(src,
                                                                          1);
  ...
  // Destination should still have no map (size = 0).
  EXPECT_EQ(map_size(dst), 0);

  // If dst already had map entries, merging empty map from src should preserve
  // them.
  ASSERT_TRUE(map_set(dst, 10, 20, arena));
  EXPECT_TRUE(upb_Message_MergeFrom(
      UPB_UPCAST(dst), UPB_UPCAST(src),
      &protobuf_0test_0messages__proto3__TestAllTypesProto3_msg_init, 0,
      arena));
  EXPECT_EQ(map_size(dst), 1);
  int32_t val = 0;
  EXPECT_TRUE(map_get(dst, 10, &val));
  EXPECT_EQ(val, 20);
  ...
  EXPECT_TRUE(upb_Message_MergeFrom(
      ...
      &protobuf_0test_0messages__proto3__TestAllTypesProto3_msg_init,
      kUpb_MergeOption_Alias, arena));
  EXPECT_EQ(map_size(dst_alias), 0);
