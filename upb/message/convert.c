...
static void upb_Message_SetFieldOrExtension(upb_Converter* c, upb_Message* msg,
                                            const upb_MiniTableField* f,
                                            const upb_MiniTableExtension* ext,
                                            const upb_MessageValue* val,
                                            upb_Arena* arena) {
  if (ext != NULL) {
    if (!upb_Message_SetExtension(msg, ext, val, arena)) {
...
static void upb_Array_DeepConvert(
  ...
  if (dst_i != size) {
    if (!upb_Array_Resize(dst, dst_i, c->arena)) {
      upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
    }
  }
}
  ...
  if (dst_f) {
    ...
    if (upb_MiniTableField_CType(dst_f) == kUpb_CType_Message) {
      ...
      const upb_MiniTable* src_sub_mt =
          upb_MiniTableExtension_GetSubMessage(ext);
      ...
      if (upb_MiniTableField_IsArray(dst_f)) {
        if (dst_sub_mt != src_sub_mt) {
          ...
          upb_Message_SetFieldOrExtension(c, dst, dst_f, dst_ext, &valid_val,
                                          c->arena);
        } else {
          // Array of messages, and the sub message types are the same.
          // Shallow copy.
          upb_Message_SetFieldOrExtension(c, dst, dst_f, dst_ext, &val,
                                          c->arena);
        }
      } else if (dst_sub_mt == src_sub_mt) {
        // Scalar message, and the message types are the same.
        // Shallow copy.
        upb_Message_SetFieldOrExtension(c, dst, dst_f, dst_ext, &val, c->arena);
      } else {
