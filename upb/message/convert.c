      ...
      if (dst_mt->UPB_PRIVATE(ext) == kUpb_ExtMode_NonExtendable) {
        ...
        if (!(c->options & kUpb_ConvertOption_Alias)) {
          ...
          if (upb_MiniTableField_IsArray(ext_f)) {
            upb_Array* arr = upb_Array_DeepClone(
                val.array_val, upb_MiniTableField_CType(ext_f),
                upb_MiniTableExtension_GetSubMessage(ext), c->arena);
            if (!arr) {
              upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
            }
            val.array_val = arr;
          } else if (upb_MiniTableField_CType(ext_f) == kUpb_CType_Message) {
            upb_Message* msg = upb_Message_DeepClone(
                val.msg_val, upb_MiniTableExtension_GetSubMessage(ext),
                c->arena);
            if (!msg) {
              upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
            }
            ...
            val.str_val = upb_StringView_FromDataAndSize(new_str, src_str.size);
          }
        }
...
bool UPB_PRIVATE(_upb_Message_Convert)(upb_Message* dst,
                                       const upb_MiniTable* dst_mt,
                                       const upb_Message* src,
                                       const upb_MiniTable* src_mt,
                                       const upb_ExtensionRegistry* extreg,
  ...
  if (dst_mt == src_mt && extreg == NULL && options == kUpb_ConvertOption_Alias)
    return true;
  ...
  bool ok = upb_Message_DoConvert(&c, dst, src, dst_mt, src_mt, extreg);
