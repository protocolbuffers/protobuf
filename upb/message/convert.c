...
static void _upb_Message_CheckIfClosedEnum(const upb_Message* dst,
  ...
  if (upb_MiniTableField_IsClosedEnum(dst_f)) {
    ...
    if (upb_MiniTableField_IsArray(dst_f)) {
      const upb_Array* arr =
          *(const upb_Array**)UPB_PRIVATE(_upb_Message_DataPtr)(dst, dst_f);
      if (arr) {
        ...
        for (size_t i = 0; i < n; i++) {
          ...
          if (!upb_MiniTableEnum_CheckValue(dst_e, val.int32_val)) {
            upb_ErrorHandler_ThrowError(err, kUpb_ErrorCode_Malformed);
          }
        }
      }
    } else {
      int32_t val = *(int32_t*)UPB_PRIVATE(_upb_Message_DataPtr)(dst, dst_f);
...
static void _upb_Extension_CheckIfClosedEnum(
    const upb_MiniTableField* dst_f, const upb_MiniTableExtension* dst_ext,
    upb_MessageValue val, upb_ErrorHandler* err) {
  if (upb_MiniTableField_IsClosedEnum(dst_f)) {
    const upb_MiniTableEnum* dst_e =
    ...
    if (upb_MiniTableField_IsArray(dst_f)) {
      ...
      if (arr) {
        size_t n = upb_Array_Size(arr);
        for (size_t i = 0; i < n; i++) {
          upb_MessageValue arr_val = upb_Array_Get(arr, i);
          if (!upb_MiniTableEnum_CheckValue(dst_e, arr_val.int32_val)) {
            upb_ErrorHandler_ThrowError(err, kUpb_ErrorCode_Malformed);
          }
        }
      }
...
static bool upb_Message_ConvertArrayField(upb_Converter* c, upb_Message* dst,
  ...
  if (dst_sub_mt != src_sub_mt) {
    UPB_ASSERT(upb_MiniTableField_CType(dst_f) == kUpb_CType_Message &&
               upb_MiniTableField_CType(src_f) == kUpb_CType_Message);
    upb_Array* dst_arr = upb_Array_New(c->arena, kUpb_CType_Message);
    if (!dst_arr)
      upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
    upb_Array_DeepConvert(c, dst_arr, src_arr, dst_sub_mt, src_sub_mt, extreg,
                          depth);
...
static void upb_Message_ConvertField(upb_Converter* c, upb_Message* dst,
  ...
  if (upb_MiniTableField_CType(dst_f) == kUpb_CType_Message) {
    if (upb_MiniTableField_IsScalar(dst_f)) {
      ...
      if (UPB_UNLIKELY(!upb_MiniTableField_IsArray(src_f))) {
      ...
      if (UPB_UNLIKELY(!upb_MiniTableField_IsMap(src_f))) {
        upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_Malformed);
      }
      ...
      if (UPB_UNLIKELY(upb_MiniTableField_Type(dst_key_f) !=
  ...
  // Additional check for closed enum, as the value may have been set to an
  // invalid value if the source enum is open.
  _upb_Message_CheckIfClosedEnum(dst, dst_f, &c->err);
...
static void upb_Message_ConvertExtensions(upb_Converter* c, upb_Message* dst,
  ...
  while (upb_Message_NextExtension(src, &ext, &val, &iter)) {
    ...
    if (dst_f) {
      ...
      if (UPB_UNLIKELY(upb_MiniTableField_Type(dst_f) !=
                           upb_MiniTableField_Type(src_f) ||
      ...
      // Similarly to regular fields, check for closed enum, as the value may
      // have been set to an invalid value if the source enum is open.
      _upb_Extension_CheckIfClosedEnum(dst_f, dst_ext, val, &c->err);
    } else {
      // The extension is not defined in the destination message or registry.
      // Encode the extension as an unknown field in the destination message.
...
static void upb_Message_ConvertInternal(upb_Converter* c, upb_Message* dst,
  ...
  while (dst_f != dst_first || src_f != src_first) {
    ...
    if (dst_nr == src_nr) {
      ...
      if (UPB_UNLIKELY(upb_MiniTableField_Type(src_next) !=
                           upb_MiniTableField_Type(dst_next) ||
                       upb_MiniTableField_IsArray(src_next) !=
                           upb_MiniTableField_IsArray(dst_next) ||
                       upb_MiniTableField_IsMap(src_next) !=
                           upb_MiniTableField_IsMap(dst_next))) {
        upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_Malformed);
      }
