...
enum {
   ...
   *
   * IMPORTANT: If using this option, the caller is responsible for ensuring:
   * 1. The source message and its submessages remain valid (i.e. not freed
   *    or modified) for the lifetime of the destination message.
   * 2. The destination message is not mutated unless the source message is
   *    known to be safe to mutate.
   */
