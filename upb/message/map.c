...
upb_Map* _upb_Map_New(upb_Arena* a, size_t key_size, size_t value_size) {
  ...
  if (key_size <= sizeof(uintptr_t) && key_size != UPB_MAPTYPE_STRING) {
    // Use the inttable for primitive keys, but exclude uint64_t, int64_t, and
    // double values, as inttable currently does not support UINT64_MAX in the
    // array part.
    upb_inttable_init(&map->t.inttable, a);
