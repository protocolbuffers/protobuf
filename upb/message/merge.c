...
static bool upb_Map_Merge(upb_MergeCtx* ctx, upb_Map** dst_map_ptr,
  ...
  if (*dst_map_ptr == NULL) {
    ...
    // We pretend that the map is mutable so that we can alias it, but this is
    // subject to the warning in kUpb_MergeOption_Alias.
    *dst_map_ptr = (upb_Map*)upb_Merge_CloneOrAliasMap(
...
bool upb_Message_MergeFrom(upb_Message* dst, const upb_Message* src,
                           ...
                           upb_Arena* arena) {
  // If necessary, we could make this configurable through `options`.
  const int kMaxMergeDepth = 100;

  upb_MergeCtx ctx = {
      .options = options,
      .arena = arena,
      .depth = kMaxMergeDepth,
  };
  upb_ErrorHandler err;
