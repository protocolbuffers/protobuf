...
static void upb_UnknownFields_Grow(upb_UnknownField_Context* ctx,
  ...
  size_t old = (*ptr - *base);
  if (old > SIZE_MAX / 2) upb_UnknownFields_OutOfMemory(ctx);
  size_t new = UPB_MAX(4, old * 2);

  if (SIZE_MAX / sizeof(**base) < new) upb_UnknownFields_OutOfMemory(ctx);
...
static void upb_UnknownFields_Sort(upb_UnknownField_Context* ctx,
  ...
  if (ctx->tmp_size < fields->size) {
    if (SIZE_MAX / sizeof(*ctx->tmp) < ctx->tmp_size) {
      upb_UnknownFields_OutOfMemory(ctx);
    }
    size_t oldsize = ctx->tmp_size * sizeof(*ctx->tmp);
    ctx->tmp_size = UPB_MAX(8, ctx->tmp_size);
    while (ctx->tmp_size < fields->size) {
      if (ctx->tmp_size > SIZE_MAX / 2) {
        upb_UnknownFields_OutOfMemory(ctx);
      }
      ctx->tmp_size *= 2;
    }
    ...
    size_t newsize = ctx->tmp_size * sizeof(*ctx->tmp);
