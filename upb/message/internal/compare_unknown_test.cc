...
TEST(CompareTest, MessageSetNonCanonicalExtMatchesUnknown) {
  ...
  upb_MiniTableExtension custom_ext =
      *upb_test_MessageSetMember_message_set_extension_ext;
  upb_MiniTableExtension_SetSubMessage(&custom_ext, custom_sub_table);

  // 2. Create custom sub-message (MessageSetMember) under custom_sub_table
  upb_Message* extension1 = _upb_Message_New(custom_sub_table, arena.ptr());
  const upb_MiniTableField* custom_f =
      upb_MiniTable_GetFieldByIndex(custom_sub_table, 0);
  upb_Message_SetInt32(extension1, custom_f, 123, arena.ptr());
