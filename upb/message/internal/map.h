...
UPB_INLINE uintptr_t _upb_map_tointkey(const void* key, size_t size) {
  // We have to cast to the correct type based on the actual type of the key.
  if (size == 4) {
    return *(int32_t*)key;
  } else if (size == 8) {
    return *(int64_t*)key;
...
UPB_INLINE void* _upb_map_next(const struct upb_Map* map, size_t* iter) {
  if (map->UPB_PRIVATE(is_strtable)) {
    ...
    *iter = it.index;
    ...
    uintptr_t key;
    upb_value val;
    upb_inttable_next(&map->t.inttable, &key, &val, (intptr_t*)iter);
    if (upb_inttable_done(&map->t.inttable, *iter)) return NULL;
    // TODO: We need to fix the assumption that the return value
    // is a pointer to the table entry. In an inttable, when the entry lives in
    // the array, the `upb_tabent` item does not exist.
    //
    // This function is somehow called by the generated `_upb_msg_map_key` code
    // in google3/third_party/upb/upb/message/map_gencode_util.h, but it's not
    // clear how.
    //
    // We need to fix this assumption before we can use the inttable.
    return (void*)iter;
  }
}
