...
UPB_NODISCARD UPB_INLINE struct upb_Array* UPB_PRIVATE(
  ...
  size_t elem_bytes;
  if (upb_ShlOverflow(init_capacity, (size_t)elem_size_lg2, &elem_bytes)) {
    return NULL;
  }
...
#if UPB_FUTURE_32BIT_ARRAY
        size_t cap = pool_bytes >> elem_size_lg2;
        if (cap > UINT32_MAX) {
          cap = UINT32_MAX;
        }
  ...
  size_t total_bytes;
  if (upb_AddOverflow(array_size, elem_bytes, &total_bytes)) {
    return NULL;
  }
...
UPB_NODISCARD UPB_FORCEINLINE bool UPB_PRIVATE(_upb_Array_TryFastRealloc)(
  ...
  size_t new_bytes;
  if (upb_ShlOverflow(capacity, (size_t)elem_size_lg2, &new_bytes)) {
    return false;
  }
