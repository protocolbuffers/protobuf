...
static const char* _upb_Decoder_DecodeEmptyMessage(upb_Decoder* d,
  ...
  if (ptr == NULL || ptr - d->input.end > d->input.limit) {
    UPB_PRIVATE(upb_EpsCopyInputStream_ReturnError)(&d->input);
    return NULL;
  }

  const char* end_input =
      UPB_PRIVATE(upb_EpsCopyInputStream_GetInputPtr)(&d->input, ptr);
  UPB_ASSERT(end_input >= start_input);
  size_t size = end_input - start_input;
  if (size > 0) {
    if (!UPB_PRIVATE(_upb_Message_AddUnknown)(
            msg, start_input, size, &d->arena,
            _upb_Decoder_GetAddUnknownMode(d, start_input))) {
      upb_ErrorHandler_ThrowError(d->err, kUpb_DecodeStatus_OutOfMemory);
    }
  }
  return ptr;
