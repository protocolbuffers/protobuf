...
const char* DoYield(const char* start, const char* end);
...
int Yield(const char* start, const char* end) {
  if (start < end) {
...
// This is the logic of EpsCopyInputStream, state1 and state2 are labels corresponding
// to the states of EpsCopyInputStream.
// state1 corresponds to next_chunk_ != patch_buffer_ as here we going to doll out the chunk from ZeroCopyInputStream directly.
// state2 corresponds to next_chunk_ == patch_buffer_ as we are going to return data from patch_buffer_.
...
bool PushParse(ChunkStreamer chunk_stream) {
  ...
  while (chunk_stream(&ptr, &size)) {
    ...
    if (size > 16) {
      ...
      overrun = Yield(buffer + overrun, buffer + 16);
state1:  // ptr and size need to be saved across Yield point (next_chunk_ and size_ in EpsCopyInputStream).
      end = ptr + size - 16;
      overrun = Yield(ptr + overrun, end);
    } else {
      memcpy(buffer + 16, ptr, size);
      end = buffer + size;
      overrun = Yield(buffer + overrun, end);
    }
state2:  // end needs to be saved across Yield point (buffer_end_ in EpsCopyInputStream).
