...
// Encode a known field with a 1-byte tag as an overlong 2-byte varint (that
// maps to but mismatches a known slot) and ensure Fasttable handles it
// correctly.
TEST(FuzzTest, OverlongTagMismatchedFasttableSlot) {
  DecodeEncodeArbitrarySchemaAndPayload(
      upb::fuzz::MiniTableFuzzInput{
          {"$0<$%$$$$$$"},
          {"", "", "", "", "", "", "", "",  "", "", "", "", "", "", "", "",
           "", "", "", "", "", "", "", "e", "", "", "", "", "", "", "", ""},
          "ddd",
          {3165123283, 3165123283}},
      std::string("%\344\377F\300\220\000\000", 8), 2112000741, -1146615638,
      false);
}

// Encode a known field with a 1-byte tag as an overlong 2-byte varint (that
// maps to an empty slot) and ensure Fasttable handles it correctly.
TEST(FuzzTest, OverlongTagEmptyFasttableSlot) {
  DecodeEncodeArbitrarySchemaAndPayload(
      upb::fuzz::MiniTableFuzzInput{{"$0<r("}, {}, "", {}},
      std::string("\x90\x00\x00", 3), 0, 0, false);
}

// This test verifies that the fuzzer does not dynamically mutate
// field descriptor types post-construction when sub-enum tables fail to link,
// preventing invariant violations in the fast table decoder.
TEST(FuzzTest, DecodeEncodeArbitrarySchemaAndPayloadRegressionMissingSubEnum) {
  DecodeEncodeArbitrarySchemaAndPayload(
      upb::fuzz::MiniTableFuzzInput{{"$$4"}, {"M"}, "", {}}, "\027", 1,
      2147483647, false);
}
