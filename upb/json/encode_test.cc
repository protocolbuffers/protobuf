...
static std::string JsonEncode(const upb_test_Box* msg, int options) {
  ...
  size_t json_size = upb_JsonEncode((upb_Message*)msg, m.ptr(), defpool.ptr(),
                                    options, nullptr, 0, status.ptr());
  char* json_buf = (char*)upb_Arena_Malloc(a.ptr(), json_size + 1);

  size_t size = upb_JsonEncode((upb_Message*)msg, m.ptr(), defpool.ptr(),
                               options, json_buf, json_size + 1, status.ptr());
