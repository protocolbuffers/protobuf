...
void* UPB_PRIVATE(_upb_Arena_SlowMalloc)(upb_Arena* a, size_t size) {
  if (!_upb_Arena_AllocBlock(a, size)) return NULL;  // Out of memory.
...
static upb_Arena* _upb_Arena_InitSlow(upb_alloc* alloc) {
  ...
  a->UPB_PRIVATE(block_alloc) = _upb_Arena_MakeBlockAlloc(alloc, 0);
  upb_Atomic_Init(&a->UPB_PRIVATE(parent_or_count),
                  _upb_Arena_TaggedFromRefcount(1));
...
upb_Arena* upb_Arena_Init(void* mem, size_t n, upb_alloc* alloc) {
  ...
  upb_Atomic_Init(&a->UPB_PRIVATE(tail), a);
  upb_Atomic_Init(&a->UPB_PRIVATE(blocks), NULL);
  a->UPB_PRIVATE(block_alloc) = _upb_Arena_MakeBlockAlloc(alloc, 1);
  a->head.ptr = mem;
...
static void arena_dofree(upb_Arena* a) {
  UPB_ASSERT(_upb_Arena_RefCountFromTagged(a->UPB_PRIVATE(parent_or_count)) ==
             1);
