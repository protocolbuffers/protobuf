...
static upb_ArenaInternal* _upb_Arena_TailFromTagged(
  ...
  UPB_ASSERT(_upb_Arena_IsTaggedRefcount(previous_or_tail));
...
static upb_ArenaInternal* _upb_Arena_PreviousFromTagged(
    uintptr_t previous_or_tail) {
  UPB_ASSERT(!_upb_Arena_IsTaggedPointer(previous_or_tail));
...
static uintptr_t _upb_Arena_TaggedFromPrevious(upb_ArenaInternal* ai) {
  ...
  UPB_ASSERT(_upb_Arena_IsTaggedPointer(previous));
  return previous;
}
...
static upb_ArenaRoot _upb_Arena_FindRoot(const upb_Arena* a) {
  ...
  if (_upb_Arena_IsTaggedRefcount(poc)) {
    // Fast, relaxed path - arenas that have never been fused only need relaxed
    // memory order, since they're returning themselves and the refcount.
...
size_t upb_Arena_SpaceAllocated(const upb_Arena* arena, size_t* fused_count) {
  upb_ArenaInternal* root = _upb_Arena_FindRoot(arena).root;
  upb_ArenaInternal* original = upb_Arena_Internal(arena);
  size_t memsize = 0;
  size_t local_fused_count = 0;

  // In order to find all reachable arenas fused to this one, we start from the
  // root and iterate forwards. If we encounter the target arena, we continue
  // until we hit the end; if we don't encounter it, then we iterate backwards
  // from the target arena until we hit the last node reachable from the root,
  // then iterate forwards from the target arena.
  upb_ArenaInternal* ai = root;
  upb_ArenaInternal* last_from_root = root;
  while (ai != NULL) {
  ...
  while (ai != NULL) {
    ...
    if (!_upb_Arena_IsTaggedPrevious(previous_or_tail)) break;
    ai = _upb_Arena_PreviousFromTagged(previous_or_tail);
    UPB_TSAN_CHECK_PUBLISHED(ai);
    // Avoid double counting
    if (ai == last_from_root) break;
    // Relaxed is safe - no subsequent reads depend this one
    memsize += upb_Atomic_Load(&ai->space_allocated, memory_order_relaxed);
    local_fused_count++;
  }
...
static void _upb_Arena_DoFuseArenaLists(upb_ArenaInternal* const parent,
  ...
  while (true) {
    ...
    if (upb_Atomic_CompareExchangeStrong(&parent_tail->next, &parent_tail_next,
                                         child, memory_order_release,
                                         memory_order_acquire)) {
      UPB_ASSERT(parent_tail_next == NULL);
      break;
    }
    parent_tail = parent_tail_next;
  }
  uintptr_t child_tail = upb_Atomic_Exchange(
      &child->previous_or_tail, _upb_Arena_TaggedFromPrevious(parent_tail),
      memory_order_acq_rel);
  if (_upb_Arena_IsTaggedTail(child_tail)) {
    parent_tail = _upb_Arena_TailFromTagged(child_tail);
    UPB_TSAN_CHECK_PUBLISHED(parent_tail);
  }
  upb_Atomic_Store(&parent->previous_or_tail,
                   _upb_Arena_TaggedFromTail(parent_tail),
