...
bool upb_Arena_HasRefChain(const upb_Arena* from, const upb_Arena* to) {
  ...
  while (current != NULL) {
    ...
    upb_ArenaRef* ref = upb_Atomic_Load(&current->refs, memory_order_acquire);
    while (ref != NULL) {
...
#ifndef NDEBUG
  // Add to the dedicated list of refs using CAS for thread safety.
  ref->next_ref = upb_Atomic_Load(&ai->refs, memory_order_relaxed);
  while (!upb_Atomic_CompareExchangeWeak(&ai->refs, &ref->next_ref, ref,
                                         memory_order_release,
                                         memory_order_relaxed)) {
    // Reload next_ref if CAS failed due to a race.
  }
#endif
...
bool upb_Arena_HasRef(const upb_Arena* from, const upb_Arena* to) {
  upb_ArenaInternal* ai = upb_Arena_Internal(from);
  upb_ArenaRef* ref = upb_Atomic_Load(&ai->refs, memory_order_acquire);
  while (ref != NULL) {
    if (upb_Arena_IsFused(ref->arena, to)) {
      return true;
    }
