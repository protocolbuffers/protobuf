...
bool upb_strtable_resize(upb_strtable* t, size_t size_lg2, upb_Arena* a) {
  ...
  while (upb_strtable_next2(t, &key, &val, &iter)) {
    ...
    upb_key tabkey = (uintptr_t)(key.data - sizeof(uint32_t));

    UPB_ASSERT(upb_key_strview(tabkey).data == key.data);
    UPB_ASSERT(upb_key_strview(tabkey).size == key.size);
...
upb_StringView upb_strtable_iter_key(const upb_strtable_iter* i) {
  ...
  key.data = upb_key_cstr(str_tabent(i)->key, &len);
  key.size = len;
  return key;
}
...
bool upb_strtable_next2(const upb_strtable* t, upb_StringView* key,
  ...
  if (tab_idx < upb_table_size(&t->t)) {
    ...
    uint32_t len;
    key->data = upb_key_cstr(ent->key, &len);
    key->size = len;
    *val = ent->val;
    *iter = tab_idx;
    return true;
...
static upb_value* mutable_array(upb_inttable* t) {
  if (t->array_size == 1) {
    return &t->array.single_value;
  }
  return (upb_value*)t->array.ptr;
}
...
static const upb_value* inttable_array_get(const upb_inttable* t,
  ...
  const upb_value* val =
      t->array_size == 1 ? &t->array.single_value : &t->array.ptr[key];
...
bool upb_inttable_sizedinit(upb_inttable* t, uint32_t asize, int hsize_lg2,
  ...
  t->array_count = 0;
  if (UPB_UNLIKELY(t->array_size > 1)) {
  ...
  if (key < t->array_size) {
    ...
    if (isfull(&t->t)) {
      ...
      for (i = begin(&t->t); i < upb_table_size(&t->t); i = next(&t->t, i)) {
        ...
        insert(&new_table, intkey(e->key), e->key, e->val, hash, &inthash,
               &inteql);
      }
...
bool upb_inttable_remove(upb_inttable* t, uintptr_t key, upb_value* val) {
  ...
  if (key < t->array_size) {
    if (upb_inttable_arrhas(t, key)) {
      t->array_count--;
      if (val) {
        *val = t->array_size == 1 ? t->array.single_value : t->array.ptr[key];
      }
