 ...
 */
template <typename T>
typename T::CProxy MakeCHandle(upb_Message* msg, const upb_MiniTable* minitable,
                               upb_Arena* arena) {
  ABSL_CHECK(minitable == internal::AssociatedUpbTypes<T>::kMiniTable);
  return internal::PrivateAccess::CProxy<T>((upb_Message*)msg, arena);
}
...
typename T::Proxy MakeHandle(upb_Message* msg, const upb_MiniTable* minitable,
                             upb_Arena* arena) {
  ABSL_CHECK(minitable == internal::AssociatedUpbTypes<T>::kMiniTable);
  return internal::PrivateAccess::Proxy<T>((upb_Message*)msg, arena);
}
