...
PyObject* pycheck_cpp_proto(PyObject* self, PyObject* args) {
  ...
  // ABSL_LOG(ERROR) << "file "
  //                << msg.value().get().GetDescriptor()->file()->name()
  //                << " is not from generated pool";
  // ABSL_LOG(ERROR) << "(proto_api_example) generated pool is: "
  //                 << google::protobuf::DescriptorPool::generated_pool();
  // ABSL_LOG(ERROR) << "file is from: "
  //                 << msg->get().GetDescriptor()->file()->pool();
  Py_RETURN_FALSE;
}

PyObject* pymessage_get(PyObject* self, PyObject* args) {
  const proto3_unittest::TestAllTypes empty;
  // ABSL_LOG(ERROR) << "(proto_api_example other) generated pool is: "
  //                 << empty.GetDescriptor()->file()->pool();
  PyObject* py_message;
  if (!PyArg_ParseTuple(args, "O", &py_message)) return nullptr;
