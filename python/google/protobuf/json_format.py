...
class _Parser(object):
  ...
  def _ConvertFieldValuePair(self, js, message, path):
    ...
    def ClearFieldOrExtension(message, field):
      if field.is_extension:
        message.ClearExtension(field)
      else:
        message.ClearField(field.name)

    def GetFieldOrExtension(message, field):
      if field.is_extension:
        return message.Extensions[field]
      else:
        return getattr(message, field.name)

    def SetFieldOrExtension(message, field, value):
      if field.is_extension:
      ...
      try:
        ...
        if value is None:
          if (
            ...
            sub_message = GetFieldOrExtension(message, field)
            sub_message.null_value = 0
          elif (
              field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_ENUM
              and field.enum_type.full_name == 'google.protobuf.NullValue'
          ):
            SetFieldOrExtension(message, field, 0)
          else:
            ClearFieldOrExtension(message, field)
          continue
        ...
        if _IsMapEntry(field):
          ...
          if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
            ...
            for index, item in enumerate(value):
              sub_message = GetFieldOrExtension(message, field).add()
