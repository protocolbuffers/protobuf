...
class _Parser(object):
  ...
  def _ConsumeAnyTypeUrl(self, tokenizer):
    ...
    """
    # Consume all tokens with valid URL characters until ]. Whitespace is
    # handled by the Tokenizer (all whitespace between [] is ignored).
    tokens = []
    last_slash = -1
    while True:
      token = tokenizer.token
      if tokenizer.TryConsumeUrlChars():
        tokens.append(token)
      elif tokenizer.TryConsume('/'):
    ...
    for i in range(len(prefix)):
      if prefix[i] == '%' and not _PERCENT_ENCODING.match(prefix[i : i + 3]):
        raise tokenizer.ParseError(
            'Invalid percent escape, got "%s".' % prefix[i : i + 3]
        )
...
class Tokenizer(object):
  ...
  def ConsumeUrlChars(self):
    ...
    Excludes '/' so that it can treated specially as a delimiter.
