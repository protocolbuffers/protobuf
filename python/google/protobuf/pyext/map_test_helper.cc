...
PyObject* TestSumAllInt32FieldsUsingRepeatedFields(PyObject* m,
  ...
  if (!PyObject_TypeCheck(py_message, CMessage_Type)) {
    PyErr_SetString(PyExc_TypeError, "Argument must be a Message");
    return nullptr;
  }

  CMessage* cmsg = reinterpret_cast<CMessage*>(py_message);
  const google::protobuf::Message* current_msg = cmsg->message;
