...
static PyObject* MapKeyToPython(MapContainer* self, const MapKeyConstRef& key) {
  const FieldDescriptor* field_descriptor =
      self->parent_field_descriptor->message_type()->map_key();
  switch (field_descriptor->cpp_type()) {
    case FieldDescriptor::CPPTYPE_INT32:
...
PyObject* MapReflectionFriend::ScalarMapToStr(PyObject* _self) {
  ...
  for (google::protobuf::MapIterator it =
    ...
    key.reset(MapKeyToPython(self, it.GetKeyRef()));
    if (key == nullptr) {
      return nullptr;
    }
...
PyObject* MapReflectionFriend::IterNext(PyObject* _self) {
  ...
  PyObject* ret = MapKeyToPython(self->container, self->iter->GetKeyRef());
