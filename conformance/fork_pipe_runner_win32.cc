...
void ForkPipeRunner::SpawnTestProgram() {
  ...
  if (!CreateProcessA(nullptr, &command_line[0], nullptr, nullptr, TRUE,
                      ...
                      &process_info)) {
    // A testee that can't be started (e.g. a missing binary) is not the
    // runner's failure: the POSIX runner only learns of a failed exec from
    // the child's death, which RunTest() reports as a runtime error for the
    // request in flight, and a missing testee must not be fatal for the
    // protocol discovery handshake in ConformanceEnvironment::SetUp() either.
    // So leave the pipes unset; the request's write fails and is reported
    // the same way, and the next RunTest() call tries to spawn again.
    ABSL_LOG(ERROR) << "CreateProcess failed for " << executable_ << ": "
                    << LastSystemError();
