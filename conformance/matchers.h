...
//   - A test that needs a newer conformance protocol than the testee speaks
//     (TestResult::Outcome::kUnsupportedProtocol, see testee.h) was never
//     sent.  Yields() reports it to the TestManager as unsupported (which
//     counts it in unsupported_protocol_tests() and keeps its failure list
//     entry, if any, as it is; see TestManager::ReportUnsupportedProtocol())
//     and marks the gtest test skipped with the reason, so that a run against
//     an older testee shows where its coverage stops.  It is never a failure,
//     listed or not.
...
class FailureMatcher {
 ...
 private:
  std::string name_;
  TestResult::Outcome expected_outcome_;
...
//
//   EXPECT_THAT(Testee().ParseBinary(d, w).SerializeBinary(),
