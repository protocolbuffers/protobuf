...
absl::StatusOr<ConformanceResponse> CppConformanceHarness::RunTest(
  ...
  const Descriptor* descriptor =
      pool_->FindMessageTypeByName(request.message_type());
  ...
  std::unique_ptr<Message> test_message(
      factory_->GetPrototype(descriptor)->New());
  ...
  switch (request.merge_payload_case()) {
    case ConformanceRequest::kMergeProtobufPayload: {
      if (!test_message->MergeFromString(request.merge_protobuf_payload())) {
        response.set_parse_error(
            "merge_payload parse error (no more details available)");
        return response;
      }
      break;
    }
    ...
    case ConformanceRequest::kMergeJsonPayload: {
      // Parsing JSON straight into the message isn't MergeFrom(): the JSON
      // parser clears a repeated or message field the first time it sees it
      // (and Clear()s Struct/Value/ListValue outright), so the payload is
      // parsed into a fresh message that is then merged in.
      JsonParseOptions options;
      options.ignore_unknown_fields =
          (request.test_category() ==
           ::conformance::JSON_IGNORE_UNKNOWN_PARSING_TEST);
      std::unique_ptr<Message> merged(test_message->New());
      absl::Status status = JsonStringToMessage(request.merge_json_payload(),
                                                merged.get(), options);
      if (!status.ok()) {
        response.set_parse_error(
            absl::StrCat("merge_payload parse error: ", status.message()));
        return response;
      }
    ...
    case ConformanceRequest::kMergeTextPayload: {
      if (!TextFormat::MergeFromString(request.merge_text_payload(),
                                       test_message.get())) {
        response.set_parse_error(
            "merge_payload parse error (no more details available)");
        return response;
