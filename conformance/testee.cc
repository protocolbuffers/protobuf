...
std::string GetTestName(const TestName& name, absl::string_view legacy_name,
  ...
  std::string full_name(PriorityLevelName(priority));
...
InMemoryMessage Test::ParseBinary(const Descriptor* type, Wire input) && {
  ...
  return InMemoryMessage(testee_, name_, legacy_name_, priority_, type,
                         std::move(request));
}
...
TestResult InMemoryMessage::Finish(
  ...
  std::string full_name =
      GetTestName(name_, legacy_name_, priority_, request_, *type_, name_style);
