...
TestResult::TestResult(ConformanceTestInfo info, const Descriptor* type,
  ...
  switch (response_.result_case()) {
    ...
    case ConformanceResponse::kResults:
      // A protocol version 2 answer.  This runner only sends version 1
      // requests so far, so a testee answering this way did not run what it
      // was given; the runner learns to send and read version 2 separately.
      outcome_ = Outcome::kRuntimeError;
      message_ =
          "the testee answered with protocol version 2 results to a version 1 "
          "request";
      break;
    case ConformanceResponse::kProtobufPayload:
