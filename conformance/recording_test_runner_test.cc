...
TEST(CanonicalizeRequestTest, FallsBackToSortedWireFieldsWhenParseFails) {
  ...
  constexpr absl::string_view kAscending = "\x12\x01\xff\x22\x01\x78";
  constexpr absl::string_view kDescending = "\x22\x01\x78\x12\x01\xff";
  ASSERT_FALSE(::conformance::ConformanceRequest().ParseFromString(kAscending));

  EXPECT_EQ(internal::CanonicalizeRequest(kAscending),
            internal::CanonicalizeRequest(kDescending));
  EXPECT_EQ(internal::CanonicalizeRequest(kDescending), kAscending);
}
