...
TEST_P(JsonFieldMaskTest, FieldMask) {
  ...
  EXPECT_THAT(
      Testee("FieldMask").ParseJson(message(), kInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("FieldMask").ParseJson(message(), kInput).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonFieldMaskTest, EmptyFieldMask) {
  ...
  EXPECT_THAT(
      Testee("EmptyFieldMask").ParseJson(message(), kInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      Testee("EmptyFieldMask").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}

TEST_P(JsonFieldMaskTest, FieldMaskInvalidCharacter) {
  EXPECT_THAT(
      Testee(kP3, "FieldMaskInvalidCharacter")
...
TEST_P(JsonFieldMaskTest, FieldMaskPathsDontRoundTrip) {
  EXPECT_THAT(Testee(kP3, "FieldMaskPathsDontRoundTrip")
...
TEST_P(JsonFieldMaskTest, FieldMaskNumbersDontRoundTrip) {
  EXPECT_THAT(
      Testee(kP3, "FieldMaskNumbersDontRoundTrip")
          .ParseBinary(message(),
                       LengthPrefixedField(
                           kOptionalFieldMask,
...
TEST_P(JsonFieldMaskTest, FieldMaskTooManyUnderscore) {
  EXPECT_THAT(Testee(kP3, "FieldMaskTooManyUnderscore")
                  .ParseBinary(message(), LengthPrefixedField(
                                              kOptionalFieldMask,
                                              LengthPrefixedField(
