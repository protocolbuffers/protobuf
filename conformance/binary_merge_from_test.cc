...
//
// The Merge*() tests use the one merge shape protocol version 1 can express:
// a second payload merged into the parsed message before anything else is
// done to it.  The MergeFrom() tests at the end merge into a message the
// testee created empty, or after its unknown fields were discarded, which
// needs protocol version 2; a testee that speaks version 1 never sees them
// and the runner reports them as unsupported rather than failed.
...
// The tests below need MergeFrom(): a merge that protocol version 1 can't
// express, so they are only sent to a testee that speaks version 2.  (A
// MergeFrom() whose source is merely parsed and which precedes
// DiscardUnknownFields() is the same request as the Merge*() shortcut above,
// on either version, so there is no point in repeating those tests with it.)

// Merging a parsed message into an empty one yields the parsed message, known
// and unknown fields alike: an empty message contributes nothing to the merge.
// (The payload is in field number order with the unknown field last, which is
// how the parsed message serializes on its own, so the bytes can be compared.)
TEST_P(MergeFromTest, MergedIntoNewMessageEqualsParsedMessage) {
  const Wire payload = Wire(
      VarintField(SingularField(FieldDescriptor::TYPE_INT32), 1),
      LengthPrefixedField(SingularField(FieldDescriptor::TYPE_STRING), "a"),
      VarintField(kUnknownFieldNumber, 3));
  EXPECT_THAT(Testee()
                  .New(message())
                  .MergeFrom(Testee().ParseBinary(message(), payload))
                  .SerializeBinary(),
              Yields(Payload(payload)));
}

// Discarding unknown fields before the merge (the order
// MergedUnknownFieldsDiscarded above can't have) only affects the message
// parsed so far: the merged message's unknown fields arrive afterwards and
// stay.
TEST_P(MergeFromTest, UnknownFieldsDiscardedBeforeMergeKeepsMergedOnes) {
  UnknownFieldSet expected;
  expected.AddVarint(kUnknownFieldNumber, 2);
  EXPECT_THAT(Testee()
                  .ParseBinary(message(), VarintField(kUnknownFieldNumber, 1))
                  .DiscardUnknownFields()
                  .MergeFrom(Testee().ParseBinary(
                      message(), VarintField(kUnknownFieldNumber, 2)))
                  .SerializeBinary(),
              Yields(ParsedPayload(HasUnknownFieldsInOrder(expected))));
}
