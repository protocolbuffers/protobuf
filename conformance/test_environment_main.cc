...
int main(int argc, char* argv[]) {
  ...
  if (!GTEST_FLAG_GET(list_tests)) {
    ...
    // The results written with --output_result_file name the implementation
    // after the testee binary, or after this binary for an in-process testee.
    if (options.implementation_name.empty()) {
      options.implementation_name = google::protobuf::conformance::internal::Basename(
          options.testee_binary.empty() ? positional_args[0]
                                        : options.testee_binary);
    }
