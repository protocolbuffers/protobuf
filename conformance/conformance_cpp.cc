...
absl::StatusOr<bool> Harness::ServeConformanceRequest() {
  uint32_t in_len;
  if (!ReadFile(stdin, reinterpret_cast<char*>(&in_len), sizeof(in_len)).ok()) {
  ...
  ABSL_RETURN_IF_ERROR(ReadFile(stdin, &serialized_input[0], in_len));
  ...
  ABSL_RETURN_IF_ERROR(WriteFile(stdout, &out_len, sizeof(out_len)));
  ABSL_RETURN_IF_ERROR(
      WriteFile(stdout, serialized_output.data(), serialized_output.size()));
  if (fflush(stdout) != 0) {
