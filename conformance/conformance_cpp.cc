...
//
// Usage: conformance_cpp [--dynamic]
//
//   --dynamic  Test DynamicMessage instead of the generated message classes
//              (see CppConformanceHarness::MessageImplementation).
...
#endif
  using MessageImplementation =
      google::protobuf::conformance::CppConformanceHarness::MessageImplementation;
  MessageImplementation implementation = MessageImplementation::kGenerated;
  for (int i = 1; i < argc; ++i) {
    if (absl::string_view(argv[i]) == "--dynamic") {
      implementation = MessageImplementation::kDynamic;
    } else {
      // Testees have always ignored their arguments, and the runner tests rely
      // on being able to pass this one arbitrary arguments, so only warn.
      ABSL_LOG(WARNING) << "conformance_cpp: ignoring unknown argument "
                        << argv[i] << " (the only option is --dynamic)";
    }
  }

  google::protobuf::conformance::CppConformanceHarness harness(implementation);
  int total_runs = 0;
  while (true) {
