...
//     handles edition gating, per-test statistics and the suite's default
//     priority (DefaultPriority(); see TestPriority in testee.h).
...
struct ConformanceEnvironmentOptions {
  ...
  // The conformance protocol version to speak to the testee (see
  // ConformanceRequest.protocol_version in conformance.proto).  0, the
  // default, detects it: SetUp() runs the discovery handshake
  // (internal::Testee::DetectProtocolVersion()) and speaks whatever the testee
  // answers, so a testee that doesn't take part in the handshake gets protocol
  // version 1, exactly as before there was a version 2.  N (between 1 and
  // internal::kLatestProtocolVersion; anything else fails SetUp()) pins the
  // version: the handshake still runs, SetUp() fails fatally if the testee
  // answers with an older version, and otherwise N is spoken even if the
  // testee answered with a newer one.  Tests that need a newer version than
  // the one spoken are skipped by the runner (see
  // internal::TestResult::Outcome::kUnsupportedProtocol).
  int protocol_version = 0;

  // Where to write the run's results as a ConformanceRunResult textproto
  // (conformance_result.proto): one ConformanceTestCaseResult per test that
  // ran (a test the runner's --test filter left out isn't listed) with its
  // coordinate identity (see ConformanceTestInfo in testee.h) and outcome,
  // plus the totals.  Empty (the default) writes nothing.  A relative path is
  // resolved like `fix_output_file`.  Unlike --fix, a partial run is written
  // too; it simply contains fewer results.  Under test sharding each shard
  // writes its own file, "<file>.shard<index>[.<ext>]" (e.g.
  // "cpp.shard0.textproto"), or nothing if TEST_SHARD_INDEX is unset.  The
  // file is the input of report_generator.py (see README.md).
  //
  // Test numbers (the third coordinate part) are assigned to the distinct
  // test cases of each section in sorted order, so two runs of the same set
  // of tests agree, but a run of a different set of tests (--gtest_filter, a
  // shard, another suite set) can number the same test differently.  Compare
  // runs by test_name, not by coordinate, until the numbers are checked in.
  // TODO: b/564551336 - assign numbers from a checked-in table.
  std::string result_file;
  // The implementation_name of the written ConformanceRunResult.  Defaults to
  // the basename of `testee_binary`, or, when the testee isn't a binary (an
  // in-process testee, a caller-owned runner), the basename of the test
  // binary itself.
...
struct Statistics {
  ...
  // The tests that weren't sent because they need a newer conformance
  // protocol than the testee speaks; see
  // TestManager::unsupported_protocol_tests().
  ...
  // expected_successes, unexpected_successes and unsupported_protocol_tests.
...
// The last component of `path` ("a/b/c" -> "c", "c" -> "c"), on either kind
// of slash.  This is how the default implementation_name of a
// ConformanceRunResult is derived from a binary's path (see
// ConformanceEnvironmentOptions::implementation_name).
...
class ConformanceEnvironment : public testing::Environment {
 public:
  ...
  // Loads the failure lists and runs the discovery handshake that settles the
  // conformance protocol version to speak to the testee (see
  // ConformanceEnvironmentOptions::protocol_version).  Any failure here is
  // fatal, so no tests run.  Run by gtest before the first test (see
  // Install()).
  void SetUp() override;

  // Records the run's statistics as test properties, writes the run's results
  // to `result_file` if one was given, rewrites the failure list
 ...
 private:
  ...
  // Writes the ConformanceRunResult of the results recorded so far to
  // `options_.result_file` (see the option); a no-op when it is empty.  A
  // write failure is a gtest failure.
  void WriteRunResult();
...
// Creates a test against the global testee, named after the current gtest
// test (see internal::TestName):
//
...
// is kP0.  The overload takes the priority explicitly instead, for the tests
// of a suite that are more or less important than the rest of it:
// Testee(kP3).
//
// A test body that sends more than one request with the same input and output
// formats gives each its own `suffix` (e.g. Testee("Drop") and
// Testee("Print")); a duplicated full name is fatal.  The suffix becomes one
// segment of the name, so it may only contain [A-Za-z0-9_].
