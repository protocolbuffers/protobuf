...
void TestManager::AppendSavedLines(absl::string_view line,
  ...
  // a different message.  A test that wasn't sent to the testee (unsupported
  // protocol) is no news about the entry.
  const bool changed = absl::c_any_of(matches, [](const auto& match) {
    return match.second.kind != MatchOutcome::kExpectedFailure &&
           match.second.kind != MatchOutcome::kUnsupportedProtocol;
  });
  if (!changed) {
    lines.emplace_back(line);
    return;
  ...
  for (const auto& [test_name, outcome] : matches) {
    switch (outcome.kind) {
      ...
      case MatchOutcome::kUnsupportedProtocol:
...
void TestManager::ReportUnsupportedProtocol(absl::string_view test_name) {
  bool unique = InsertUniqueTest(seen_tests_, test_name);
  IncrementIfUnique(unique, unsupported_protocol_tests_);
  absl::optional<std::string> failure_match = MarkMatched(test_name);
  if (!failure_match.has_value()) {
    return;
  }
  // The test exists, so the entry isn't unseen; but nothing is known about
  // its outcome, so SaveFailureList() keeps the entry as it is.  Like a skip
  // it doesn't count toward the entry's wildcard expansion cap.
  unseen_expected_failures_.erase(*failure_match);
  entry_matches_[*failure_match][test_name] = {
      /*kind=*/MatchOutcome::kUnsupportedProtocol};
}
...
void TestManager::RecordCaseResult(
    ::conformance::ConformanceTestCaseResult result) {
  case_results_.push_back(std::move(result));
}

::conformance::ConformanceRunResult TestManager::BuildRunResult(
    absl::string_view implementation_name) const {
  using ::conformance::ConformanceTestCaseResult;
  ::conformance::ConformanceRunResult run_result;
  run_result.set_implementation_name(implementation_name);

  // Test numbers were assigned in first-appearance order as the tests ran
  // (see TestNumberer in taxonomy.h); reassign them from the sorted distinct
  // (section, test case) pairs so that the run order (--gtest_shuffle, the
  // order suites happen to be linked in) doesn't change them.
  // TODO: b/564551336 - assign numbers from a checked-in table instead, so
  // that the set of tests run doesn't change them either.
  absl::btree_map<std::string, absl::btree_map<std::string, int>> numbers;
  for (const ConformanceTestCaseResult& result : case_results_) {
    numbers[result.section_coordinate()][result.test_case()] = 0;
  }
  for (auto& [section_coordinate, test_cases] : numbers) {
    int next = 0;
    for (auto& [test_case, number] : test_cases) number = ++next;
  }
  ...
  int crashed = 0;
  for (const ConformanceTestCaseResult& result : case_results_) {
    ConformanceTestCaseResult& exported = *run_result.add_results();
    exported = result;
    exported.set_test_number(
        numbers[result.section_coordinate()][result.test_case()]);
    exported.set_coordinate(
        absl::StrFormat("%s.%03d.%02d", exported.section_coordinate(),
                        exported.test_number(), exported.variant_number()));
    switch (result.status()) {
      case ConformanceTestCaseResult::PASS_CANONICAL:
      case ConformanceTestCaseResult::PASS_ALTERNATE:
      case ConformanceTestCaseResult::PASS_OTHER:
        ++passed;
        break;
      case ConformanceTestCaseResult::FAILURE:
        ++failed;
        break;
      case ConformanceTestCaseResult::SKIPPED:
        ++skipped;
        break;
      case ConformanceTestCaseResult::CRASH:
        ++crashed;
        break;
      default:
        break;
  ...
  run_result.set_total_tests(static_cast<int>(case_results_.size()));
  run_result.set_passed_tests(passed);
  run_result.set_failed_tests(failed);
  run_result.set_skipped_tests(skipped);
  run_result.set_crashed_tests(crashed);
  return run_result;
}
