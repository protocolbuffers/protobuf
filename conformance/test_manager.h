...
class TestManager {
 public:
  ...
  // Reports a test the runner didn't send because it needs a newer conformance
  // protocol than the testee speaks (see
  // TestResult::Outcome::kUnsupportedProtocol in testee.h).  Such a test is
  // neither a failure nor a skip: nothing is known about how it would come
  // out, so it is only counted (see unsupported_protocol_tests()).  The entry
  // its name matches, if any, counts as matched and seen (Finalize() doesn't
  // complain about it, since the test does exist) and is kept as it is by
  // SaveFailureList(), whether or not the test would still fail on a testee
  // that could run it.
  void ReportUnsupportedProtocol(absl::string_view test_name);
  ...
  // Records the exported result of a test: its identity (see
  // ConformanceTestInfo in testee.h) and outcome, as written to
  // --output_result_file.  The matchers (matchers.h) record one for every
  // result they report, alongside the Report*() call; recording is
  // independent of the failure list and of the statistics above.
  void RecordCaseResult(::conformance::ConformanceTestCaseResult result);

  // The results recorded so far, in recording order.
  const std::vector<::conformance::ConformanceTestCaseResult>& case_results()
      const {
    return case_results_;
  }

  // Bundles the recorded results into a run result for `implementation_name`
  // with the totals by status: passed counts PASS_CANONICAL, PASS_ALTERNATE
  // and PASS_OTHER; failed, skipped and crashed count FAILURE, SKIPPED and
  // CRASH; total counts every result.  `timestamp_epoch_sec` is left for the
  // caller to set.
  ::conformance::ConformanceRunResult BuildRunResult(
  ...
  // The number of tests that were not sent to the testee because they need a
  // newer conformance protocol than it speaks.  See
  // ReportUnsupportedProtocol().
  int unsupported_protocol_tests() const { return unsupported_protocol_tests_; }
 ...
 private:
  ...
  struct MatchOutcome {
    enum Kind {
      ...
      // Wasn't sent to the testee (see ReportUnsupportedProtocol()); saved
      // like kExpectedFailure, since nothing newer is known.
  ...
  std::vector<::conformance::ConformanceTestCaseResult> case_results_;

  int skipped_ = 0;
  int listed_skips_ = 0;
  int tolerated_recommended_failures_ = 0;
  int unsupported_protocol_tests_ = 0;
