...
// DynamicMessage (cpp_in_process_test_runner_test.cc covers that helper), so
// all there is to observe here is that the runner it returns answers requests;
// by design, nothing in a response tells a DynamicMessage testee from a
// generated one.
TEST(CppDynamicInProcessTesteeTest, AnswersRequests) {
...
TEST(CppDynamicInProcessTesteeDeathTest, RejectsATesteeBinary) {
  EXPECT_DEATH(
      MakeTesteeRunner("/some/testee", {}),
      "--testee_binary=/some/testee was given, but this test links "
      "the C\\+\\+ testee in-process \\(cpp_dynamic_in_process_testee\\)");
}

TEST(CppDynamicInProcessTesteeDeathTest, RejectsTesteeArgs) {
