...
// mkdir() and its MSVC CRT spelling.
...
std::string NewOutputDir() {
  ...
  std::string dir =
      absl::StrCat(testing::TempDir(), test_info.test_suite_name(), "_",
                   test_info.name(), "/");
  ABSL_PCHECK(MakeDirectory(dir) == 0 || errno == EEXIST) << dir;
  return dir;
}
...
TEST(ParseConformanceRunnerArgsTest, Defaults) {
  ...
  EXPECT_THAT(options->output_result_file, IsEmpty());
...
TEST(ParseConformanceRunnerArgsTest, ParsesEveryFlag) {
  absl::StatusOr<ConformanceRunnerOptions> options =
      Parse({"--performance", "--debug", "--verbose", "--enforce_recommended",
             ...
             "Some.Test", "--failure_list", "list.txt", "--protocol_version",
             "2", "--output_result_file", "result.textproto",
             "--implementation_name", "cpp", "testee"});
  ASSERT_THAT(options, IsOk());
  ...
  EXPECT_EQ(options->output_result_file, "result.textproto");
...
// The parser only checks that --protocol_version is a non-negative integer;
// whether the runner speaks that version is checked when the environment is
// set up, which is where the latest version is known.
TEST(ParseConformanceRunnerArgsTest, ProtocolVersion) {
  for (absl::string_view invalid : {"bogus", "-1", "2.0", "1x", ""}) {
    EXPECT_THAT(Parse({"--protocol_version", std::string(invalid), "testee"}),
                StatusIs(absl::StatusCode::kInvalidArgument,
                         HasSubstr(absl::StrCat("Invalid protocol version: ",
                                                invalid))))
        << invalid;
  }

  absl::StatusOr<ConformanceRunnerOptions> options =
      Parse({"--protocol_version", "1", "testee"});
  ASSERT_THAT(options, IsOk());
  EXPECT_EQ(options->protocol_version, 1);

  // 0 is the default, "detect it", spelled out.
  options = Parse({"--protocol_version", "0", "testee"});
  ASSERT_THAT(options, IsOk());
  EXPECT_EQ(options->protocol_version, 0);

  // Out of range versions are the environment's to reject.
  options = Parse({"--protocol_version", "99", "testee"});
  ASSERT_THAT(options, IsOk());
  EXPECT_EQ(options->protocol_version, 99);
}
...
TEST(PrintConformanceRunnerUsageTest, DocumentsEveryOption) {
  ...
  for (absl::string_view option :
       {"--failure_list", "--text_format_failure_list", "--enforce_recommended",
        ...
        "--performance", "--protocol_version", "--verbose",
        "--output_result_file", "--implementation_name",
        "--gtest_<flag>=<value>"}) {
