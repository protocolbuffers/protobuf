...
cp "${TEST_SRCDIR}/google3/third_party/protobuf/conformance/update_failure_list_tests/to_update_failure_list.txt" \
...
"${TEST_SRCDIR}/google3/third_party/protobuf/conformance/update_failure_list" \
  --add "${TEST_SRCDIR}/google3/third_party/protobuf/conformance/update_failure_list_tests/failing_tests.txt" \
  --remove "${TEST_SRCDIR}/google3/third_party/protobuf/conformance/update_failure_list_tests/succeeding_tests.txt" \
  "${TEST_UNDECLARED_OUTPUTS_DIR}/to_update_failure_list.txt"

# Compare the to-update file with golden_failure_list.txt for content equality
if cmp -s "${TEST_SRCDIR}/google3/third_party/protobuf/conformance/update_failure_list_tests/golden_failure_list.txt" \
