...
TEST_F(TestManagerTest, ReportUnsupportedProtocol) {
  CreateFailureList({{"foo", "abc"}, {"bar.*", "abc"}, {"baz", "abc"}});
  TestManager manager;
  ASSERT_THAT(manager.LoadFailureList(failure_list()), IsOk());

  manager.ReportUnsupportedProtocol("foo");
  manager.ReportUnsupportedProtocol("bar.x");
  manager.ReportUnsupportedProtocol("unlisted");
  // Reporting a test twice counts it once.
  manager.ReportUnsupportedProtocol("unlisted");

  // The tests exist, so the entries they match are matched and seen; but
  // nothing is known about their outcome, so they are neither successes,
  // failures nor skips.
  EXPECT_THAT(manager.UnmatchedExpectedFailures(), ElementsAre("baz"));
  EXPECT_THAT(manager.UnseenExpectedFailures(), ElementsAre("baz"));
  EXPECT_THAT(manager.Finalize(),
              StatusIs(absl::StatusCode::kFailedPrecondition,
                       HasSubstr("were not seen: baz")));
  EXPECT_EQ(manager.unsupported_protocol_tests(), 3);
  EXPECT_EQ(manager.expected_failures(), 0);
  EXPECT_EQ(manager.unexpected_failures(), 0);
  EXPECT_EQ(manager.expected_successes(), 0);
  EXPECT_EQ(manager.unexpected_successes(), 0);
...
TEST_F(TestManagerTest, ReportUnsupportedProtocolIsCountedOnceAcrossKinds) {
  CreateFailureList({});
  TestManager manager;
  ASSERT_THAT(manager.LoadFailureList(failure_list()), IsOk());
  ...
  EXPECT_EQ(manager.unsupported_protocol_tests(), 1);
  EXPECT_EQ(manager.expected_successes(), 0);
}
...
::conformance::ConformanceTestCaseResult CaseResult(
    absl::string_view test_name,
    ::conformance::ConformanceTestCaseResult::Status status,
    absl::string_view section_coordinate = "01.01", int test_number = 0,
    int variant_number = 0) {
  ::conformance::ConformanceTestCaseResult result;
  result.set_test_name(test_name);
  result.set_status(status);
  result.set_section_coordinate(section_coordinate);
  result.set_test_case(test_name);
  result.set_test_number(test_number);
  result.set_variant_number(variant_number);
  result.set_coordinate(absl::StrFormat("%s.%03d.%02d", section_coordinate,
                                        test_number, variant_number));
  return result;
}

TEST_F(TestManagerTest, CaseResultsStartEmpty) {
  TestManager manager;
  EXPECT_THAT(manager.case_results(), IsEmpty());
  EXPECT_THAT(manager.BuildRunResult("cpp"), EqualsProto(R"pb(
...
TEST_F(TestManagerTest, RecordCaseResultKeepsEveryResultInOrder) {
  using ::conformance::ConformanceTestCaseResult;
  TestManager manager;

  manager.RecordCaseResult(
      CaseResult("a", ConformanceTestCaseResult::PASS_CANONICAL));
  manager.RecordCaseResult(CaseResult("b", ConformanceTestCaseResult::FAILURE));
  manager.RecordCaseResult(
      CaseResult("a", ConformanceTestCaseResult::PASS_CANONICAL));

  ASSERT_THAT(manager.case_results(), SizeIs(3));
  EXPECT_EQ(manager.case_results()[0].test_name(), "a");
  EXPECT_EQ(manager.case_results()[0].status(),
            ConformanceTestCaseResult::PASS_CANONICAL);
  EXPECT_EQ(manager.case_results()[1].test_name(), "b");
  EXPECT_EQ(manager.case_results()[1].status(),
            ConformanceTestCaseResult::FAILURE);
  EXPECT_EQ(manager.case_results()[2].test_name(), "a");
  EXPECT_EQ(manager.case_results()[2].status(),
            ConformanceTestCaseResult::PASS_CANONICAL);
  EXPECT_THAT(manager.Finalize(), IsOk());
}

TEST_F(TestManagerTest, BuildRunResultCountsEveryStatus) {
  using ::conformance::ConformanceTestCaseResult;
  TestManager manager;
  manager.RecordCaseResult(
      CaseResult("canonical", ConformanceTestCaseResult::PASS_CANONICAL));
  manager.RecordCaseResult(
      CaseResult("alternate", ConformanceTestCaseResult::PASS_ALTERNATE));
  manager.RecordCaseResult(
      CaseResult("other", ConformanceTestCaseResult::PASS_OTHER));
  manager.RecordCaseResult(
      CaseResult("failed", ConformanceTestCaseResult::FAILURE));
  manager.RecordCaseResult(
      CaseResult("skipped", ConformanceTestCaseResult::SKIPPED));
  manager.RecordCaseResult(
      CaseResult("crashed", ConformanceTestCaseResult::CRASH));
  manager.RecordCaseResult(
      CaseResult("unknown", ConformanceTestCaseResult::STATUS_UNSPECIFIED));

  ::conformance::ConformanceRunResult run_result =
      manager.BuildRunResult("upb");

  EXPECT_EQ(run_result.implementation_name(), "upb");
  EXPECT_EQ(run_result.total_tests(), 7);
  EXPECT_EQ(run_result.passed_tests(), 3);
  EXPECT_EQ(run_result.failed_tests(), 1);
  EXPECT_EQ(run_result.skipped_tests(), 1);
  EXPECT_EQ(run_result.crashed_tests(), 1);
  EXPECT_THAT(run_result.results(), SizeIs(7));
  // The results keep their recording order.
  EXPECT_EQ(run_result.results(0).test_name(), "canonical");
  EXPECT_EQ(run_result.results(0).status(),
            ConformanceTestCaseResult::PASS_CANONICAL);
  // The timestamp is the caller's business.
  EXPECT_EQ(run_result.timestamp_epoch_sec(), 0);
  EXPECT_THAT(manager.Finalize(), IsOk());
}

TEST_F(TestManagerTest, BuildRunResultNumbersTestCasesInSortedOrder) {
  using ::conformance::ConformanceTestCaseResult;
  // Recorded in run order with the run's first-appearance numbers: b was seen
  // first in section 01.01, then a; c is alone in section 01.02.
  TestManager manager;
  manager.RecordCaseResult(CaseResult("b", ConformanceTestCaseResult::FAILURE,
                                      "01.01", /*test_number=*/1,
                                      /*variant_number=*/11));
  manager.RecordCaseResult(
      CaseResult("a", ConformanceTestCaseResult::PASS_CANONICAL, "01.01",
                 /*test_number=*/2, /*variant_number=*/21));
  manager.RecordCaseResult(CaseResult("b", ConformanceTestCaseResult::FAILURE,
                                      "01.01", /*test_number=*/1,
                                      /*variant_number=*/21));
  manager.RecordCaseResult(
      CaseResult("c", ConformanceTestCaseResult::PASS_CANONICAL, "01.02",
                 /*test_number=*/1, /*variant_number=*/11));

  ::conformance::ConformanceRunResult run_result =
      manager.BuildRunResult("cpp");

  // a comes before b in the sorted order, so their numbers swap; every
  // variant of b gets the same number; c's section is numbered on its own.
  // Everything else about the results is untouched.
  ASSERT_THAT(run_result.results(), SizeIs(4));
  EXPECT_EQ(run_result.results(0).test_number(), 2);
  EXPECT_EQ(run_result.results(0).coordinate(), "01.01.002.11");
  EXPECT_EQ(run_result.results(1).test_number(), 1);
  EXPECT_EQ(run_result.results(1).coordinate(), "01.01.001.21");
  EXPECT_EQ(run_result.results(2).test_number(), 2);
  EXPECT_EQ(run_result.results(2).coordinate(), "01.01.002.21");
  EXPECT_EQ(run_result.results(3).test_number(), 1);
  EXPECT_EQ(run_result.results(3).coordinate(), "01.02.001.11");
  EXPECT_THAT(run_result.results(0), EqualsProto(R"pb(
                test_name: "b"
                test_case: "b"
                section_coordinate: "01.01"
                test_number: 2
                variant_number: 11
                coordinate: "01.01.002.11"
                status: FAILURE
              )pb"));
  // The manager's own record is left as it was.
  EXPECT_EQ(manager.case_results()[0].test_number(), 1);
  EXPECT_THAT(manager.Finalize(), IsOk());
}
...
TEST_F(TestManagerTest, SaveFailureListKeepsUnsupportedProtocolEntry) {
  // Nothing is known about a test the runner couldn't send, so its entry is
  // kept as is (modulo the usual re-alignment of comments), wildcard or not,
  // and isn't reported as unseen.
  CreateFailureList(R"(
foo.*.bar # abc
qux       # def
)");
  TestManager manager;
  ASSERT_THAT(manager.LoadFailureList(failure_list()), IsOk());
  manager.ReportUnsupportedProtocol("foo.a.bar");
  manager.ReportUnsupportedProtocol("foo.b.bar");
  manager.ReportUnsupportedProtocol("qux");
  ASSERT_THAT(manager.Finalize(), IsOk());
...
TEST_F(TestManagerTest, SaveFailureListExpansionKeepsUnsupportedProtocolMatch) {
  // When a wildcard has to be expanded because one of its matches passed, a
  // match the runner couldn't send is kept with the entry's message, like
...
foo.*.bar # abc
  ...
  ASSERT_THAT(manager.ReportFailure("foo.a.bar", "abc"), IsOk());
  ASSERT_THAT(manager.ReportSuccess("foo.b.bar"), Not(IsOk()));
  manager.ReportUnsupportedProtocol("foo.c.bar");
  ASSERT_THAT(manager.Finalize(), IsOk());
