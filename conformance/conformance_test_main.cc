...
// text_conformance_tests), against the testee named on its command line.  The
// command line and the report format are those of the original runner, which
// ran hand-written suites instead (see conformance_test_runner.h), so nothing
// changes for its users, the test targets and scripts that invoke it with
// their failure lists and --output_dir:
//
...
//     TODO: b/563657875 - goes away with this runner; conformance_test()
...
struct FailureListEntry {
  size_t report;  // An index into the FailureListReports.
  std::string message;
};

// The report block for `flag`, appended (with no files) if there is none yet.
...
bool ReportFailureListResults(const FailureListReport& report,
  ...
  const std::string failure_list =
      report.files.empty() ? "" : report.files.back();
  std::string output;
  ReportTestStatusSet(
...
absl::StatusOr<GtestPhaseResult> RunGtestPhase(
  ...
  // Only a full run can tell that an entry is stale: if only some of the
  // gtest tests ran (--gtest_filter, sharding, --gtest_fail_fast), or none at
  // all (in which case gtest didn't even set the environment up), an entry
  // they didn't match may well belong to a test that wasn't selected, so
  // nothing is reported rather than guessed.
