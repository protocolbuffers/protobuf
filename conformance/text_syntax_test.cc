...
TEST_P(TextFieldSeparatorTest, Comma) {
  ...
  EXPECT_THAT(Testee(TestName("FieldSeparatorCommaTopLevel"))
...
TEST_P(TextFieldSeparatorTest, Semi) {
  const std::string input = Input(";");
  EXPECT_THAT(Testee(TestName("FieldSeparatorSemiTopLevelSingle"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(Testee(TestName("FieldSeparatorSemiTopLevelSingle"))
...
TEST_P(TextFieldSeparatorTest, CommaDuplicatesFails) {
  EXPECT_THAT(Testee(TestName("FieldSeparatorCommaTopLevelDuplicatesFails"))
                  .ParseText(message(), Input(",,"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextFieldSeparatorTest, SemiDuplicateFails) {
  EXPECT_THAT(Testee(TestName("FieldSeparatorSemiTopLevelDuplicateFails"))
                  .ParseText(message(), Input(";;"))
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextListSeparatorTest, Comma) {
  ...
  EXPECT_THAT(Testee(TestName("ListSeparator"))
                  .ParseText(message(), input)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(input))));
  EXPECT_THAT(Testee(TestName("ListSeparator"))
...
TEST_P(TextListSeparatorTest, SemiFails) {
  EXPECT_THAT(
      Testee(TestName("ListSeparatorSemiFails"))
...
TEST_P(TextListSeparatorTest, Missing) {
  ...
  if (test_case().adjacent_values_concatenate) {
    EXPECT_THAT(Testee(TestName("ListSeparatorMissingIsOneValue"))
                    .ParseText(message(), input)
                    .SerializeBinary(),
                Yields(ParsedPayload(EqualsTextProto(input))));
    EXPECT_THAT(Testee(TestName("ListSeparatorMissingIsOneValue"))
    ...
    EXPECT_THAT(Testee(TestName("ListSeparatorMissingFails"))
                    .ParseText(message(), input)
                    .ParseOnly(),
                Yields(IsParseError()));
  }
...
TEST_P(TextListSeparatorTest, DuplicateFails) {
  EXPECT_THAT(
      Testee(TestName("ListSeparatorDuplicateFails"))
          .ParseText(message(), Input(absl::StrCat(value(), ",,", value())))
          .ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(TextListSeparatorTest, SingleTrailingFails) {
  EXPECT_THAT(Testee(TestName("ListSeparatorSingleTrailingFails"))
...
TEST_P(TextListSeparatorTest, TwoValuesTrailingFails) {
  EXPECT_THAT(
      Testee(TestName("ListSeparatorTwoValuesTrailingFails"))
          .ParseText(message(), Input(absl::StrCat(value(), ",", value(), ",")))
          .ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(TextNestedSeparatorTest, Comma) {
  EXPECT_THAT(Testee("FieldSeparatorCommaNested")
...
TEST_P(TextNestedSeparatorTest, Semi) {
  EXPECT_THAT(Testee("FieldSeparatorSemiNested")
                  .ParseText(message(), "optional_nested_message: { a: 123; }")
                  .SerializeBinary(),
              Yields(ParsedPayload(
                  EqualsTextProto("optional_nested_message: { a: 123; }"))));
...
TEST_P(TextNestedSeparatorTest, CommaDuplicates) {
  EXPECT_THAT(Testee("FieldSeparatorCommaNestedDuplicates")
                  .ParseText(message(), "optional_nested_message: { a: 123,, }")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextNestedSeparatorTest, SemiDuplicates) {
  EXPECT_THAT(Testee("FieldSeparatorSemiNestedDuplicates")
                  .ParseText(message(), "optional_nested_message: { a: 123;; }")
                  .ParseOnly(),
              Yields(IsParseError()));
}
