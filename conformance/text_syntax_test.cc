...
TEST_P(TextListSeparatorTest, Missing) {
  ...
  if (test_case().adjacent_values_concatenate) {
    EXPECT_THAT(
        Testee("IsOneValue").ParseText(message(), input).SerializeBinary(),
        Yields(ParsedPayload(EqualsTextProto(input))));
    EXPECT_THAT(
        Testee("IsOneValue").ParseText(message(), input).SerializeText(),
        Yields(ParsedPayload(EqualsTextProto(input))));
  } else {
    EXPECT_THAT(Testee("Fails").ParseText(message(), input).ParseOnly(),
                Yields(IsParseError()));
  }
}
