...
// ---------------------------------------------------------------------------
// Legacy JSON test-name layouts and ParseOnly() output override
// ---------------------------------------------------------------------------

TEST(TesteeTest, SerializeJsonDefaultNameIsTheUsualLayout) {
  MockTestRunner mock;
  Testee testee(&mock);
  ...
  TestResult result = Checked(
      testee.CreateTest("foo", TestStrictness::kRequired)
          .ParseJson(TestAllTypesProto2::descriptor(), "json")
          .SerializeJson(
              {.legacy_name = JsonSerializationOptions::LegacyName::kDefault}));

  EXPECT_EQ(result.name(), "Required.Proto2.JsonInput.foo.JsonOutput");
}

TEST(TesteeTest, SerializeJsonWithValidatorSuffixName) {
  MockTestRunner mock;
  Testee testee(&mock);
  EXPECT_CALL(
      mock,
      RunTest("Recommended.Proto2.JsonInput.foo.Validator", RequestEquals(R"pb(
                json_payload: "json"
                requested_output_format: JSON
                message_type: "protobuf_test_messages.proto2.TestAllTypesProto2"
                test_category: JSON_TEST
  ...
  TestResult result = Checked(
      testee.CreateTest("foo", TestStrictness::kRecommended)
          .ParseJson(TestAllTypesProto2::descriptor(), "json")
          .SerializeJson(
              {.legacy_name =
                   JsonSerializationOptions::LegacyName::kValidatorSuffix}));

  EXPECT_EQ(result.name(), "Recommended.Proto2.JsonInput.foo.Validator");
  EXPECT_EQ(result.strictness(), TestStrictness::kRecommended);
  EXPECT_THAT(result.format(), ::conformance::JSON);
  EXPECT_THAT(result.response(), EqualsProto(R"pb(json_payload: "{}")pb"));
}

// Only the name differs between the validator layout and the default one; the
// request is byte-identical.
TEST(TesteeTest, SerializeJsonValidatorSendsSameRequestAsDefault) {
  MockTestRunner mock;
  Testee testee(&mock);
  EXPECT_CALL(mock, RunTest)
      .WillRepeatedly(RespondWith(R"pb(json_payload: "{}")pb"));

  TestResult default_name =
      Checked(testee.CreateTest("foo", TestStrictness::kRequired)
                  .ParseJson(TestAllTypesProto2::descriptor(), "json")
                  .SerializeJson());
  TestResult validator = Checked(
      testee.CreateTest("foo", TestStrictness::kRequired)
          .ParseJson(TestAllTypesProto2::descriptor(), "json")
          .SerializeJson(
              {.legacy_name =
  ...
  EXPECT_EQ(default_name.name(), "Required.Proto2.JsonInput.foo.JsonOutput");
  EXPECT_EQ(validator.name(), "Required.Proto2.JsonInput.foo.Validator");
  EXPECT_EQ(validator.request().SerializeAsString(),
            default_name.request().SerializeAsString());
}

TEST(TesteeTest, SerializeJsonWithoutInputFormatName) {
  // The legacy ExpectSerializeFailureForJson layout: binary input, JSON
  // output, and no "ProtobufInput." component in the name.  (The legacy
  // runner sent test_category JSON_TEST for these; that is a separate knob.)
  MockTestRunner mock;
  Testee testee(&mock);
  EXPECT_CALL(
      mock,
      RunTest("Recommended.Proto2.foo.JsonOutput", RequestEquals(R"pb(
                protobuf_payload: "wire"
                requested_output_format: JSON
                message_type: "protobuf_test_messages.proto2.TestAllTypesProto2"
                test_category: BINARY_TEST
  ...
  TestResult result = Checked(
      testee.CreateTest("foo", TestStrictness::kRecommended)
          ...
          .SerializeJson(
              {.legacy_name =
                   JsonSerializationOptions::LegacyName::kWithoutInputFormat}));

  EXPECT_EQ(result.name(), "Recommended.Proto2.foo.JsonOutput");
  EXPECT_THAT(result.format(), ::conformance::JSON);
