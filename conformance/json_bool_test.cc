...
TEST_P(JsonBoolTest, BoolFieldTrue) {
  ...
  EXPECT_THAT(Testee(kP0, "BoolFieldTrue")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      Testee(kP0, "BoolFieldTrue").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonBoolTest, BoolFieldFalse) {
  ...
  EXPECT_THAT(Testee(kP0, "BoolFieldFalse")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee(kP0, "BoolFieldFalse")
...
TEST_P(JsonBoolTest, BoolFieldIntegerZero) {
  EXPECT_THAT(Testee("BoolFieldIntegerZero")
...
TEST_P(JsonBoolTest, BoolFieldIntegerOne) {
  EXPECT_THAT(Testee("BoolFieldIntegerOne")
...
TEST_P(JsonBoolTest, BoolFieldCamelCaseTrue) {
  EXPECT_THAT(Testee("BoolFieldCamelCaseTrue")
...
TEST_P(JsonBoolTest, BoolFieldCamelCaseFalse) {
  EXPECT_THAT(Testee("BoolFieldCamelCaseFalse")
...
TEST_P(JsonBoolTest, BoolFieldAllCapitalTrue) {
  EXPECT_THAT(Testee("BoolFieldAllCapitalTrue")
                  .ParseJson(message(), R"({"optionalBool":TRUE})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonBoolTest, BoolFieldAllCapitalFalse) {
  EXPECT_THAT(Testee("BoolFieldAllCapitalFalse")
                  .ParseJson(message(), R"({"optionalBool":FALSE})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonBoolTest, BoolFieldDoubleQuotedTrue) {
  EXPECT_THAT(Testee("BoolFieldDoubleQuotedTrue")
                  .ParseJson(message(), R"({"optionalBool":"true"})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonBoolTest, BoolFieldDoubleQuotedFalse) {
  EXPECT_THAT(Testee("BoolFieldDoubleQuotedFalse")
                  .ParseJson(message(), R"({"optionalBool":"false"})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
