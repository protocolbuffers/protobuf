...
//
// This header is deliberately gtest-free.  The fixtures and
// INSTANTIATE_TEST_SUITE_P name generators built on top of these helpers live
// in message_type_fixtures.h.
...
// send, and the repeated-field tests send all of them in this order.  Both the
// binary->binary and the binary->JSON legs iterate these tables.  Check-fails
// for TYPE_GROUP, which has no cases.
...
// ValidDataOneofBinary.<TYPE>.*), in the legacy order.
...
// field types" (valid data, premature EOF, ...); ValidDataCases() covers
// exactly these types.
