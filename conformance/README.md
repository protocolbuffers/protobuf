...
`P0.TextStringFieldTest.BadUtf8Octal/Proto3.TextFormatInput`, i.e.

```
<Level>.<Suite>.<Test>[/<Params>][.<Suffix>].<Input>Input[.<Output>Output]
```

`<Level>` is the test's priority, `P0` to `P3` (see "Priorities" below): `P0`
tests must pass for an implementation to be conformant; failures of `P3` tests
fail the run only under `--enforce_recommended`, which `conformance_test()`
passes by default. `<Suite>.<Test>[/<Params>]` is the name
...
`Testee(kP3).ParseJson(...)`. The priority's name, `P0`...`P3`, is the
`<Level>` of the test's name, so a failure list entry shows it.
...
```
P3.JsonDuplicateFieldNameTest.FieldNameDuplicate/Proto3.JsonInput  # Should have failed to parse, but didn't.
```

A `*` in an entry matches any run of characters up to the next `.` or `/`, so
`P3.JsonDuplicateFieldNameTest.FieldNameDuplicate/*.JsonInput` covers
the test under every message type,
`P0.ValidDataScalarTest.Scalar/*_INT32_0.ProtobufInput.ProtobufOutput` one
...
(`P0.FooTest.X/Proto3.ProtobufInput`,
`P0.FooTest.X/EditionsProto3.ProtobufInput`) rather than as
`P0.FooTest.X/*.ProtobufInput`.
...
and `enforce_recommended = False` tolerates failures of `P3` tests.
...
`P0.JsonFieldNameTest.FieldNameWithNumbers/*.JsonInput.ProtobufOutput`,
