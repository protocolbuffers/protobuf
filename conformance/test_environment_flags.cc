...
// --enforce_recommended, --maximum_edition, --protocol_version, --fix,
// --fix_output_file, --output_result_file and --implementation_name flags.
...
ABSL_FLAG(int, protocol_version, 0,
          "The conformance protocol version to speak to the testee.  0 (the "
          "default) detects it from the testee's answer to the discovery "
          "handshake; N pins it to N, which fails the run if the testee "
          "answers the handshake with an older version.  See "
          "ConformanceRequest.protocol_version in conformance.proto.");
...
ABSL_FLAG(std::string, fix_output_file, "",
          ...
          "in your workspace.");
ABSL_FLAG(std::string, output_result_file, "",
          "Write the run's results as a ConformanceRunResult textproto "
          "(conformance_result.proto) to this file at the end of the run, for "
          "report_generator.py.  A relative path is resolved under "
          "$BUILD_WORKSPACE_DIRECTORY when that is set, like "
          "--fix_output_file.  Off by default.");
ABSL_FLAG(std::string, implementation_name, "",
          "The implementation_name recorded in --output_result_file.  "
          "Defaults to the basename of --testee_binary, or of this binary for "
          "an in-process testee.");
...
ConformanceEnvironmentOptions OptionsFromFlags(
  ...
  options.protocol_version = absl::GetFlag(FLAGS_protocol_version);
  options.fix = absl::GetFlag(FLAGS_fix);
  options.fix_output_file = absl::GetFlag(FLAGS_fix_output_file);
  options.result_file = absl::GetFlag(FLAGS_output_result_file);
  options.implementation_name = absl::GetFlag(FLAGS_implementation_name);
  return options;
}
