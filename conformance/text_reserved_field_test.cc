...
TEST_P(TextReservedFieldTest, Boolean) {
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: true").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: true"))));
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: true").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: true"))));
}

TEST_P(TextReservedFieldTest, Integer) {
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: -123").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: -123"))));
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: -123").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: -123"))));
}

TEST_P(TextReservedFieldTest, Float) {
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: 0.123").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: 0.123"))));
  EXPECT_THAT(
      Testee().ParseText(message(), "reserved_field: 0.123").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("reserved_field: 0.123"))));
}
