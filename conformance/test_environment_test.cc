...
class ConformanceEnvironmentTest : public ::testing::Test {
 protected:
  ...
  // Runs a P0 test with the name suffix `suffix` against the global testee;
  // its name is TestName("P0", suffix).
  ...
  static std::string FooTestName() { return TestName("P0", "Foo"); }
  static std::string BarTestName() { return TestName("P0", "Bar"); }
...
TEST_F(ConformanceEnvironmentTest, InfersTestNameFromCurrentTest) {
  ...
  EXPECT_CALL(mock, RunTest("P0.ConformanceEnvironmentTest."
  ...
  EXPECT_CALL(mock, RunTest("P3.ConformanceEnvironmentTest."
                            "InfersTestNameFromCurrentTest.ProtobufInput."
                            "ProtobufOutput",
                            _))
      .WillOnce(Return(SerializedResponse(R"pb(protobuf_payload: "\010c")pb")));
...
TEST_P(ConformanceEnvironmentParamTest, InfersParamsFromCurrentTest) {
  ...
  EXPECT_CALL(mock, RunTest(absl::StrCat("P0.ConformanceEnvironmentParamTest.",
                                         test_info.name(),
...
TEST_F(ConformanceEnvironmentTest,
  ...
  std::string failure_list = WriteFile(
      "failures.txt",
      absl::StrCat(TestName("P3", "Foo"), " # Should have failed to parse\n"));
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.failure_list_files = {failure_list};
  options.enforce_recommended = false;
...
// ConformanceTest's DefaultPriority() is kP0.
TEST_F(FixtureTest, TesteeDefaultsToTheSuitePriority) {
  ...
  EXPECT_THAT(result.name(), testing::StartsWith("P0."));
  EXPECT_THAT(result,
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
}
...
TEST_F(FixtureTest, TesteeTakesAnExplicitPriority) {
  ...
  EXPECT_THAT(result.name(), testing::StartsWith("P3."));
  EXPECT_THAT(result,
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
}

TEST_F(FixtureTest, MiddlePrioritiesAreNamedAfterThemselves) {
  internal::TestResult p1 =
      Testee(kP1, "P1")
          .ParseBinary(TestAllTypesProto3::descriptor(), VarintField(1, 99))
          .SerializeBinary();
  EXPECT_EQ(p1.priority(), kP1);
  EXPECT_THAT(p1.name(), testing::StartsWith("P1."));
  ...
  EXPECT_THAT(p2.name(), testing::StartsWith("P2."));
