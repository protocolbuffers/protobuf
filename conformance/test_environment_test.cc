...
// A mock runner.  SetUp() sends every runner the discovery handshake before
// any test (under kProbeName, see test_runner.h); tests that don't care about
// it use a NiceMock, which answers it with an empty string, i.e. as a protocol
// version 1 testee.
...
TEST_F(ConformanceEnvironmentTest, NoResultFileIsWrittenByDefault) {
  internal::ScopedGlobalConformanceEnvironment env({/*runner=*/&fake_runner_});
  env->SetUp();
  EXPECT_THAT(RunRequired("Foo"),
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
  env->TearDown();
  // Nothing to check on disk; the point is that TearDown() doesn't fail.
  EXPECT_EQ(env->test_manager().case_results().size(), 1);
}

TEST_F(ConformanceEnvironmentTest, TearDownWritesTheResultFile) {
  MockTestRunner mock;
  std::string result_file = TmpPath("result.textproto");
  ConformanceEnvironmentOptions options;
  options.runner = &mock;
  options.result_file = result_file;
  options.implementation_name = "fake_impl";
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();

  EXPECT_CALL(mock, RunTest(FooTestName(), _))
      .WillOnce(Return(SerializedResponse(R"pb(protobuf_payload: "\010c")pb")));
  EXPECT_CALL(mock, RunTest(BarTestName(), _))
      .WillOnce(Return(SerializedResponse(R"pb(runtime_error: "boom")pb")));
  EXPECT_CALL(mock, RunTest(TestName("Required", "Baz"), _))
      .WillOnce(Return(SerializedResponse(R"pb(skipped: "not supported")pb")));
  EXPECT_THAT(RunRequired("Foo"),
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
  EXPECT_NONFATAL_FAILURE(
  ...
  EXPECT_THAT(RunRequired("Baz"), Yields(IsParseError()));
  env->TearDown();

  ::conformance::ConformanceRunResult run_result;
  ASSERT_TRUE(TextFormat::ParseFromString(ReadFile(result_file), &run_result))
      << ReadFile(result_file);
  EXPECT_EQ(run_result.implementation_name(), "fake_impl");
  EXPECT_GT(run_result.timestamp_epoch_sec(), 0);
  EXPECT_EQ(run_result.total_tests(), 3);
  EXPECT_EQ(run_result.passed_tests(), 1);
  EXPECT_EQ(run_result.failed_tests(), 0);
  EXPECT_EQ(run_result.skipped_tests(), 1);
  EXPECT_EQ(run_result.crashed_tests(), 1);
  ASSERT_EQ(run_result.results_size(), 3);

  const ::conformance::ConformanceTestCaseResult& foo = run_result.results(0);
  EXPECT_EQ(foo.status(),
            ::conformance::ConformanceTestCaseResult::PASS_CANONICAL);
  EXPECT_EQ(foo.legacy_test_name(), FooTestName());
  // This fixture isn't a conformance suite, so it is unmapped.
  EXPECT_EQ(foo.domain(), "unmapped");
  EXPECT_EQ(foo.section(), "conformance_environment");
  // Test numbers are assigned in sorted test case order when the file is
  // written, not in run order: bar, baz, foo.
  EXPECT_EQ(foo.test_number(), 3);
  EXPECT_EQ(foo.coordinate(), "00.00.003.01");
  EXPECT_EQ(foo.variant(), "pb2pb");
  EXPECT_EQ(foo.priority(), "P0");
  EXPECT_THAT(foo.test_name(), StartsWith("unmapped.conformance_environment."));

  const ::conformance::ConformanceTestCaseResult& bar = run_result.results(1);
  EXPECT_EQ(bar.status(), ::conformance::ConformanceTestCaseResult::CRASH);
  EXPECT_EQ(bar.error_message(),
            "Should have failed to parse, but raised an error instead.");
  EXPECT_EQ(bar.test_number(), 1);
  EXPECT_EQ(bar.coordinate(), "00.00.001.01");

  const ::conformance::ConformanceTestCaseResult& baz = run_result.results(2);
  EXPECT_EQ(baz.status(), ::conformance::ConformanceTestCaseResult::SKIPPED);
  EXPECT_EQ(baz.error_message(), "not supported");
}

TEST_F(ConformanceEnvironmentTest,
       ImplementationNameDefaultsToTheTesteeBinary) {
  // The testee binary is never spawned (ForkPipeRunner is lazy), but its
  // name is still the best default for the implementation name.
  std::string result_file = TmpPath("result.textproto");
  ConformanceEnvironmentOptions options;
  options.testee_binary = TmpPath("upb_testee");
  options.result_file = result_file;
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();
  env->TearDown();
  ...
  ::conformance::ConformanceRunResult run_result;
  ASSERT_TRUE(TextFormat::ParseFromString(ReadFile(result_file), &run_result));
  EXPECT_EQ(run_result.implementation_name(), "upb_testee");
  EXPECT_EQ(run_result.total_tests(), 0);
}

TEST_F(ConformanceEnvironmentTest, ImplementationNameFallsBackToUnknown) {
  std::string result_file = TmpPath("result.textproto");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = result_file;
  ...
  EXPECT_THAT(ReadFile(result_file),
              HasSubstr("implementation_name: \"unknown\""));
}

TEST_F(ConformanceEnvironmentTest, NoResultFileWithoutSetUp) {
  std::string result_file = TmpPath("result.textproto");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = result_file;
  ...
  EXPECT_FALSE(File::Exists(result_file));
}

TEST_F(ConformanceEnvironmentTest, RelativeResultFileIsResolvedInTheWorkspace) {
  std::string workspace = CreateTmpDir("workspace");
  ScopedEnvironmentVariable build_workspace("BUILD_WORKSPACE_DIRECTORY",
                                            workspace);
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = "out/result.textproto";
  options.implementation_name = "cpp";
  ABSL_CHECK_OK(File::RecursivelyCreateDir(absl::StrCat(workspace, "/out"),
                                           0777));
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();
  env->TearDown();

  EXPECT_THAT(ReadFile(absl::StrCat(workspace, "/out/result.textproto")),
              HasSubstr("implementation_name: \"cpp\""));
}

TEST_F(ConformanceEnvironmentTest, UnwritableResultFileFailsTearDown) {
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = TmpPath("no/such/dir/result.textproto");
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();
  EXPECT_NONFATAL_FAILURE(env->TearDown(), "Failed to open the result file");
}

TEST_F(ConformanceEnvironmentTest, PartialRunStillWritesTheResultFile) {
  // Unlike --fix, the export has no reason to refuse a --gtest_filter run: it
  // just lists fewer tests.
  std::string result_file = TmpPath("result.textproto");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = result_file;
  ...
  ::conformance::ConformanceRunResult run_result;
  ASSERT_TRUE(TextFormat::ParseFromString(ReadFile(result_file), &run_result));
  EXPECT_EQ(run_result.implementation_name(), "cpp");
  EXPECT_EQ(run_result.total_tests(), 1);
  ASSERT_EQ(run_result.results_size(), 1);
  EXPECT_EQ(run_result.results(0).legacy_test_name(), FooTestName());
}

TEST_F(ConformanceEnvironmentTest, EachShardWritesItsOwnResultFile) {
  ScopedEnvironmentVariable total_shards("TEST_TOTAL_SHARDS", "3");
  ScopedEnvironmentVariable shard_index("TEST_SHARD_INDEX", "2");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = TmpPath("result.textproto");
  options.implementation_name = "cpp";
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();
  env->TearDown();
  ...
  EXPECT_FALSE(
      File::Exists(TmpPath("result.textproto")));
  EXPECT_THAT(ReadFile(TmpPath("result.shard2.textproto")),
              HasSubstr("implementation_name: \"cpp\""));
}

TEST_F(ConformanceEnvironmentTest, ShardSuffixGoesAtTheEndWithoutExtension) {
  ScopedEnvironmentVariable total_shards("TEST_TOTAL_SHARDS", "2");
  ScopedEnvironmentVariable shard_index("TEST_SHARD_INDEX", "0");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = TmpPath("some.dir/result");
  ABSL_CHECK_OK(File::RecursivelyCreateDir(TmpPath("some.dir"),
                                           0777));
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  env->SetUp();
  env->TearDown();

  // The dot in the directory name isn't an extension.
  EXPECT_TRUE(
      File::Exists(TmpPath("some.dir/result.shard0")));
}

TEST_F(ConformanceEnvironmentTest, SingleShardWritesThePlainResultFile) {
  ScopedEnvironmentVariable total_shards("TEST_TOTAL_SHARDS", "1");
  ScopedEnvironmentVariable shard_index("TEST_SHARD_INDEX", "0");
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = TmpPath("result.textproto");
  ...
  EXPECT_TRUE(File::Exists(TmpPath("result.textproto")));
}

TEST_F(ConformanceEnvironmentTest, ShardWithoutAnIndexWritesNothing) {
  ScopedEnvironmentVariable total_shards("TEST_TOTAL_SHARDS", "3");
  ScopedEnvironmentVariable shard_index("TEST_SHARD_INDEX", absl::nullopt);
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.result_file = TmpPath("result.textproto");
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
...
TEST_F(ConformanceEnvironmentTest, SetUpDetectsTheProtocolVersion) {
  std::string failure_list = WriteFile(
      "failures.txt",
      absl::StrCat(FooTestName(), " # Should have failed to parse\n"));
  MockTestRunner mock;
  ...
  // The probe is the first and, until a test runs, only thing the testee
  // hears.  It is answered here as a version 2 testee.
  EXPECT_CALL(mock, RunTest(kProbeName, _))
      .WillOnce(Return(SerializedResponse(R"pb(
        results { serialized { binary: "" } }
        protocol_version: 2
      )pb")));
  env->SetUp();
  EXPECT_EQ(env->testee().protocol_version(), 2);

  // The probe isn't a test: nothing is counted and the failure list is
  // untouched.
  internal::Statistics statistics =
      internal::Statistics::From(env->test_manager());
  EXPECT_EQ(statistics.expected_successes, 0);
  EXPECT_EQ(statistics.unexpected_failures, 0);
  EXPECT_EQ(statistics.skipped_tests, 0);
  EXPECT_EQ(statistics.unsupported_protocol_tests, 0);
  EXPECT_THAT(env->test_manager().UnmatchedExpectedFailures(),
              ElementsAre(FooTestName()));

  // A version 2 testee gets action lists.
  EXPECT_CALL(mock, RunTest(FooTestName(), _))
      .WillOnce([](absl::string_view, absl::string_view input) {
        ::conformance::ConformanceRequest request;
        ABSL_CHECK(request.ParseFromString(input));
        EXPECT_EQ(request.protocol_version(), 2);
        EXPECT_EQ(request.actions_size(), 2);
        return SerializedResponse(
            R"pb(results { serialized { binary: "\010c" } })pb");
      });
  EXPECT_THAT(RunRequired("Foo"), Yields(IsParseError()));
  EXPECT_EQ(env->test_manager().expected_failures(), 1);
  env->TearDown();
...
TEST_F(ConformanceEnvironmentTest, SetUpDefaultsToVersion1ForASilentTestee) {
  // fake_runner_ never sets the response's protocol_version, like every
  // testee that predates the handshake.
  internal::ScopedGlobalConformanceEnvironment env({/*runner=*/&fake_runner_});
  env->SetUp();
  EXPECT_EQ(env->testee().protocol_version(), 1);
  EXPECT_THAT(RunRequired("Foo"),
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
  env->TearDown();
}

TEST_F(ConformanceEnvironmentTest, PinnedVersionIsSpokenWhenTheTesteeAllows) {
  MockTestRunner mock;
  ConformanceEnvironmentOptions options;
  options.runner = &mock;
  ...
  EXPECT_CALL(mock, RunTest(kProbeName, _))
      .WillOnce(Return(SerializedResponse(R"pb(protocol_version: 2)pb")));
  env->SetUp();
  EXPECT_EQ(env->testee().protocol_version(), 1);

  // ...so a version 2 testee still gets the flat request.
  EXPECT_CALL(mock, RunTest(FooTestName(), _))
      .WillOnce([](absl::string_view, absl::string_view input) {
        ::conformance::ConformanceRequest request;
        ABSL_CHECK(request.ParseFromString(input));
        EXPECT_EQ(request.protocol_version(), 0);
        EXPECT_EQ(request.protobuf_payload(), VarintField(1, 99).data());
        return SerializedResponse(R"pb(protobuf_payload: "\010c")pb");
      });
  EXPECT_THAT(RunRequired("Foo"),
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 99"))));
...
TEST_F(ConformanceEnvironmentTest, ViolatedPinIsFatalInSetUp) {
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  ...
  EXPECT_TRUE(failures.GetTestPartResult(0).fatally_failed());
  EXPECT_THAT(failures.GetTestPartResult(0).message(),
              HasSubstr("--protocol_version=2 pins the testee to protocol v2, "
                        "but it answered the probe as v1"));
  EXPECT_EQ(env->testee().protocol_version(), 1);
...
TEST_F(ConformanceEnvironmentTest, OutOfRangePinIsFatalInSetUp) {
  ConformanceEnvironmentOptions options;
  options.runner = &fake_runner_;
  options.protocol_version = internal::kLatestProtocolVersion + 1;
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  ...
  EXPECT_THAT(failures.GetTestPartResult(0).message(),
              HasSubstr(absl::StrCat(
                  "--protocol_version=", internal::kLatestProtocolVersion + 1,
                  " is out of range")));
  internal::ScopedPartialRunOverride full_run(false);
  env->TearDown();
}

TEST_F(ConformanceEnvironmentTest, UnansweredProbeIsFatalInSetUp) {
  MockTestRunner mock;
  ConformanceEnvironmentOptions options;
  options.runner = &mock;
  internal::ScopedGlobalConformanceEnvironment env(std::move(options));
  EXPECT_CALL(mock, RunTest(kProbeName, _))
      .WillOnce(Return(SerializedResponse(
          R"pb(timeout_error: "timeout from test program")pb")));
...
TEST_F(ConformanceEnvironmentTest, StatisticsCountUnsupportedProtocolTests) {
  internal::ScopedGlobalConformanceEnvironment env({/*runner=*/&fake_runner_});
  env->SetUp();
  ASSERT_EQ(env->testee().protocol_version(), 1);
  internal::Statistics before = internal::Statistics::From(env->test_manager());

  // A test that needs version 2 isn't sent to the version 1 testee; Yields()
  // skips the gtest test, which is intercepted here so that this test isn't
  // the one skipped.
  testing::TestPartResultArray part_results;
  {
    testing::ScopedFakeTestPartResultReporter reporter(
        ...
        &part_results);
    EXPECT_THAT(
        Testee("Foo").New(TestAllTypesProto3::descriptor()).SerializeBinary(),
  ...
  EXPECT_THAT(part_results.GetTestPartResult(0).message(),
              HasSubstr("requires conformance protocol v2 (the testee speaks "
                        "v1)"));

  internal::Statistics delta =
      internal::Statistics::From(env->test_manager()) - before;
  EXPECT_EQ(delta.unsupported_protocol_tests, 1);
  EXPECT_EQ(delta.skipped_tests, 0);
  EXPECT_EQ(delta.expected_successes, 0);
  EXPECT_EQ(delta.unexpected_failures, 0);
  ...
  total += delta;
  total += delta;
  EXPECT_EQ(total.unsupported_protocol_tests, 2);
  env->TearDown();
}
...
TEST_F(ConformanceEnvironmentTest, SpawnsForkPipeRunnerLazily) {
  ...
  // first use, so constructing the environment must succeed.  SetUp() is that
  // first use (the protocol discovery handshake): a testee that can't be
  // started is reported by the runner like one that crashed, i.e. as an
  // unversioned answer, so SetUp() succeeds with protocol version 1 and the
  // failure surfaces on each test instead.
  ConformanceEnvironmentOptions options;
