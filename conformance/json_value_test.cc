...
// asked for JSON output under "<name>.Validator" and inspected the JSON text.
// ExpectSerializeFailureForJson() sent the serialized text-format message as
// binary input, categorized as a JSON test, under "<name>.JsonOutput" without
// an input prefix; the same bytes are built here with the wire-format
// builders, and ParseBinary() categorizes them as BINARY_TEST, which no
// testee tells apart from JSON_TEST on a binary payload (b/563657722).
...
TEST_P(JsonValueTest, ValueRejectInfNumberValue) {
  EXPECT_THAT(
      RecommendedTest("ValueRejectInfNumberValue")
          .ParseBinary(
              message(),
              LengthPrefixedField(
                  kOptionalValue,
