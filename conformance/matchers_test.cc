...
constexpr absl::string_view kRequiredFoo =
    "P0.YieldsTest.foo.ProtobufInput.ProtobufOutput";
constexpr absl::string_view kRecommendedFoo =
    "P3.YieldsTest.foo.ProtobufInput.ProtobufOutput";
...
TEST_F(YieldsTest, TextFormatOutput) {
  ...
  EXPECT_THAT(
      test_manager_.UnexpectedFailures(),
      ElementsAre(
          FieldsAre("P0.YieldsTest.mismatch.ProtobufInput.TextFormatOutput",
                    "Output was not equivalent to reference message: "
                    "modified: optional_int32: 1 -> 2",
                    absl::nullopt),
          FieldsAre("P0.YieldsTest.unparseable.ProtobufInput.TextFormatOutput",
                    "TEXT_FORMAT output we received from test was unparseable.",
                    absl::nullopt)));
}
...
TEST_F(YieldsTest, ListedFailureViaWildcardPasses) {
  ASSERT_THAT(test_manager_.AddExpectedFailure(
                  "P0.YieldsTest.foo.*.ProtobufOutput", kNotAParseError),
              IsOk());
...
TEST_F(YieldsTest, ListedSuccessViaWildcardNamesTheWildcard) {
  ASSERT_THAT(test_manager_.AddExpectedFailure(
                  "P0.YieldsTest.foo.*.ProtobufOutput", ""),
              IsOk());

  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kParseError), Yields(IsParseError())),
      "test P0.YieldsTest.foo.ProtobufInput.ProtobufOutput (matched to "
      "P0.YieldsTest.foo.*.ProtobufOutput) is in the failure list, "
...
TEST_F(YieldsTest, NotComposes) {
  ...
  EXPECT_THAT(Explain(matcher, result),
              Rejects(absl::StrCat(
                  ...
                  "P0.YieldsTest.second.ProtobufInput.ProtobufOutput")));
  EXPECT_NONFATAL_FAILURE(EXPECT_THAT(result, matcher),
                          "which doesn't match (is not a parse error)");
  EXPECT_EQ(GetCounts(test_manager_),
            (Counts{/*expected_successes=*/1, /*expected_failures=*/0,
...
TEST_F(YieldsTest, CheckingAPassedResultAgainFails) {
  ...
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(result, matcher),
      "TestResult for P0.YieldsTest.foo.ProtobufInput.ProtobufOutput was "
...
TEST_F(YieldsTest, JsonOutput) {
  ...
  EXPECT_THAT(
      test_manager_.UnexpectedFailures(),
      ElementsAre(
          FieldsAre("P0.YieldsTest.invalid.JsonInput.JsonOutput",
                    testing::StartsWith(
                        "JSON payload cannot be parsed as valid JSON: "),
                    absl::nullopt),
          FieldsAre("P0.YieldsTest.mismatch.JsonInput.JsonOutput",
                    "Expect: JSON payload has member \"optionalInt64\", but "
                    "got: \"{\\\"optionalInt32\\\": 9}\"",
                    absl::nullopt),
          FieldsAre("P0.YieldsTest.unparseable.JsonInput.JsonOutput",
                    "JSON output we received from test was unparseable.",
                    absl::nullopt)));
}
