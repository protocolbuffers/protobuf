...
TestResult CreateResult(absl::string_view test_name, ConformanceRequest request,
  ...
  TestResult result = TestResult::ForTesting(
      test_name, priority, TestAllTypesProto2::descriptor(), std::move(request),
      std::move(response));
  result.MarkChecked();
  return result;
}
...
struct Counts {
  ...
  int unsupported_protocol_tests = 0;
  ...
  bool operator==(const Counts& other) const {
    return expected_successes == other.expected_successes &&
           ...
           unsupported_protocol_tests == other.unsupported_protocol_tests;
  }
  ...
  friend void PrintTo(const Counts& counts, std::ostream* os) {
    *os << "{expected_successes: " << counts.expected_successes
        ...
        << ", unsupported_protocol_tests: " << counts.unsupported_protocol_tests
        << "}";
  }
};
...
Counts GetCounts(const TestManager& manager) {
  return Counts{
      ...
      /*unsupported_protocol_tests=*/manager.unsupported_protocol_tests(),
  };
}
...
class YieldsTest : public testing::Test {
 protected:
  ...
  TestResult Run(absl::string_view response,
    ...
    return testee_.CreateTest(Named(name), priority)
  ...
  // Runs the test `name`, which needs protocol version 2 (the message is
  // created empty), against the version 1 testee: the runner never sends it.
  TestResult RunUnsupported(TestPriority priority = TestPriority::kP0,
                            absl::string_view name = "foo") {
    return testee_.CreateTest(Named(name), priority)
...
// The name RunUnsupported() produces for the test named "foo".
constexpr absl::string_view kUnsupportedFoo =
    "Required.YieldsTest.foo.EmptyInput.ProtobufOutput";
constexpr absl::string_view kUnsupportedMessage =
...
TEST_F(YieldsTest, RecommendedFailureIsToleratedWhenNotEnforced) {
  ...
  auto matcher = Yields(IsParseError());
  TestResult result = Run(kPayload2, TestPriority::kP3);
...
TEST_F(YieldsTest, RecommendedFailureFailsWhenEnforced) {
  ...
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kPayload2, TestPriority::kP3), Yields(IsParseError())),
      "Should have failed to parse, but didn't.");
  EXPECT_EQ(GetCounts(test_manager_), (Counts{/*expected_successes=*/0,
                                              /*expected_failures=*/0,
                                              /*unexpected_successes=*/0,
...
TEST_F(YieldsTest, RecommendedSuccessPasses) {
  ...
  EXPECT_THAT(Run(kParseError, TestPriority::kP3), Yields(IsParseError()));
...
// A test the runner didn't send because it needs a newer protocol than the
// testee speaks passes, is counted as unsupported, skips the gtest test with
// the reason, and never reaches the inner matcher.
TEST_F(YieldsTest, UnsupportedProtocolSkipsTheTest) {
  absl::ScopedMockLog log;
  EXPECT_CALL(log, Log(_, _, HasSubstr(kUnsupportedFoo))).Times(0);
  log.StartCapturingLogs();
  ...
  TestResult result = RunUnsupported();
  ASSERT_EQ(result.outcome(), TestResult::Outcome::kUnsupportedProtocol);

  // Yields() skips the current gtest test; intercept the skip so that this
  // test isn't the one skipped.
  testing::TestPartResultArray part_results;
  testing::AssertionResult explanation = testing::AssertionFailure();
  {
    testing::ScopedFakeTestPartResultReporter reporter(
        testing::ScopedFakeTestPartResultReporter::
            INTERCEPT_ONLY_CURRENT_THREAD,
        &part_results);
    explanation = Explain(matcher, result);
  }
  EXPECT_THAT(explanation,
              Accepts(absl::StrCat("which was not sent because it ",
                                   kUnsupportedMessage)));
  ASSERT_EQ(part_results.size(), 1);
  EXPECT_TRUE(part_results.GetTestPartResult(0).skipped());
  EXPECT_THAT(part_results.GetTestPartResult(0).message(),
              HasSubstr(kUnsupportedMessage));
  EXPECT_FALSE(inner_called);
  EXPECT_EQ(GetCounts(test_manager_),
            (Counts{/*expected_successes=*/0, /*expected_failures=*/0,
                    /*unexpected_successes=*/0, /*unexpected_failures=*/0,
                    /*skipped=*/0, /*listed_skips=*/0,
                    /*tolerated_recommended_failures=*/0,
                    /*unsupported_protocol_tests=*/1}));
  // The result file lists it as skipped, with the reason.
  ASSERT_THAT(test_manager_.case_results(), SizeIs(1));
  EXPECT_EQ(test_manager_.case_results()[0].status(),
            ::conformance::ConformanceTestCaseResult::SKIPPED);
  EXPECT_EQ(test_manager_.case_results()[0].error_message(),
            kUnsupportedMessage);
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

// A recommended test the runner didn't send isn't a failure of any kind, so
// it is not counted as a tolerated recommended failure either, enforced or
// not; only the unsupported counter moves.
TEST_F(YieldsTest, RecommendedUnsupportedProtocolIsNotAToleratedFailure) {
  test_manager_.set_enforce_recommended(false);
  absl::ScopedMockLog log;
  EXPECT_CALL(log, Log(absl::LogSeverity::kWarning, _, _)).Times(0);
  log.StartCapturingLogs();

  auto matcher = Yields(IsParseError());
  TestResult result = RunUnsupported(TestPriority::kP3);
  ASSERT_EQ(result.name(),
            "Recommended.YieldsTest.foo.EmptyInput.ProtobufOutput");
...
// A listed test the runner couldn't send is neither a listed skip nor an
// unexpected success: the entry counts as seen and stays as it is.
TEST_F(YieldsTest, ListedUnsupportedProtocolKeepsTheEntry) {
  ASSERT_THAT(test_manager_.LoadFailureList(WriteTempFile(
                  "failure_list.txt",
                  absl::StrCat(kUnsupportedFoo, " # some message\n"))),
              IsOk());
  auto matcher = Yields(IsParseError());
  TestResult result = RunUnsupported();
  ...
  EXPECT_EQ(GetCounts(test_manager_),
            (Counts{/*expected_successes=*/0, /*expected_failures=*/0,
                    ...
                    /*tolerated_recommended_failures=*/0,
                    /*unsupported_protocol_tests=*/1}));
  EXPECT_THAT(test_manager_.ListedSkips(), IsEmpty());
  EXPECT_THAT(test_manager_.UnexpectedSuccesses(), IsEmpty());
  EXPECT_THAT(test_manager_.UnseenExpectedFailures(), IsEmpty());
  EXPECT_THAT(test_manager_.UnmatchedExpectedFailures(), IsEmpty());
  EXPECT_THAT(test_manager_.Finalize(), IsOk());

  // --fix keeps the entry verbatim: nothing is known about the test.
...
// Matched directly (outside Yields()), such a result is simply not an
// output.
TEST_F(YieldsTest, UnsupportedProtocolIsNotAnOutput) {
  TestResult result = RunUnsupported();
  result.MarkChecked();
  EXPECT_THAT(Explain(ParsedPayload(_), result),
              Rejects(absl::StrCat("the test was not sent to the testee: it ",
                                   kUnsupportedMessage)));
  EXPECT_THAT(Explain(IsParseError(), result), Rejects(_));
}
...
// ---------------------------------------------------------------------------
// Result export (TestManager::case_results())
// ---------------------------------------------------------------------------

using ::conformance::ConformanceTestCaseResult;

TEST_F(YieldsTest, RecordsAPassWithTheTestsIdentity) {
  EXPECT_THAT(Run(kPayload2),
              Yields(ParsedPayload(EqualsTextProto("optional_int32: 2"))));

  ASSERT_THAT(test_manager_.case_results(), SizeIs(1));
  const ConformanceTestCaseResult& recorded = test_manager_.case_results()[0];
  EXPECT_EQ(recorded.status(), ConformanceTestCaseResult::PASS_CANONICAL);
  EXPECT_EQ(recorded.legacy_test_name(), kRequiredFoo);
  // YieldsTest isn't a conformance suite, so it is unmapped.
  EXPECT_EQ(recorded.test_name(), "unmapped.yields.yields_foo.pb2pb");
  EXPECT_EQ(recorded.coordinate(), "00.00.001.01");
  EXPECT_EQ(recorded.domain(), "unmapped");
  EXPECT_EQ(recorded.section(), "yields");
  EXPECT_EQ(recorded.variant(), "pb2pb");
  EXPECT_EQ(recorded.priority(), "P0");
  EXPECT_TRUE(recorded.error_message().empty());
  EXPECT_TRUE(recorded.matched_behavior_id().empty());
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

TEST_F(YieldsTest, RecordsAFailureWithItsMessage) {
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kPayload2),
                  Yields(ParsedPayload(EqualsTextProto("optional_int32: 1")))),
      "modified: optional_int32: 1 -> 2");
  ...
  ASSERT_THAT(test_manager_.case_results(), SizeIs(1));
  const ConformanceTestCaseResult& recorded = test_manager_.case_results()[0];
  EXPECT_EQ(recorded.status(), ConformanceTestCaseResult::FAILURE);
  EXPECT_THAT(recorded.error_message(),
              HasSubstr("modified: optional_int32: 1 -> 2"));
}

TEST_F(YieldsTest, ExpectedAndRecommendedFailuresAreStillRecordedAsFailures) {
  ASSERT_THAT(test_manager_.AddExpectedFailure(kRequiredFoo, ""), IsOk());
  EXPECT_THAT(Run(kPayload2), Yields(IsParseError()));
  EXPECT_THAT(Run(kPayload2, TestPriority::kP3, "other"),
              Yields(IsParseError()));

  ASSERT_THAT(test_manager_.case_results(), SizeIs(2));
  EXPECT_EQ(test_manager_.case_results()[0].status(),
            ConformanceTestCaseResult::FAILURE);
  EXPECT_EQ(test_manager_.case_results()[0].error_message(), kNotAParseError);
  EXPECT_EQ(test_manager_.case_results()[1].status(),
            ConformanceTestCaseResult::FAILURE);
  EXPECT_EQ(test_manager_.case_results()[1].priority(), "P3");
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

TEST_F(YieldsTest, RecordsRuntimeAndTimeoutErrorsAsCrashes) {
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kRuntimeError), Yields(Not(IsParseError()))),
      "Failed to parse input or produce output.");
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(Run(kTimeoutError, TestPriority::kP0, "slow"),
                  Yields(Not(IsParseError()))),
      "Failed to parse input or produce output.");
  EXPECT_NONFATAL_FAILURE(EXPECT_THAT(Run("", TestPriority::kP0, "empty"),
                                      Yields(Not(IsParseError()))),
                          "");

  ASSERT_THAT(test_manager_.case_results(), SizeIs(3));
  EXPECT_EQ(test_manager_.case_results()[0].status(),
            ConformanceTestCaseResult::CRASH);
  EXPECT_EQ(test_manager_.case_results()[0].error_message(),
            "Failed to parse input or produce output.");
  EXPECT_EQ(test_manager_.case_results()[1].status(),
            ConformanceTestCaseResult::CRASH);
  // An empty response is a plain failure, not a crash.
  EXPECT_EQ(test_manager_.case_results()[2].status(),
            ConformanceTestCaseResult::FAILURE);
}

TEST_F(YieldsTest, RecordsSkipsWithTheirReason) {
  EXPECT_THAT(Run(kSkipped), Yields(IsParseError()));

  ASSERT_THAT(test_manager_.case_results(), SizeIs(1));
  EXPECT_EQ(test_manager_.case_results()[0].status(),
            ConformanceTestCaseResult::SKIPPED);
  EXPECT_EQ(test_manager_.case_results()[0].error_message(), "not supported");
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

TEST_F(YieldsTest, DoesNotRecordATestTheRunnerFilteredOut) {
  const std::string not_selected =
      absl::StrCat("skipped: \"", kTestNotSelectedSkipReason, "\"");
  EXPECT_THAT(Run(not_selected), Yields(IsParseError()));

  // The test never ran, so the result file mustn't list it.
  EXPECT_THAT(test_manager_.case_results(), IsEmpty());
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

TEST_F(YieldsTest, InformationalPassIsAnAlternate) {
  ConformanceTestInfo info;
  info.test_name = "informational.unknown_field_order.some_test.pb2pb";
  info.legacy_test_name = kRequiredFoo;
  info.coordinate = "999.01.001.01";
  info.is_informational = true;
  ConformanceResponse response;
  response.set_protobuf_payload("\010\002");
  TestResult result = TestResult::ForTesting(
      info, TestAllTypesProto2::descriptor(),
      BinaryInputRequest(::conformance::PROTOBUF), std::move(response));
  ...
  ASSERT_THAT(test_manager_.case_results(), SizeIs(1));
  EXPECT_EQ(test_manager_.case_results()[0].status(),
            ConformanceTestCaseResult::PASS_ALTERNATE);
  EXPECT_EQ(test_manager_.case_results()[0].test_name(),
            "informational.unknown_field_order.some_test.pb2pb");
  EXPECT_THAT(test_manager_.Finalize(), IsOk());
}

TEST_F(YieldsTest, EachResultIsRecordedOnceDespiteGtestRetrying) {
  EXPECT_NONFATAL_FAILURE(EXPECT_THAT(Run(kPayload2), Yields(IsParseError())),
                          "Should have failed to parse, but didn't.");
  EXPECT_THAT(test_manager_.case_results(), SizeIs(1));
}
...
TEST_F(YieldsTest, JsonOutput) {
  ...
  auto run_json = [&](absl::string_view response, absl::string_view name) {
    runner_.RespondWith(response);
    return testee_.CreateTest(Named(name), TestPriority::kP0)
        .ParseJson(TestAllTypesProto2::descriptor(), "{}")
        .SerializeJson();
  };
  constexpr absl::string_view kJsonResponse =
      R"pb(json_payload: "{\"optionalInt32\": 9}")pb";
  // Kept out of the EXPECT_NONFATAL_FAILURE() arguments below: MSVC's
  // traditional preprocessor mis-scans raw string literals containing \" or
  // an odd number of quotes when they appear inside macro arguments.
  constexpr absl::string_view kUnparseableJsonResponse =
      R"pb(json_payload: "{\"nonsense\": 1}")pb";
  constexpr absl::string_view kInvalidJsonResponse = R"pb(json_payload: "{")pb";
  ...
  EXPECT_NONFATAL_FAILURE(
      EXPECT_THAT(run_json(kUnparseableJsonResponse, "unparseable"),
                  Yields(ParsedPayload(_))),
      "JSON output we received from test was unparseable.");
  EXPECT_NONFATAL_FAILURE(EXPECT_THAT(run_json(kInvalidJsonResponse, "invalid"),
