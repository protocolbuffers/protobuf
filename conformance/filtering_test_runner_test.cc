...
// The discovery handshake's probe isn't subject to the filter: it reaches the
// delegate whatever was selected (even nothing), so that a --test run detects
// the same protocol version as a full one; and it isn't one of the names
// --test asked for, so NamesRun() doesn't report it.
TEST(FilteringTestRunnerTest, ProbeIsAlwaysForwarded) {
  {
    FakeTestRunner fake;
    FilteringTestRunner runner(&fake, {"Test.One"});

    EXPECT_EQ(runner.RunTest(kProbeName, "probe"),
              absl::StrCat("response:", kProbeName, ":probe"));

    EXPECT_THAT(fake.calls(), ElementsAre(Pair(kProbeName, "probe")));
    EXPECT_THAT(runner.NamesRun(), IsEmpty());
  }
  {
    // An empty selection still forwards it.
    FakeTestRunner fake;
    FilteringTestRunner runner(&fake, {});
