...
TEST_P(TextFloatLiteralTest, Zero) {
  EXPECT_THAT(Testee().ParseText(message(), Input("0")).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(""))));
  EXPECT_THAT(Testee().ParseText(message(), Input("0")).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(""))));
}
...
TEST_P(TextFloatTest, NoHex) {
  EXPECT_THAT(Testee().ParseText(message(), "optional_float: 0x1").ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextFloatTest, NoOctal) {
  EXPECT_THAT(Testee().ParseText(message(), "optional_float: 012").ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(TextFloatTest, OverflowInfinity) {
  EXPECT_THAT(
      Testee().ParseText(message(), "optional_float: 1e50").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto("optional_float: inf"))));
  EXPECT_THAT(
      Testee().ParseText(message(), "optional_float: 1e50").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("optional_float: inf"))));
}
...
TEST_P(TextFloatTest, LargeNegativeExponentParsesAsZero) {
  EXPECT_THAT(
      Testee().ParseText(message(), "optional_float: 1e-50").SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(""))));
  EXPECT_THAT(
      Testee().ParseText(message(), "optional_float: 1e-50").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto(""))));
}
...
TEST_P(TextDoubleTest, OverflowInfinity) {
  ...
  EXPECT_THAT(
      Testee().ParseText(message(), "optional_double: 1e9999").SerializeText(),
      Yields(ParsedPayload(EqualsTextProto("optional_double: inf"))));
}
