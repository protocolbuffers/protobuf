...
TEST_P(OneofZeroTest, Uint32) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(111, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_uint32: 0)pb"))));
}

TEST_P(OneofZeroTest, Uint32Json) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(111, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_uint32: 0)pb"))));
}
...
TEST_P(OneofZeroTest, Bool) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(115, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_bool: false)pb"))));
}

TEST_P(OneofZeroTest, BoolJson) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(115, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_bool: false)pb"))));
}
...
TEST_P(OneofZeroTest, Uint64) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(116, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_uint64: 0)pb"))));
}

TEST_P(OneofZeroTest, Uint64Json) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(116, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_uint64: 0)pb"))));
}
...
TEST_P(OneofZeroTest, Float) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), FloatField(117, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_float: 0)pb"))));
}

TEST_P(OneofZeroTest, FloatJson) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), FloatField(117, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_float: 0)pb"))));
}
...
TEST_P(OneofZeroTest, Double) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), DoubleField(118, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_double: 0)pb"))));
}

TEST_P(OneofZeroTest, DoubleJson) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), DoubleField(118, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_double: 0)pb"))));
}
...
TEST_P(OneofZeroTest, Enum) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(119, 0)).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_enum: FOO)pb"))));
}

TEST_P(OneofZeroTest, EnumJson) {
  EXPECT_THAT(
      Testee().ParseBinary(message(), VarintField(119, 0)).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(R"pb(oneof_enum: FOO)pb"))));
}
