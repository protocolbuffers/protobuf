...
internal::TestName CurrentTestName(absl::string_view suffix) {
  ...
  ABSL_CHECK(test_info != nullptr)
      << "Testee() can only be called from inside a test body.";
...
Statistics Statistics::From(const TestManager& manager) {
  ...
  statistics.unsupported_protocol_tests = manager.unsupported_protocol_tests();
  return statistics;
}
...
Statistics Statistics::operator-(const Statistics& other) const {
  ...
  delta.unsupported_protocol_tests =
...
void Statistics::RecordProperties() const {
  ...
  testing::Test::RecordProperty("unsupported_protocol_tests",
                                unsupported_protocol_tests);
}
...
void ConformanceEnvironment::SetUp() {
  ...
  // The discovery handshake; see the option's comment.  It is the first thing
  // the testee hears from us.  An out-of-range pin is a configuration error,
  // reported here rather than check-failed in the Testee.
  ASSERT_TRUE(options_.protocol_version == 0 ||
              (options_.protocol_version >= 1 &&
               options_.protocol_version <= internal::kLatestProtocolVersion))
      << "--protocol_version=" << options_.protocol_version
      << " is out of range: this runner speaks conformance protocol versions "
         "1 to "
      << internal::kLatestProtocolVersion << " (0 detects the testee's)";
  absl::Status status =
      testee().DetectProtocolVersion(options_.protocol_version);
  ASSERT_TRUE(status.ok()) << status.message();

  set_up_succeeded_ = true;
}

void ConformanceEnvironment::WriteRunResult() {
  // Nothing ran if SetUp() didn't; don't clobber an earlier run's results.
  if (options_.result_file.empty() || !set_up_succeeded_) return;

  std::string output_file = options_.result_file;
  const char* workspace = std::getenv("BUILD_WORKSPACE_DIRECTORY");
  if (!IsAbsolutePath(output_file) && workspace != nullptr) {
    // Like a relative --fix_output_file, a relative --output_result_file is
    // resolved against the workspace under `bazel run`, where the working
    // directory is the runfiles tree.
    output_file = absl::StrCat(workspace, "/", output_file);
  }
  // Under test sharding every shard is its own process and would overwrite
  // the others' file, so each shard writes "<file>.shard<index>[.<ext>]".
  if (const char* total_shards = std::getenv("TEST_TOTAL_SHARDS");
      total_shards != nullptr) {
    int shard_count = 0;
    int shard_index = 0;
    const char* index = std::getenv("TEST_SHARD_INDEX");
    if (absl::SimpleAtoi(total_shards, &shard_count) && shard_count > 1) {
      if (index == nullptr || !absl::SimpleAtoi(index, &shard_index)) {
        ABSL_LOG(WARNING) << "Not writing " << output_file
                          << ": TEST_TOTAL_SHARDS is set but TEST_SHARD_INDEX "
                             "isn't, so the shards would overwrite each other";
        return;
      }
      const std::string suffix = absl::StrCat(".shard", shard_index);
      const size_t basename_start =
          output_file.size() - internal::Basename(output_file).size();
      const size_t dot = output_file.rfind('.');
      if (dot == std::string::npos || dot < basename_start) {
        absl::StrAppend(&output_file, suffix);
      } else {
        output_file.insert(dot, suffix);
      }
    }
  }

  std::string implementation_name = options_.implementation_name;
  if (implementation_name.empty()) {
    implementation_name = internal::Basename(options_.testee_binary);
  }
  if (implementation_name.empty()) implementation_name = "unknown";

  ::conformance::ConformanceRunResult run_result =
      test_manager_.BuildRunResult(implementation_name);
  run_result.set_timestamp_epoch_sec(absl::ToUnixSeconds(absl::Now()));

  std::string text;
  if (!TextFormat::PrintToString(run_result, &text)) {
    ADD_FAILURE() << "Failed to serialize the conformance run results";
    return;
  }
  std::ofstream out(output_file);
  if (!out.is_open()) {
    ADD_FAILURE() << "Failed to open the result file " << output_file
                  << " for writing (does the directory exist?)";
    return;
  }
  out << text;
  out.close();
  if (out.fail()) {
    ADD_FAILURE() << "Failed to write the result file " << output_file;
    return;
  }
  ABSL_LOG(INFO) << "Wrote " << run_result.total_tests()
                 << " conformance results to " << output_file;
}
...
internal::Test Testee(absl::string_view suffix) {
  return Testee(suite_default_priority.value_or(kP0), suffix);
}

internal::Test Testee(TestPriority priority, absl::string_view suffix) {
  return ConformanceEnvironment::Get().testee().CreateTest(
      CurrentTestName(suffix), priority);
}
