...
void ConformanceEnvironment::TearDown() {
  ...
  // Release the testee: for an owned runner this destroys it (and, with
  ...
  // RUN_ALL_TESTS() -- the merged runner relies on this.  Either way the leaked
  // environment (see Install()) neither keeps a testee alive until the process
  // exits nor holds a dangling pointer to one.
  testee_.reset();
  owned_runner_.reset();
}
