...
testee, one `cc_test` per suite, with that testee's failure lists; see
`conformance_test()` and `failure_lists()`.  The testee is either a binary
that the test forks and talks to over a pipe, or (`in_process = True`) a
`cc_library` linked into the test binary itself.
...
This creates `:cpp_binary_test`, `:cpp_json_test`, `:cpp_text_test` and
`:cpp_performance_test` (one `:cpp_<suite>_test` per suite), plus a `:cpp`
test_suite running all of them.  The expected failures for `:cpp_binary_test`
live in `failure_lists/cpp_binary.txt`; when that file doesn't exist, the
testee is expected to pass everything.  Either way
...
wraps the conformance_test_runner command line (`--failure_list` /
`--text_format_failure_list`) and which the open-source build's language
packages still use.
TODO: b/563707827 - retire defs.bzl once they have moved to this macro.
...
# after them (see failure_lists()).  Every suite but `performance` covers one
# wire format; `performance` holds the tests that stress the testee (deeply
# nested and very large payloads) for every format, which are kept apart
# because they are slow and their expected failures are unrelated to the
# format's.  Every gtest fixture in the performance suite is named
# `*PerformanceTest` and no fixture outside it is: that is how
# conformance_test_runner's --performance tells them apart (see
# test_environment.h).
_SUITES = ["binary", "text", "json", "performance"]

# The newest conformance protocol version the test binaries speak; the upper
# bound of conformance_test(protocol_version = ...).  Keep in sync with
# kLatestProtocolVersion in testee.h.
_LATEST_PROTOCOL_VERSION = 2
...
def conformance_suite(name, srcs, deps = [], **kwargs):
    ...
    Args:
      ...
      srcs: The test sources.  Fixtures in the `performance` suite must be
        named `*PerformanceTest`; fixtures in any other suite must not be.
...
def conformance_test(
    ...
    `cpp_binary_test`, `cpp_performance_test`), and a `test_suite` named
    `name` running all of them.
    ...
    the testee in their own process: no fork, one process to debug, and a
    testee-side crash or log shows up in the test itself.  The flip side is
    that there is no per-test isolation: a testee crash or hang takes the
    whole test binary down instead of failing one test (the forked form
    respawns the testee), so only use it for testees that never crash on
    conformance input.  Both modes take the same failure lists and flags.

    Args:
      name: The test's name; also the prefix of its failure lists.
      testee: The testee.  Without `in_process`, a binary run as a subprocess.
        With `in_process`, a `cc_library` defining `MakeTesteeRunner()`, like
        this package's own `:cpp_in_process_testee` (private to it).  Such a
        library must be `alwayslink = 1` and `testonly = 1`, and exactly one
        of them may be linked into a test: none fails to link, two is an ODR
        violation (a duplicate-symbol error under static linking, silently
        ambiguous under dynamic linking).  A macro can't check any of this,
        nor the kind of a label, so passing a binary here with `in_process`
        fails at analysis time (a cc_test can't depend on a binary), and a
        library without it fails at analysis time on `$(rootpath)` or, if
        that resolves to its single archive, when the test tries to run it.
      failure_lists: The failure_lists() of this package.
      suites: The suites to run; default all of them, the performance suite
        included.  Use this to skip a suite the testee doesn't support at all
        (e.g. `text` for a runtime without text format).
      ...
      enforce_recommended: Whether failures in recommended tests fail the run
        (the default, as conformance_test_wrapper.sh always passed it).  Passed
        as `--enforce_recommended`.
      protocol_version: The conformance protocol version to speak to the
        testee (see ConformanceRequest.protocol_version in conformance.proto).
        By default (`None`) the tests detect it: they open with a discovery
        handshake the testee answers with the highest version it implements,
        and a testee that doesn't take part gets version 1.  An int pins the
        version: the handshake still runs, and the tests fail if the testee
        answers with an older version, so pin a testee once it implements a
        version to make sure it keeps doing so.  Tests that need a newer
        version than the one spoken are skipped.  Passed as
        `--protocol_version`; must be between 1 and the newest version this
        runner speaks.
      in_process: Whether `testee` is a cc_library to link into each test
        instead of a binary to spawn.
      result_file: Where each test writes its results as a ConformanceRunResult
        textproto (conformance_result.proto), the input of report_generator.py;
        default none.  `{suite}` in it is replaced with the suite's name, so
        the suites don't overwrite each other's file.  A relative path is
        resolved under $BUILD_WORKSPACE_DIRECTORY, i.e. into the workspace
        under `blaze run` (under `blaze test` it lands in the sandbox).  Passed
        as `--output_result_file`.
      implementation_name: The implementation_name recorded in `result_file`;
        default the testee binary's name (or the test's, with `in_process`).
        Passed as `--implementation_name`.
    ...
    if protocol_version != None:
        if type(protocol_version) != "int" or protocol_version < 1 or protocol_version > _LATEST_PROTOCOL_VERSION:
            fail(("conformance_test(%r): protocol_version must be None (detect) or an int between " +
                  "1 and %d, not %r.") % (name, _LATEST_PROTOCOL_VERSION, protocol_version))
    ...
    for suite in suites:
        ...
        if protocol_version != None:
            args.append("--protocol_version=%d" % protocol_version)
        if result_file:
            args.append("--output_result_file=%s" % result_file.replace("{suite}", suite))
        if implementation_name:
            args.append("--implementation_name=%s" % implementation_name)
