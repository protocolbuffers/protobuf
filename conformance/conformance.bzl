...
This creates `:cpp_binary_test` and `:cpp_performance_test` (and one
`:cpp_<suite>_test` per further suite), plus a `:cpp` test_suite running all
of them.  The expected failures for `:cpp_binary_test` live in
`failure_lists/cpp_binary.txt`; when that file doesn't exist, the testee is
expected to pass everything.  Either way
...
_SUITES = ["binary", "performance"]

# The tags every `<name>_performance_test` gets on top of the caller's: the
# performance suite's payloads (20000-deep messages among them) overflow the
# stack under the sanitizers' larger frames, in the test as much as in an
# in-process testee, so those tests never run under a sanitizer.
# TODO: b/563731788 - drop these once the deep requests are built and
# serialized without recursion.
_PERFORMANCE_TEST_TAGS = ["noasan", "nodiorite", "nomsan", "notsan", "noubsan"]
...
def conformance_test(
    ...
    Args:
      ...
      **kwargs: Passed through to every cc_test (e.g. `size`, `timeout`,
        `tags`); `tags` also apply to the test_suite, and the performance
        test additionally gets the tags that keep it off the sanitizers (see
        `_PERFORMANCE_TEST_TAGS`).  `args`, `deps` and `data` are set by the
        macro and can't be given.
    """
    if suites == None:
        suites = _SUITES
    for suite in suites:
    ...
    for suite in suites:
        ...
        test_tags = ["conformance"] + tags
        if suite == "performance":
            test_tags += [t for t in _PERFORMANCE_TEST_TAGS if t not in tags]
        cc_test(
            name = test_name,
            args = args,
            data = data,
