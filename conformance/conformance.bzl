...
# The gtest main that turns a suite library into a conformance test binary.
...
def conformance_test(
        ...
        in_process = False,
        **kwargs):
    """Runs the conformance suites against a testee binary.
    ...
    it against `testee` with the failure list `<failure_lists
    directory>/<name>_<suite>.txt` if that file exists; otherwise the testee is
    expected to pass every test in the suite, and `blaze run ... -- --fix`
    creates the file (see failure_lists()).

    Args:
      name: The test's name; also the prefix of its failure lists.
      testee: The testee binary, run as a subprocess speaking the conformance
      ...
      performance: Whether to run the performance tests instead of the regular
        ...
        `--performance`.
      in_process: Reserved: link the testee, a cc_library, into the test binary
        instead of spawning it.  Not supported yet.
    ...
    """
    if in_process:
        # TODO: b/410122435 - link the suite library against an in-process testee.
        fail("in_process testees are added in a follow-up CL")
    if suites == None:
        suites = _SUITES
    for suite in suites:
        _check_suite(suite)
    ...
    for suite in suites:
        ...
        cc_test(
            ...
            deps = [
                _suite_library(suite),
                _TEST_ENVIRONMENT_MAIN,
            ],
            **kwargs
        )
