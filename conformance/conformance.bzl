...
wraps the conformance_test_runner command line (legacy `--failure_list` /
`--text_format_failure_list` flags) and stays until the language runtimes
have migrated to this macro.
...
def conformance_test(
    ...
    Args:
      ...
      enforce_recommended: Whether failures in recommended tests fail the run
        (the default, as the sh_test wrappers of conformance_test_runner always
        pass it).  Passed as `--enforce_recommended`.
