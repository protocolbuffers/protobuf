...
This creates `:cpp_binary_test` and `:cpp_text_test` (and one
`:cpp_<suite>_test` per further suite), plus a `:cpp` test_suite running all of
them.  The expected failures for `:cpp_binary_test` live in
