...
TEST_P(JsonStructTest, Struct) {
  ...
  EXPECT_THAT(Testee("Struct").ParseJson(message(), kInput).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("Struct").ParseJson(message(), kInput).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonStructTest, StructWithEmptyListValue) {
  ...
  EXPECT_THAT(Testee("StructWithEmptyListValue")
  ...
  const std::string expected =
      absl::StrCat("optional_struct: {\n",
                   ...
                   Repeat("      }\n"
                          ...
                          25),
                   "}\n");
  EXPECT_THAT(Testee(kP3, "StructDeepNesting25")
...
TEST_P(JsonStructTest, StructDeepNesting200) {
  ...
  EXPECT_THAT(Testee(kP3, "StructDeepNesting200")
                  .ParseJson(message(), input)
                  .ParseOnly(),
              Yields(IsParseError()));
}
