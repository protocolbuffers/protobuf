...
TEST_P(IllegalTagsTest, IllegalZeroFieldNumCase0) {
  EXPECT_THAT(Testee().ParseBinary(message(), Wire("\1DEADBEEF")).ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(IllegalTagsTest, IllegalZeroFieldNumCase1) {
  EXPECT_THAT(Testee().ParseBinary(message(), Wire("\2\1\1")).ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(IllegalTagsTest, IllegalZeroFieldNumCase2) {
  EXPECT_THAT(Testee().ParseBinary(message(), Wire("\3\4")).ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(IllegalTagsTest, IllegalZeroFieldNumCase3) {
  EXPECT_THAT(Testee().ParseBinary(message(), Wire("\5DEAD")).ParseOnly(),
              Yields(IsParseError()));
}
