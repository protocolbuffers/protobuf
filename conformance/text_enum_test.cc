...
TEST_P(TextOpenEnumTest, ByNumber) {
  EXPECT_THAT(
      Testee().ParseText(message(), kKnownNumberInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kKnownNumberInput))));
  EXPECT_THAT(Testee().ParseText(message(), kKnownNumberInput).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(kKnownNumberInput))));
}

TEST_P(TextOpenEnumTest, WithUnknownNumber) {
  EXPECT_THAT(
      Testee().ParseText(message(), kUnknownNumberInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kUnknownNumberInput))));
  EXPECT_THAT(
      Testee().ParseText(message(), kUnknownNumberInput).SerializeText(),
      Yields(ParsedPayload(EqualsTextProto(kUnknownNumberInput))));
}
...
TEST_P(TextClosedEnumTest, WithUnknownNumber) {
  EXPECT_THAT(Testee().ParseText(message(), kUnknownNumberInput).ParseOnly(),
              Yields(IsParseError()));
}
