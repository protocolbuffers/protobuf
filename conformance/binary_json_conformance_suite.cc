...
void BinaryAndJsonConformanceSuiteImpl<MessageType>::RunAllTests() {
  ...
  if (!suite_.performance_) {
    // The ValidDataScalar, RepeatedScalarSelectsLast and ValidDataRepeated
    // tests live in binary_valid_data_scalar_test.cc and
    // binary_valid_data_repeated_test.cc (see BUILD).
    // The map tests (ValidDataMap.*) and the merge tests
    // (RepeatedScalarMessageMerge, ValidDataMap.STRING.MESSAGE.MergeValue,
    // ValidDataOneof.MESSAGE.Merge) live in the binary_conformance_tests
    // library (see BUILD), both legs.
