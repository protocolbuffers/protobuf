...
void BinaryAndJsonConformanceSuite::RunSuiteImpl() {
  ...
  // The message set tests live in the binary_conformance_tests library (see
  // BUILD).
...
void BinaryAndJsonConformanceSuiteImpl<MessageType>::RunAllTests() {
  if (!suite_.performance_) {
    // Groups already migrated to gtest live in the binary_conformance_tests
    // library (see BUILD) and are no longer run from here.
    ...
    // Unknown fields.  UnknownVarint lives in the binary_conformance_tests
    // library (see BUILD).
    TestUnknownOrdering();
    TestOneofMessage();
