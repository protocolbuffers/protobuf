...
constexpr SuiteSection kSuiteSections[] = {
    ...
    {"MergeFromTest", ConformanceSection::kWire_LengthDelimited_Submessages},
    // A freshly constructed message serializes to nothing (needs protocol v2's
    // "new message" action).
    // TODO: b/564551336 - confirm section
    {"NewMessageTest", ConformanceSection::kWire_LengthDelimited_Submessages},
...
std::string PayloadTag(::conformance::WireFormat input_format,
                       ::conformance::WireFormat output_format) {
  const absl::string_view in = internal::FormatTag(input_format);
  const absl::string_view out = internal::FormatTag(output_format);
...
int PayloadDigit(::conformance::WireFormat input_format,
                 ::conformance::WireFormat output_format) {
  const int in = FormatIndex(input_format);
  const int out = FormatIndex(output_format);
