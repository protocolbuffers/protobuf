...
// The payload transform of a test that parses its input from `input_format`
// and asks for its output in `output_format`, "<input>2<output>" (e.g.
// "pb2json"), or "" if either format is unknown.  The formats are the ones
// the test asked for, not those of a particular protocol version's request.
...
//
// 0 if either format is unknown; see PayloadTag() for the arguments.
