...
TEST_P(JsonStringTest, StringField) {
  ...
  EXPECT_THAT(
      Testee("StringField").ParseJson(message(), kInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      Testee("StringField").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonStringTest, StringFieldUnicode) {
  ...
  EXPECT_THAT(Testee("StringFieldUnicode")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      Testee("StringFieldUnicode").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonStringTest, StringFieldEscape) {
  ...
  EXPECT_THAT(Testee("StringFieldEscape")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(
      Testee("StringFieldEscape").ParseJson(message(), kInput).SerializeJson(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonStringTest, StringFieldUnicodeEscape) {
  ...
  constexpr absl::string_view kExpected =
      R"(optional_string: "\xE8\xB0\xB7\xE6\xAD\x8C")";
  EXPECT_THAT(Testee("StringFieldUnicodeEscape")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("StringFieldUnicodeEscape")
...
TEST_P(JsonStringTest, StringFieldSurrogatePair) {
  ...
  EXPECT_THAT(Testee("StringFieldSurrogatePair")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("StringFieldSurrogatePair")
...
TEST_P(JsonStringTest, StringFieldEmbeddedNull) {
  ...
  EXPECT_THAT(Testee("StringFieldEmbeddedNull")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("StringFieldEmbeddedNull")
...
TEST_P(JsonStringTest, StringFieldUppercaseEscapeLetter) {
  EXPECT_THAT(Testee(kP3, "StringFieldUppercaseEscapeLetter")
...
TEST_P(JsonStringTest, StringFieldInvalidEscape) {
  EXPECT_THAT(Testee(kP3, "StringFieldInvalidEscape")
...
TEST_P(JsonStringTest, StringFieldUnterminatedEscape) {
  EXPECT_THAT(Testee(kP3, "StringFieldUnterminatedEscape")
...
TEST_P(JsonStringTest, StringFieldUnpairedHighSurrogate) {
  EXPECT_THAT(Testee(kP3, "StringFieldUnpairedHighSurrogate")
...
TEST_P(JsonStringTest, StringFieldUnpairedLowSurrogate) {
  EXPECT_THAT(Testee(kP3, "StringFieldUnpairedLowSurrogate")
                  .ParseJson(message(), "{\"optionalString\": \"\\uDC00\"}")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonStringTest, StringFieldSurrogateInWrongOrder) {
  EXPECT_THAT(
      Testee(kP3, "StringFieldSurrogateInWrongOrder")
          .ParseJson(message(), "{\"optionalString\": \"\\uDE01\\uD83D\"}")
          .ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(JsonStringTest, StringFieldNotAString) {
  EXPECT_THAT(Testee("StringFieldNotAString")
...
TEST_P(JsonStringTest, StringFieldSingleQuoteKey) {
  EXPECT_THAT(Testee(kP3, "StringFieldSingleQuoteKey")
...
TEST_P(JsonStringTest, StringFieldSingleQuoteValue) {
  EXPECT_THAT(Testee(kP3, "StringFieldSingleQuoteValue")
                  .ParseJson(message(), R"({"optionalString": 'Hello world!'})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonStringTest, StringFieldSingleQuoteBoth) {
  EXPECT_THAT(Testee(kP3, "StringFieldSingleQuoteBoth")
                  .ParseJson(message(), R"({'optionalString': 'Hello world!'})")
                  .ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonBytesTest, BytesField) {
  ...
  EXPECT_THAT(
      Testee("BytesField").ParseJson(message(), kInput).SerializeBinary(),
      Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee("BytesField").ParseJson(message(), kInput).SerializeJson(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
}
...
TEST_P(JsonBytesTest, BytesFieldBase64Url) {
  ...
  EXPECT_THAT(Testee(kP3, "BytesFieldBase64Url")
                  .ParseJson(message(), kInput)
                  .SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kExpected))));
  EXPECT_THAT(Testee(kP3, "BytesFieldBase64Url")
