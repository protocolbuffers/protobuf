...
class TestResult {
 public:
  ...
  // Every serialized output, in the order they were requested; empty unless
  ...
  // no input at all (only possible for results built with ForTesting()).
...
class InMemoryMessage {
 public:
  ...
  // would, after parsing succeeded and before serializing, by setting the
  // request's `merge_payload`.  The payload's format is independent of the
  // parsed input's.  A JSON merge payload follows the request's single
  // test_category for `ignore_unknown_fields`: it is lenient only after
  // ParseJson(..., JsonParseOptions{/*ignore_unknown_fields=*/true}) and
  // always strict after a binary or text input.  Whether an explicit default
  // value in the merge payload overwrites a field without presence is
  ...
  //
  // A test merges at most once (a second Merge*() is a DCHECK failure), and
  // merging happens before discarding unknown fields, so Merge*() must precede
  // DiscardUnknownFields() in the chain.  Testees that don't support merging
  // answer with `skipped`.
  //
  ...
  // C++'s Message::DiscardUnknownFields() does) after parsing and merging, and
  // before serializing, by setting `discard_unknown_fields` on the request.
  // This makes it possible to tell a value the testee parsed into a field from
  // one it kept as an unknown field, which a plain round trip re-emits either
  ...
  // terminal call of a test: Merge*() and DiscardUnknownFields() (and any
  // future operation, see the TODO at the top of this file) come before it,
  // not after.
