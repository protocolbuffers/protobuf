...
class InMemoryMessage {
 public:
  ...
  // C++'s Message::DiscardUnknownFields() does) after parsing and before
  // serializing, by setting `discard_unknown_fields` on the request.  This
  // makes it possible to tell a value the testee parsed into a field from one
  // it kept as an unknown field, which a plain round trip re-emits either way.
  // The test's name doesn't change; a test that sends the same input with and
  // without this uses TestName::suffix to tell the two requests apart.
  //
