...
TEST_P(TextAnyTest, AnyField) {
  ...
  EXPECT_THAT(Testee().ParseText(message(), kInput).SerializeBinary(),
              Yields(ParsedPayload(EqualsTextProto(kInput))));
  EXPECT_THAT(Testee().ParseText(message(), kInput).SerializeText(),
              Yields(ParsedPayload(EqualsTextProto(kInput))));
}
...
TEST_P(TextAnyTest, WithInvalidType) {
  ...
  EXPECT_THAT(Testee().ParseText(message(), kInput).ParseOnly(),
              Yields(IsParseError()));
}
