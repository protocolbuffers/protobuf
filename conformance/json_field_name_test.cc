...
TEST_P(JsonFieldNameTest, FieldNameNotQuoted) {
  EXPECT_THAT(Testee(kP3).ParseJson(message(), "{fieldname1: 1}").ParseOnly(),
              Yields(IsParseError()));
}
...
TEST_P(JsonObjectSyntaxTest, TrailingCommaInAnObject) {
  EXPECT_THAT(Testee().ParseJson(message(), R"({"fieldname1":1,})").ParseOnly(),
              Yields(IsParseError()));
}

TEST_P(JsonObjectSyntaxTest, TrailingCommaInAnObjectWithSpace) {
  EXPECT_THAT(
      Testee().ParseJson(message(), R"({"fieldname1":1 ,})").ParseOnly(),
      Yields(IsParseError()));
}
...
TEST_P(JsonSkipsDefaultTest, SkipsDefaultPrimitive) {
  EXPECT_THAT(
      Testee().ParseJson(message(), R"({"FieldName13": 0})").SerializeJson(),
