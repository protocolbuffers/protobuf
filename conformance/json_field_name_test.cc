...
// where such a validator shared its name with a round-trip test, it is named
// "<name>InLowerCamelCase".  RunValidJsonTestOrParseFailure() asked for
// binary output under a name without an output suffix.
...
TEST_P(JsonFieldNameSerializationTest, FieldNameWithNumbersInLowerCamelCase) {
  EXPECT_THAT(RequiredTest("FieldNameWithNumbersInLowerCamelCase")
                  .ParseJson(message(), R"({
        "field0name5": 5,
        "field0Name6": 6
      })")
...
TEST_P(JsonFieldNameSerializationTest,
       FieldNameWithMixedCasesInLowerCamelCase) {
  EXPECT_THAT(
      RequiredTest("FieldNameWithMixedCasesInLowerCamelCase")
          .ParseJson(message(), R"({
        "fieldName7": 7,
        "FieldName8": 8,
        "fieldName9": 9,
...
TEST_P(JsonFieldNameSerializationTest,
       FieldNameWithDoubleUnderscoresInLowerCamelCase) {
  EXPECT_THAT(
      RecommendedTest("FieldNameWithDoubleUnderscoresInLowerCamelCase")
          .ParseJson(message(), R"({
        "FieldName13": 13,
        "FieldName14": 14,
        "fieldName15": 15,
