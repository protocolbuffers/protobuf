...
ConformanceRequest Request(::conformance::WireFormat input,
                           ::conformance::WireFormat output) {
  ...
  request.set_requested_output_format(output);
  return request;
}
...
TEST(TaxonomyPayloadTest, TheNineTransformsOwnTheNineDigits) {
  ...
  for (int in = 0; in < 3; ++in) {
    for (int out = 0; out < 3; ++out) {
      ConformanceRequest request = Request(formats[in], formats[out]);
      EXPECT_EQ(PayloadDigit(request), expected_digit) << tags[in] << tags[out];
      EXPECT_EQ(PayloadTag(request), absl::StrCat(tags[in], "2", tags[out]));
      ++expected_digit;
    }
  }
}
...
TEST(TaxonomyPayloadTest, UnknownFormatsHaveNoDigit) {
  EXPECT_EQ(PayloadDigit(
                Request(::conformance::UNSPECIFIED, ::conformance::PROTOBUF)),
            0);
  EXPECT_EQ(
      PayloadTag(Request(::conformance::UNSPECIFIED, ::conformance::PROTOBUF)),
      "");
  EXPECT_EQ(PayloadDigit(
                Request(::conformance::PROTOBUF, ::conformance::UNSPECIFIED)),
            0);
  EXPECT_EQ(PayloadDigit(Request(::conformance::PROTOBUF, ::conformance::JSPB)),
            0);
}
