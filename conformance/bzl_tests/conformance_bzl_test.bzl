...
failure_lists, out-of-range protocol_version) happen at loading time and so
...
def conformance_bzl_test_suite(name):
    ...
    for suffix, setup in [
        ...
        ("result_options", _test_result_options),
...
def _test_options_impl(env, target):
    # A pinned protocol_version is passed last; the other tests check that
    # the default (detect) passes nothing.
...
def _test_result_options(name):
    util.helper_target(
        ...
        failure_lists = ":" + _FAILURE_LISTS,
        suites = ["binary", "json"],
        result_file = "/tmp/results/cpp_{suite}.textproto",
        implementation_name = "cpp",
    )
    analysis_test(
        name = name,
        target = _subject(name) + "_json_test",
        impl = _test_result_options_impl,
    )

def _test_result_options_impl(env, target):
    # {suite} in result_file names the suite, so every suite's test writes its
    # own file; the implementation name is passed through as is.
    test = env.expect.that_target(target)
    test.attr("args", factory = subjects.collection).contains_exactly([
        "--testee_binary=$(rootpath %s)" % _TESTEE,
        "--fix_output_file=" + _failure_list_path(target, suite = "json"),
        "--enforce_recommended=true",
        "--output_result_file=/tmp/results/cpp_json.textproto",
        "--implementation_name=cpp",
    ]).in_order()
...
def _test_in_process_impl(env, target):
    ...
    test.runfiles().not_contains_predicate(
        matching.str_matches("*/stub_testee*"),
    )
    test.tags().contains("conformance")
