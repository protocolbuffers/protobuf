      ...
      private static Dictionary<object, string> GetNameMapping([
        ...
        return enumType.GetTypeInfo()
            ...
            .ToDictionary(
                ...
                f =>
                {
                    // If the attribute hasn't been applied, fall back to the
                    // name of the field. If a custom json name has been
                    // provided, prioritize that.
                    var attr = f.GetCustomAttributes<OriginalNameAttribute>()
                        .FirstOrDefault();
