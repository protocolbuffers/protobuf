...
Due to the diversity of build systems used across the language bindings supported
by Protobuf, there is not a singular policy on how narrowly dependencies are
described. In many cases, Protobuf's dependencies' versions are allowed to float
within the range of versions that also fit within our
