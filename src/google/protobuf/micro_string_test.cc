...
class MicroStringPrevTest
 ...
 protected:
  ...
  void ExpectMemoryUsed(size_t expected_use, size_t prev_arena_used) {
    if (has_arena()) {
      EXPECT_EQ(expected_use, arena_space_used() - prev_arena_used);
    }
    if (has_arena()) {
...
TEST_P(MicroStringPrevTest, SetMicro) {
  for (size_t size : {str_.kInlineCapacity + 1, size_t{30}}) {
    ...
    size_t expected_use =
        will_reuse ? 0 : ArenaAlignDefault::Ceil(kMicroRepSize + size);
    if (has_arena()) {
...
TEST_P(MicroStringPrevTest, SetOwned) {
  for (size_t size : {str_.kMaxMicroRepCapacity + 1, size_t{300}}) {
    ...
    size_t expected_use =
        will_reuse ? 0 : ArenaAlignDefault::Ceil(kLargeRepSize + size);
    if (has_arena()) {
      EXPECT_EQ(used + expected_use, arena_space_used());
    }
    if (has_arena()) {
...
TEST_P(MicroStringPrevTest, SetAliasLarge) {
  ...
  const size_t expected_use = prev_state() == kAlias ? 0 : kLargeRepSize;
  if (has_arena()) {
    EXPECT_EQ(used + expected_use, arena_space_used());
  }
...
TEST_P(MicroStringPrevTest, SetStringMedium) {
  ...
  const size_t expected_use =
      will_reuse ? 0 : ArenaAlignDefault::Ceil(kMicroRepSize + input.size());
...
TEST_P(MicroStringPrevTest, CopyConstruct) {
  ...
  switch (prev_state()) {
    ...
    case kAlias:
      ...
      expected_use = ArenaAlignDefault::Ceil(kMicroRepSize + str_.Get().size());
      break;
    case kOwned:
      expected_use = ArenaAlignDefault::Ceil(kLargeRepSize + str_.Get().size());
      break;
  }
  if (has_arena()) {
    EXPECT_EQ(used + expected_use, arena_space_used());
...
TEST_P(MicroStringPrevTest, AssignmentViaSetMicroRep) {
  ...
  if (can_fit) {
    ...
    ExpectMemoryUsed(ArenaAlignDefault::Ceil(kMicroRepSize + str_.Get().size()),
                     used);
  }
...
TEST_P(MicroStringPrevTest, AssignmentViaSetOwned) {
  ...
  if (can_fit) {
    ...
    ExpectMemoryUsed(ArenaAlignDefault::Ceil(kLargeRepSize + str_.Get().size()),
                     used);
  }
