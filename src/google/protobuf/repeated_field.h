...
// Arena/elements are aligned to 8 bytes so we can use the lower 3 bits to
// encode whether we're in SOO mode and if so, the SOO size.
// NOTE: we also tried using 3 bits to encode SOO size and use all ones as a
// sentinel value for non-SOO mode, but that was slower in benchmarks/loadtests.
enum { kArenaAlignment = 8 };
// The mask for the size bits in SOO mode, and also a sentinel value indicating
// that the field is not in SOO mode.
enum { kSooPtrMask = ~(kArenaAlignment - 1) };
// This bit is 0 when in SOO mode and 1 when in non-SOO mode.
enum { kNotSooBit = kArenaAlignment >> 1 };
...
struct SooRep {
  ...
  void set_non_soo(bool was_soo, int capacity, void* elements) {
    ...
    ABSL_DCHECK_EQ(reinterpret_cast<uintptr_t>(elements) % kArenaAlignment, 0);
    if (was_soo) {
      PROTOBUF_UNPOISON_MEMORY_REGION(short_rep.data, sizeof(short_rep.data));
      long_rep.size = short_rep.size();
    }
...
class RepeatedField final
 ...
 private:
  ...
  // The number of elements that can be stored in the SOO rep. This is 1 for
  // int64_t, 2 for int32_t, 3 for bool, and 0 for absl::Cord.
  static constexpr int kSooCapacityElements = std::min<int>(
      internal::kSooCapacityBytes / sizeof(Element), internal::kSooSizeMask);
...
RepeatedField<Element>::~RepeatedField() {
  ...
  AnnotateSize(size, Capacity(is_soo));  // Unpoison.
...
inline void RepeatedField<Element>::InternalSwap(
  ...
  AnnotateSize(size(), Capacity());
  other->AnnotateSize(other->size(), other->Capacity());
...
inline int CalculateReserveSize(int capacity, int new_size) {
  ...
  if (sizeof(T) == 1) {
    // When sizeof(T) == 1, 0 < kSooCapacityElements < lower_limit. We need to
    // set capacity to 0 here to ensure power-of-two sized allocations.
...
PROTOBUF_NOINLINE void RepeatedField<Element>::GrowNoAnnotate(bool was_soo,
  ...
  if (arena == nullptr) {
    ...
    internal::SizedPtr res =
        internal::AllocateAtLeastAligned<internal::kArenaAlignment>(bytes);
