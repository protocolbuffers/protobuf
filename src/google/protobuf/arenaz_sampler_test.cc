...
TEST(ThreadSafeArenazSamplerTest, MultiThreaded) {
  ...
  //   Tagged seed sequence (ABSL_RANDOM_SALT_OVERRIDE=fBAY_g):
  //      SEED_MultiThreaded=XVKmv...
  // To reproduce a failure with exactly the same random sequence, do this:
  //   $ export SEED_MultiThreaded=XVKmv... bazel test <this test>
  absl::BitGen bitgen{absl::MakeTaggedSeedSeq(
      /*env_var=*/absl::StrCat("SEED_", test_info_->name()).c_str(),
      /*stream=*/absl::LogInfoStreamer().stream())};
  ThreadPool pool(10);

  for (int i = 0; i < 10; ++i) {
    const int64_t sampling_stride = 11 + i % 3;
    pool.Schedule([&sampler, &stop, &bitgen, sampling_stride]() {
      std::vector<ThreadSafeArenaStats*> infoz;
      while (!stop.HasBeenNotified()) {
        if (infoz.empty()) {
          infoz.push_back(sampler.Register(sampling_stride));
        }
        switch (absl::Uniform<uint8_t>(bitgen, 0, 1)) {
          case 0: {
            infoz.push_back(sampler.Register(sampling_stride));
            break;
          }
