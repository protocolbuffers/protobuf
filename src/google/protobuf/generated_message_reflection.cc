...
void SwapFieldHelper::SwapMessage(const Reflection* r, Message* lhs,
  ...
  if (*lhs_sub != nullptr && *rhs_sub != nullptr) {
    (*lhs_sub)->GetReflection()->Swap(*lhs_sub, *rhs_sub);
             ...
             r->HasFieldSingularOrRepeated(*rhs, field)) {
  ...
  } else if (*rhs_sub == nullptr &&
             r->HasFieldSingularOrRepeated(*lhs, field)) {
...
bool Reflection::HasField(const Message& message,
  ...
  if (field->is_extension()) {
    ...
    if (schema_.InRealOneof(field)) {
      ...
      return HasFieldSingularOrRepeated(message, field);
    }
  }
}
...
void Reflection::ClearField(Message* message,
  ...
  if (field->is_extension()) {
    ...
    return;
  } else if (!HasFieldSingularOrRepeated(*message, field)) {
...
bool Reflection::HasFieldSingularOrRepeated(
    const Message& message, const FieldDescriptor* field) const {
