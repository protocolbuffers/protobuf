...
void Reflection::ListFields(const Message& message,
  ...
  for (int i = 0; i <= last_non_weak_field_index; i++) {
    ...
    if (schema_.InRealOneof(field)) {
      ...
      if (IsFieldPresentGivenHasbits(message, field, has_bits_indices[i])) {
        append_to_output(field);
      }
    } else if (HasFieldNonWeak(message, field)) {
...
bool Reflection::IsHasBitSet(const Message& message,
                             uint32_t has_bit_index) const {
  return IsIndexInHasBitSet(GetHasBits(message), has_bit_index);
}
...
bool Reflection::IsFieldPresentGivenHasbits(const Message& message,
  ...
  if (!IsHasBitSet(message, hasbit_index)) {
    return false;
  }
...
bool Reflection::HasFieldNonWeak(const Message& message,
  ...
  if (schema_.HasBitIndex(field) != static_cast<uint32_t>(-1)) {
    return IsFieldPresentGivenHasbits(message, field,
