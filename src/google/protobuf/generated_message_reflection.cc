...
ReflectionSchema ReflectionSchema::MigrationToReflectionSchema(
  ...
  // Old result.weak_field_map_offset_ (bit 3)
  ABSL_CHECK_EQ(next(), ~uint32_t{});
  // Old result.inlined_string_donated_offset_ (bit 4)
  ABSL_CHECK_EQ(next(), ~uint32_t{});
    ...
    if (schema_.InRealOneof(field)) {
      ...
      if (static_cast<int64_t>(oneof_case) == field->number()) {
        PROTO_REFLECTION_APPEND_OR_RETURN();
                   ...
                   schema_.HasBitIndex(field, /*field_index=*/field->index());
...
const Message* Reflection::GetDefaultMessageInstance(
  ...
  if (!field->is_extension() && !field->is_repeated() && !IsLazyField(field) &&
