...
#define USAGE_CHECK_NOT_MAP(METHOD)              \
  USAGE_CHECK_EQ(field->is_map(), false, METHOD, \
                 "Field is a map; the method requires a non-map field.")
...
void SwapFieldHelper::SwapMessage(const Reflection* r, Message* lhs,
  ...
  } else if (*lhs_sub == nullptr && r->HasHasbitField(*rhs, field)) {
  ...
  } else if (*rhs_sub == nullptr && r->HasHasbitField(*lhs, field)) {
...
bool Reflection::HasField(const Message& message,
  ...
  if (field->is_extension()) {
    ...
    if (schema_.InRealOneof(field)) {
      ...
      return HasHasbitField(message, field);
    }
  }
}
...
void Reflection::ClearField(Message* message,
  ...
  if (HasHasbitField(*message, field)) {
...
void Reflection::ListFields(const Message& message,
  ...
  for (int i = 0; i <= last_non_weak_field_index; i++) {
    ...
    } else if (HasHasbitField(message, field)) {
...
Message* Reflection::AddMessage(Message* message, const FieldDescriptor* field,
  ...
  if (field->is_extension()) {
    ...
    // TODO: can I move this further down (i.e. only if AddFromCleared fails)?
    SetHasBit(message, field);
...
bool Reflection::HasHasbitField(const Message& message,
                                const FieldDescriptor* field) const {
