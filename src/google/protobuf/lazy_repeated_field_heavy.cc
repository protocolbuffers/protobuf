...
void VerifyConsistency(LazyRepeatedPtrField::LogicalState state,
  ...
  auto unparsed_msg = absl::make_unique<RepeatedPtrField<Message>>();
  if (!LazyRepeatedPtrField::ParseWithOuterContext(
          reinterpret_cast<RepeatedPtrFieldBase*>(unparsed_msg.get()), unparsed,
  ...
  // Clone a new one from "msg" to eagerly parse all lazy fields.
  const auto* msg = reinterpret_cast<const RepeatedPtrField<Message>*>(value);
  auto clone = absl::make_unique<RepeatedPtrField<Message>>();
  clone->CopyFrom(*msg);

  // Eagerly parse all lazy fields to eliminate non-canonical wireformat data.
  ABSL_CHECK_EQ(clone->size(), unparsed_msg->size());
  for (int i = 0; i < clone->size(); i++) {
    EagerParseLazyFieldIgnoreUnparsed(*clone->Mutable(i));
    EagerParseLazyFieldIgnoreUnparsed(*unparsed_msg->Mutable(i));
  }

  for (int i = 0; i < clone->size(); i++) {
    ABSL_DCHECK_EQ(DeterministicSerialization(clone->Get(i)),
                   DeterministicSerialization(unparsed_msg->Get(i)))
        << "RepeatedPtrField<" << msg->Get(i).GetTypeName() << ">(" << i << ")"
        << ": likely mutated via getters + const_cast\n"
        << "unparsed:\n"
        << EscapeEncoded(DeterministicSerialization(unparsed_msg->Get(i)))
        << "\nmessage:\n"
        << EscapeEncoded(DeterministicSerialization(clone->Get(i)));
  }
#endif  // !NDEBUG
}
...
LazyRepeatedPtrField::LazyRepeatedPtrField(Arena* arena,
  ...
  switch (rhs.GetLogicalState()) {
      ...
      rhs.unparsed_.Visit(
          ...
          [&](auto view) {
            if (arena == rhs_arena) {
              ...
              unparsed_.InitAsArray(arena, view);
            }
          });
      raw_.store(
    ...
    case LogicalState::kDirty: {
      ...
      auto* value = Arena::CreateMessage<RepeatedPtrFieldBase>(arena);
...
RepeatedPtrFieldBase* LazyRepeatedPtrField::MutableDynamic(
  ...
  // LOG(ERROR) << "MutableDynamic; this=" << this << ";type=" <<
  // type->full_name();
  ...
  switch (GetLogicalState()) {
    ...
    case LogicalState::kClear: {
      ...
      if (arena == nullptr || num_bytes > kMaxArraySize || unparsed_.IsCord()) {
        ...
        unparsed_.InitAsArray(arena, data);
      }
      SetNeedsParse();
      break;
    }
  ...
  switch (other.GetLogicalState()) {
    ...
    case LogicalState::kNoParseRequired:
      if (other.unparsed_.IsCord()) {
        ...
        // Fast path for sharing the unparsed raw array:
        //  - Same arena, not heap
        //  - We are in a clear state
        if (arena == other_arena && arena != nullptr &&
            GetLogicalState() == LogicalState::kClear) {
          unparsed_ = other.unparsed_;
          SetNeedsParse();
        } else {
