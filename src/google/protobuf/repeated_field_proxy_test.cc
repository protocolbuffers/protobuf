...
template <typename ElementTypeParam, bool kUseArena>
using test_param =
    std::integral_constant<RepeatedFieldProxyTestParams<ElementTypeParam>,
                           RepeatedFieldProxyTestParams<ElementTypeParam>{
                               .use_arena = kUseArena}>;
...
class RepeatedFieldProxyTestBase {
 protected:
  ...
  // This is a hack to get the element type to testing code without needing to
  // use `TestParam`.
#define ELEMENT_TYPE() decltype(this->PhonyElementType())

  RepeatedFieldProxy<ElementType> PhonyProxy() const {
    static_assert(false, "This should never actually be called");
  }
  // This is a hack to get the proxy type to testing code without needing to
  // use `TestParam`.
#define PROXY_TYPE() decltype(this->PhonyProxy())
...
template <typename StringType>
void TestStringIterators(google::protobuf::RepeatedFieldProxy<StringType> proxy) {
  proxy.push_back("1");
  proxy.push_back("2");
  proxy.push_back("3");
...
TEST_P(RepeatedFieldProxyTest, StdStringIterators) {
  auto field = MakeRepeatedFieldContainer<std::string>();
...
TEST_P(RepeatedFieldProxyTest, CordIterators) {
  auto field = MakeRepeatedFieldContainer<absl::Cord>();
  TestStringIterators(field.MakeProxy());
}
