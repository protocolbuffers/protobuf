...
class LazyRepeatedPtrField {
 public:
  ...
  const RepeatedPtrFieldBase* TryGetMessage() const;
    ...
    do {
      ...
      if (unparsed_.IsCord()) {
        ...
        if (unparsed_.IsEmpty()) {
          unparsed_.InitAsArray(arena, tmp);
        } else {
 ...
 public:
  ...
  class UnparsedPayload {
   ...
   public:
    ...
    // Initialize the value as an array copied from `view`.
    // Ignores existing value.
    void InitAsArray(Arena* arena, absl::string_view view) {
    ...
    void InitAsArray(Arena* arena, const absl::Cord& cord) {
      auto size = cord.size();
    ...
    char* InitAsArray(Arena* arena, ArraySizeType size) {
      ...
      // Allocated max allowed capacity.
      ...
      memcpy(c, &size, sizeof(ArraySizeType));
    ...
    size_t SpaceUsedExcludingSelf() const {
      return Visit(
          ...
          [](auto view) { return sizeof(ArraySizeType) + view.size(); });
    }
...
inline const RepeatedPtrFieldBase* LazyRepeatedPtrField::TryGetMessage() const {
  switch (GetLogicalState()) {
    case LogicalState::kDirty:
    case LogicalState::kNoParseRequired:
    case LogicalState::kParseRequired:
...
void LazyRepeatedPtrField::OverwriteForTest(RawState status,
  ...
  if (!unparsed.empty()) {
    if (arena != nullptr && unparsed.size() <= kMaxArraySize) {
      unparsed_.InitAsArray(arena, unparsed);
    } else {
