...
TEST(RepeatedFieldTest, CheckedAddOverflowSafetyBoundary) {
  ...
  EXPECT_EQ(internal::CheckedAdd(std::numeric_limits<int>::min(), 0), std::numeric_limits<int>::min());

  // 2. Verify overflow scenarios: under environments supporting death tests,
  // overflow must safely and deterministically terminate the program (e.g., via ABSL_LOG(FATAL)).
  ...
  EXPECT_DEATH(internal::CheckedAdd(std::numeric_limits<int>::max(), std::numeric_limits<int>::max()), ".*");
