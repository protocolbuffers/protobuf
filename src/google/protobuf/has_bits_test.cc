...
class HasBitsTest : public testing::Test {
 public:
  ...
  bool HasBitSet(const Message& msg, const FieldDescriptor* field) {
    ...
    return ref->IsHasBitSet(msg, has_bit_idx);
    return ref->HasField(msg, field);
  }
};
...
TEST_F(HasBitsTest, HasBitsUnsetForDefaultRepeatedField) {
  ...
  EXPECT_FALSE(
      HasBitSet(msg, msg.GetDescriptor()->FindFieldByName("repeated_int32")));
}
...
TEST_F(HasBitsTest, HasBitsSetOnMutableMap) {
  ...
  EXPECT_TRUE(
      HasBitSet(msg, msg.GetDescriptor()->FindFieldByName("map_int32_int32")));
}
