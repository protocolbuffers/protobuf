...
class PROTOBUF_EXPORT RepeatedPtrFieldBase {
 ...
 protected:
  ...
  inline void DeleteSubrange(int start, int num) {
    ...
    if (TypeHandler::UseContiguousLayout() && using_sso() && num != 0 &&
        arena == nullptr) {
      const auto alloc_traits = AllocTraits::FromValue(
          *static_cast<Value<TypeHandler>*>(tagged_rep_or_elem_));
      H::Delete(static_cast<Value<TypeHandler>*>(tagged_rep_or_elem_), arena);
      SizedDelete(tagged_rep_or_elem_, alloc_traits.size_of);
    } else {
      for (int i = 0; i < num; ++i) {
