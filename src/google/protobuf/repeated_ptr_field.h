...
class AllocTraits {
 public:
  ...
  explicit constexpr AllocTraits(const std::string& value)
      : size_of_{sizeof(decltype(value))},
        align_of_{alignof(decltype(value))} {}
  ...
  friend bool operator==(const AllocTraits& lhs, const AllocTraits& rhs) {
    return lhs.size_of_ == rhs.size_of_ && lhs.align_of_ == rhs.align_of_;
  }
...
class ABSL_ATTRIBUTE_WARN_UNUSED RepeatedPtrField final
 ...
 public:
  ...
  //
  // NOTE: `Element` must be a concrete type, i.e. not `Message` or
  // `MessageLite`. For the latter two, use `Reserve(new_size, prototype)`.
 ...
 private:
  ...
  // From //net/proto2/arena:message_movers.
  template <typename Container, typename T>
  friend void AddManyAllocated(Container, RepeatedPtrField<T>*);
  ...
  // An overload of `Reserve()` for when the `Element` type is either `Message`
  // or `MessageLite`. Takes an additional `prototype` param that is used to
  // determine the allocation traits. Available only to allowlisted callers
  // (friends).
  void Reserve(int new_size, const Element& prototype);
};
...
inline void RepeatedPtrField<Element>::Reserve(int new_size) {
  static_assert(TypeHandler::has_default_instance(),
                "Use `Reserve(new_size, prototype)` overload instead");
...
inline void RepeatedPtrField<Element>::Reserve(int new_size,
  ...
  using internal::AllocTraits;
  if constexpr (TypeHandler::has_default_instance()) {
    // Normal code should use `Reserve(new_size)` overload in this case, but
    // that can be cumbersome in generic code that instantiates us over a
    // generic `Element` - so allow this, but do a soft sanity check.
    ABSL_DCHECK(AllocTraits{prototype} ==
                AllocTraits{TypeHandler::default_instance()});
  }
