...
class ABSL_ATTRIBUTE_WARN_UNUSED RepeatedPtrField final
 ...
 private:
  ...
  // Friended to allow calling `AddAllocated` with arena pointers, which may
  // be cached in the iterator. This saves us needing to call `GetArena()` on
  // the `RepeatedPtrField` every insertion, which has 2 levels of indirection.
  template <typename T>
