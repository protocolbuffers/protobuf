...
inline bool ParseWithNullOuterContextImpl(const T& input,
  ...
  if (PROTOBUF_PREDICT_FALSE(ptr == nullptr)) return false;
  // TODO: At this moment we know how many elements there are. Also no need to
  // actually reading the tag. _InternalParse should guarantee that tags are the
  // same and are valid.
  ptr = LazyRepeatedPtrField::ParseToRepeatedMessage<uint32_t>(
...
void LazyRepeatedPtrField::Swap(LazyRepeatedPtrField* lhs, Arena* lhs_arena,
  ...
  static auto reallocate = [](LazyRepeatedPtrField* f, Arena* arena,
    ...
    auto* value = raw.mutable_value();
    if (value != nullptr) {
      auto* new_value = Arena::CreateMessage<RepeatedPtrFieldBase>(arena);
      if (!value->empty()) {
        new_value->MergeFrom<MessageLite>(*value);
      }
      if (cleanup_old) {
