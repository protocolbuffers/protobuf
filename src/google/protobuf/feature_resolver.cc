  ...
  for (int i = 0; i < descriptor.field_count(); ++i) {
    ...
    for (const auto& d : field.options().edition_defaults()) {
      ...
      if (d.edition() < Edition::EDITION_2023) {
        // Allow defaults to be specified in proto2/proto3, predating editions.
        continue;
      }
...
absl::StatusOr<FeatureSetDefaults> FeatureResolver::CompileDefaults(
  ...
  ABSL_CHECK_LE(*editions.begin(), EDITION_LEGACY);
...
FeatureResolver::ValidationResults FeatureResolver::ValidateFeatureLifetimes(
  ...
  // Unknown fields should be impossible, as it would mean the options failed to
  // be resolved.
  ABSL_DCHECK(
      pool_features->GetReflection()->GetUnknownFields(*pool_features).empty());
