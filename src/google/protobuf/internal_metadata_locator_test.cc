...
struct TaggedFieldWithInternalMetadataOffset {
  ...
  void Swap(TaggedFieldWithInternalMetadataOffset<kTagMask>& other) {
    std::swap(field, other.field);
    resolver.SwapTags(other.resolver);
  }

  int field = 0;
  TaggedInternalMetadataResolver<kTagMask> resolver;
};

template <uint32_t kTagMask, size_t kPadding = 0>
struct TaggedStructWithInternalMetadata {
  ...
  Arena* GetArena() const {
    return ResolveTaggedArena<
        &TaggedFieldWithInternalMetadataOffset<kTagMask>::resolver, kTagMask>(
        &field);
  }

  template <size_t kOtherPadding>
  void Swap(TaggedStructWithInternalMetadata<kTagMask, kOtherPadding>& other) {
  ...
  TaggedFieldWithInternalMetadataOffset<kTagMask> field;
};
...
TEST(TaggedInternalMetadataLocatorTest, ReadTag) {
  static constexpr uint32_t kTagMask = 0x1;
  TaggedStructWithInternalMetadata<kTagMask> message;
...
TEST(TaggedInternalMetadataLocatorTest, ReadTagWithArena) {
  static constexpr uint32_t kTagMask = 0x1;
  Arena arena;
  TaggedStructWithInternalMetadata<kTagMask> message(&arena);
...
TEST(TaggedInternalMetadataLocatorTest, Swap) {
  static constexpr uint32_t kTagMask = 0x3;

  Arena arena1, arena2;
  // Use different amounts of padding to ensure that the swap works correctly
  // when the internal metadata offsets are different.
  TaggedStructWithInternalMetadata<kTagMask, /*kPadding=*/16> container1(
      &arena1, 10);
  TaggedStructWithInternalMetadata<kTagMask, /*kPadding=*/32> container2(
      &arena2, 20);
