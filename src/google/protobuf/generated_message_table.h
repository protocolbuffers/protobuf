...
//
// template <size_t kNumFields, bool kHasAuxHeader, size_t kNumAux>
// struct MessageTable {
//   MessageTableBase header;
//   Array<FieldEntry, kNumFields> field_entries;
//   MessageTableAuxImpl<kHasAuxHeader> aux_header;
...
//
// If a message has split, oneof, etc. it uses `aux_header` but not `aux_entry`
// (use MessageTable<num_fields, true>).
template <bool kHasAuxHeader>
...
template <size_t kNumFields, bool kHasAuxHeader = false, size_t kNumAux = 0>
struct MessageTable {
  // Makes sure there is no padding between header and field_entries.
  static_assert(alignof(MessageTableBase) >=
                    alignof(std::array<FieldEntry, kNumFields>),
                "");
  // Makes sure there is no padding between aux_header and aux_entries.
  static_assert(alignof(MessageTableAux) >=
                    alignof(std::array<AuxEntry, kNumAux>),
                "");
  MessageTableBase header;
  ABSL_ATTRIBUTE_NO_UNIQUE_ADDRESS Array<FieldEntry, kNumFields> field_entries;
  ABSL_ATTRIBUTE_NO_UNIQUE_ADDRESS MessageTableAuxImpl<kHasAuxHeader>
      aux_header;
  ABSL_ATTRIBUTE_NO_UNIQUE_ADDRESS Array<AuxEntry, kNumAux> aux_entries;
};

constexpr MessageTable<0, false, 0> kEmptyMessageTable = {
