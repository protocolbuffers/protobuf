...
TEST_F(CommandLineInterfaceTest, NamingStyleManglingStartWithHasEnforced) {
  CreateTempFile("foo.proto",
                 R"schema(
    edition = "UNSTABLE";
    message Foo {
...
TEST_F(CommandLineInterfaceTest, NamingStyleManglingEndWithValueEnforced) {
  CreateTempFile("foo.proto",
                 R"schema(
    edition = "UNSTABLE";
    message Foo {
  ...
  Run("protocol_compiler --proto_path=$tmpdir --test_out=$tmpdir foo.proto");
  ExpectErrorSubstring(
      "Field name x_value should not end with _value. This can "
      "cause collisions.");
}
