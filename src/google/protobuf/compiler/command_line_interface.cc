...
CodeGeneratorRequest CommandLineInterface::CreateCodeGeneratorRequest(
  ...
  if (!params.parameter.empty()) {
    request.set_parameter(params.parameter);
  }

  absl::flat_hash_set<const FileDescriptor*> already_seen;
  for (const FileDescriptor* file : params.parsed_files) {
  ...
  // proto_file.
  ABSL_CHECK(!params.parsed_files.empty());
  const DescriptorPool* pool = params.parsed_files[0]->pool();
...
bool CommandLineInterface::GeneratePluginOutput(
  ...
  CodeGeneratorRequest request = CreateCodeGeneratorRequest(
      {/*parsed_files=*/parsed_files,
       ...
       /*calculate_json_name=*/!builtin_plugins->contains(plugin_name),
       /*bootstrap=*/bootstrap});
...
bool CommandLineInterface::GenerateBuiltInOutput(
  ...
  CodeGeneratorRequest request = CreateCodeGeneratorRequest(
      {/*parsed_files=*/parsed_files, /*parameter=*/parameters,
       /*calculate_json_name=*/false, /*bootstrap=*/false});
  CodeGeneratorResponse response;
