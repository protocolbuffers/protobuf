...
inline bool UseOldFileClassNameDefault(const FileDescriptor* file) {
  bool result = JavaGenerator::GetResolvedSourceFeatures(*file)
                    .GetExtension(pb::java)
                    .use_old_outer_classname_default();
  // ABSL_CHECK(result)
  //     << "AAAAAAAAAAAAAAAA " << file->name() << " "
  //     << JavaGenerator::GetResolvedSourceFeatures(*file).DebugString();
  // std::cerr << "RES RES " << result << " file: " << file->name() << "\n";
  return result;
}
