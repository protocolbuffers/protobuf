...
void PrintComment(const std::string& comment, io::Printer* printer) {
  ...
  std::vector<std::string> lines = absl::StrSplit(comment, '\n', absl::SkipEmpty());
...
std::string RBSMessageFullName(const Descriptor* message) {
  if (message->containing_type() != nullptr) {
    return absl::StrCat(RBSMessageFullName(message->containing_type()), "::", RubifyConstant(message->name()));
  } else {
    return absl::StrCat(ModulePath(message->file()), "::", RubifyConstant(message->name()));
  }
}
std::string RBSEnumFullName(const EnumDescriptor* enum_) {
  if (enum_->containing_type() != nullptr) {
    return absl::StrCat(RBSMessageFullName(enum_->containing_type()), "::", RubifyConstant(enum_->name()));
  } else {
    return absl::StrCat(ModulePath(enum_->file()), "::", RubifyConstant(enum_->name()));
  }
}
...
std::string ScalarReadType(const FieldDescriptor *field) {
  switch (field->type()) {
    ...
    case FieldDescriptor::TYPE_ENUM:
      ...
      return absl::StrCat(RBSEnumFullName(field->enum_type()), "::names | ::Integer");
...
std::string ScalarWriteType(const FieldDescriptor *field) {
  switch (field->type()) {
    ...
    case FieldDescriptor::TYPE_ENUM: {
      ...
      // - protobuf_c: Integer | Float | String | Symbol where String and Symbol must be known names
      // - protobuf_ffi: (Numeric & _ToI) | String | Symbol where String and Symbol must be known names
      const std::string enum_name = RBSEnumFullName(field->enum_type());
      return absl::StrCat(enum_name, "::names | ", enum_name, "::strings | ::Integer | ::Float");
    }
    case FieldDescriptor::TYPE_GROUP:
    case FieldDescriptor::TYPE_MESSAGE: {
      // - protobuf_c: TheMessageType | Time (for Timestamp) | (Numeric & _ToInt) (for Duration)
      // - protobuf_ffi: TheMessageType | Hash | Time (for Timestamp) | (Numeric & _ToInt) (for Duration)
      const Descriptor::WellKnownType wkt = field->message_type()->well_known_type();
      if (wkt == Descriptor::WELLKNOWNTYPE_TIMESTAMP) {
        return absl::StrCat(RBSMessageFullName(field->message_type()), " | ::Time");
      } else if (wkt == Descriptor::WELLKNOWNTYPE_DURATION) {
        return absl::StrCat(RBSMessageFullName(field->message_type()), " | ::int");
      } else {
        return RBSMessageFullName(field->message_type());
      }
    }
...
std::string FieldReadType(const FieldDescriptor *field) {
  ...
  // - read optionalities: unfortunately, neither of them are handled in a Ruby-friendly way:
  //   - oneof fields
  //   - proto2 optional
  //   - proto3 optional
  //   though you can manipulate optionality through `clear_*` and `has_*?` methods
  // - write optionalities: same as above, but real oneofs are handled as exceptions
  if (field->is_map()) {
    const std::string key_read_type = ScalarReadType(field->message_type()->map_key());
    const std::string key_write_type = ScalarWriteType(field->message_type()->map_key());
    const std::string value_read_type = ScalarReadType(field->message_type()->map_value());
    const std::string value_write_type = ScalarWriteType(field->message_type()->map_value());
    return absl::StrCat("::Google::Protobuf::Map[", key_read_type, ", ", value_read_type, ", ", key_write_type, ", ", value_write_type, "]");
  } else if (field->is_repeated()) {
    const std::string element_read_type = ScalarReadType(field);
    const std::string element_write_type = ScalarWriteType(field);
    return absl::StrCat("::Google::Protobuf::RepeatedField[", element_read_type, ", ", element_write_type, "]");
  } else if (field->type() == FieldDescriptor::TYPE_MESSAGE || field->type() == FieldDescriptor::TYPE_GROUP) {
    return MakeOptionalType(ScalarReadType(field));
  } else {
    // Proto2 optionals, proto3 optionals, and real oneof fields don't return nil.
    // And that is why they are handled here.
    // This is unfortunate, but you can still manipulate optionality through `clear_*` and `has_*?` methods.
...
std::string FieldWriteType(const FieldDescriptor *field) {
  ...
  } else if (field->type() == FieldDescriptor::TYPE_MESSAGE || field->type() == FieldDescriptor::TYPE_GROUP || field->real_containing_oneof()) {
    // In addition to sub-messages, real oneofs accept nil (but won't return it).
...
std::string FieldInitType(const FieldDescriptor *field) {
  // All of them are optional here, as nil means to just skip initialization for the field.
  if (field->is_map()) {
    const std::string key_write_type = ScalarWriteType(field->message_type()->map_key());
    const std::string value_write_type = ScalarWriteType(field->message_type()->map_value());
    ...
    return MakeOptionalType(absl::StrCat(full_name, " | ", full_name, "::init_map"));
  } else {
    return MakeOptionalType(ScalarWriteType(field));
  }
}
...
void GenerateEnumValueTypeDefinition(const EnumValueDescriptor* value, io::Printer* printer) {
...
void GenerateEnumTypeDefinition(const EnumDescriptor* enum_, io::Printer* printer) {
  SourceLocation location;
  ...
  std::unordered_map<int, std::vector<std::string>> names_by_number;
  for (int i = 0; i < enum_->value_count(); i++) {
    const EnumValueDescriptor* value = enum_->value(i);
    // Check if there is an entry from names_by_number and if so, get the position
    auto it = names_by_number.find(value->number());
    if (it == names_by_number.end()) {
      names_by_number[value->number()] = {value->name()};
      unique_numbers.push_back(value->number());
    } else {
      it->second.push_back(value->name());
    }
  }
  ...
  for (int number : unique_numbers) {
    ...
    for (int i = 1; i < names.size(); i++) {
      name_union = absl::StrCat(name_union, " | :", names[i]);
    }
    printer->Print(
        "$sep$($number$ number) -> $name_union$\n",
        "sep", UnionSeparator(overload_initial),
        "number", absl::StrCat(number),
        "name_union", WrapUnion(name_union));
  }
  printer->Print(
      "$sep$(::int number) -> names?\n",
      "sep", UnionSeparator(overload_initial));
  printer->Print(
      "$sep$...\n",
      "sep", UnionSeparator(overload_initial));
  ...
  for (int i = 0; i < enum_->value_count(); i++) {
    const EnumValueDescriptor* value = enum_->value(i);
    printer->Print(
        "$sep$(:$name$ name) -> $number$\n",
        "sep", UnionSeparator(overload_initial),
        "name", value->name(),
        "number", absl::StrCat(value->number()));
  }
  printer->Print(
      "$sep$(::Symbol name) -> numbers?\n",
      "sep", UnionSeparator(overload_initial));
  printer->Print(
      "$sep$...\n",
      "sep", UnionSeparator(overload_initial));
  ...
  for (int i = 1; i < enum_->value_count(); i++) {
    all_name_union = absl::StrCat(all_name_union, " | :", enum_->value(i)->name());
  }
  InsertMidLine(initial, printer);
  printer->Print(
      "type names = $name_union$\n",
      "name_union", all_name_union);

  std::string all_string_union = absl::StrCat("\"", enum_->value(0)->name(), "\"");
  for (int i = 1; i < enum_->value_count(); i++) {
    all_string_union = absl::StrCat(all_string_union, " | \"", enum_->value(i)->name(), "\"");
  }
  InsertMidLine(initial, printer);
  printer->Print(
      "type strings = $string_union$\n",
      "string_union", all_string_union);

  std::string all_number_union = absl::StrCat(unique_numbers[0]);
  for (int i = 1; i < unique_numbers.size(); i++) {
    all_number_union = absl::StrCat(all_number_union, " | ", unique_numbers[i]);
  }
  InsertMidLine(initial, printer);
  printer->Print(
      "type numbers = $number_union$\n",
      "number_union", all_number_union);

  printer->Outdent();
  printer->Print("end\n");
}

void GenerateFieldTypeDefinition(const FieldDescriptor* field, io::Printer* printer) {
  SourceLocation location;
  ...
  if (!field->is_repeated() && IsWrapper(field)) {
    ...
    const FieldDescriptor *wrapped_field = field->message_type()->FindFieldByNumber(1);
    if (wrapped_field != nullptr) {
      // attr_accessor my_field_as_value(): ::Integer?
      std::string wrapped_read_type = MakeOptionalType(ScalarReadType(wrapped_field));
      std::string wrapped_write_type = MakeOptionalType(ScalarWriteType(wrapped_field));
      if (wrapped_read_type == wrapped_write_type) {
        printer->Print(
            "attr_accessor $name$_as_value(): $read_type$\n",
            "name", field->name(),
            "read_type", wrapped_read_type);
      } else {
        printer->Print(
  ...
  if (field->type() == FieldDescriptor::TYPE_ENUM) {
    ...
    if (field->is_repeated()) {
      printer->Print(
          "attr_reader $name$_const(): ::Array[::Integer]\n",
          "name", field->name());
    } else {
      // Always non-optional
      printer->Print(
          "attr_reader $name$_const(): ::Integer\n",
          "name", field->name());
    }
  }
...
void GenerateOneofDeclTypeDefinition(const OneofDescriptor* oneof, io::Printer* printer) {
  SourceLocation location;
...
void GenerateMessageInitMap(const Descriptor* message, io::Printer* printer) {
  ...
  for (int i = 0; i < message->field_count(); i++) {
    ...
    printer->Print(
        // "?$name$: $type$,\n", // Wait for https://github.com/ruby/rbs/pull/1717
        "$name$: $type$,\n",
        "name", field->name(),
        "type", init_type);
    printer->Print(
...
void GenerateIndexReaderDefinition(const Descriptor* message, io::Printer* printer) {
  printer->Print("def []:\n");
  printer->Indent();
  bool overload_initial = true;
  for (int i = 0; i < message->field_count(); i++) {
...
void GenerateIndexWriterDefinition(const Descriptor* message, io::Printer* printer) {
  printer->Print("def []=:\n");
  printer->Indent();
  bool overload_initial = true;
  for (int i = 0; i < message->field_count(); i++) {
...
void GenerateMessageTypeDefinition(const Descriptor* message, io::Printer* printer) {
  SourceLocation location;
  ...
  printer->Print(
      "class $classname$ < ::Google::Protobuf::AbstractMessage\n",
  ...
  InsertMidLine(initial, printer);
  printer->Print(
    "include ::Google::Protobuf::_MessageClass[$name$]\n",
    "name", RBSMessageFullName(message));
...
void GenerateEnumLookup(const EnumDescriptor* enum_, io::Printer* printer, bool &initial) {
  printer->Print(
    "$sep$(\"$full_name$\" name) -> (::Google::Protobuf::EnumDescriptor & ::Google::Protobuf::_SpecificEnumDescriptor[singleton($ruby_name$)])\n",
    "sep", UnionSeparator(initial),
    "full_name", enum_->full_name(),
    "ruby_name", RBSEnumFullName(enum_));
}
void GenerateMessageLookup(const Descriptor* message, io::Printer* printer, bool &initial) {
  printer->Print(
    "$sep$(\"$full_name$\" name) -> (::Google::Protobuf::Descriptor & ::Google::Protobuf::_SpecificDescriptor[singleton($ruby_name$)])\n",
    "sep", UnionSeparator(initial),
...
void GenerateDescriptorLookupOverride(const FileDescriptor* file, io::Printer* printer) {
