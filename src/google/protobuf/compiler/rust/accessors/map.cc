...
void Map::InMsgImpl(Context<FieldDescriptor> field) const {
  ...
  field.Emit({{"field", field.desc().name()},
              {"KeyRsType", PrimitiveRsTypeName(key_type)},
              {"ValueRsType", PrimitiveRsTypeName(value_type)},
              ...
              {"getter",
               [&] {
                 if (field.is_upb()) {
                   field.Emit({}, R"rs(
                pub fn $field$(&self) -> $pbr$::Map<'_, $KeyRsType$, $ValueRsType$> {
                  let inner = unsafe {
                    $getter_thunk$(self.inner.msg)
                  }.map_or_else(|| unsafe {$pbr$::empty_map()}, |raw| {
                    $pbr$::MapInner{ raw, arena: &self.inner.arena }
                  });
                  $pbr$::Map::from_inner($pbi$::Private, inner)
                }

                pub fn $field$_mut(&mut self) -> $pbr$::Map<'_, $KeyRsType$, $ValueRsType$> {
                  let raw = unsafe {
  ...
  field.Emit(
      {
          ...
          {"getter",
           [&] {
             if (field.is_upb()) {
               field.Emit({}, R"rs(
          fn $getter_thunk$(raw_msg: $pbi$::RawMessage) -> Option<$pbi$::RawMap>;
          fn $getter_thunk_mut$(raw_msg: $pbi$::RawMessage, arena: $pbi$::RawArena) -> $pbi$::RawMap;
        )rs");
             }
           }},
      },
