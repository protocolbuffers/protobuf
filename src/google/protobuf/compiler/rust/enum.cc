...
void UpbTypeConversions(Context& ctx, const EnumDescriptor& desc) {
  ...
  ctx.Emit(
      {
          ...
          {"upb_type_conversions_impl", [&] { UpbTypeConversions(ctx, desc); }},
      },
      R"rs(
      #[repr(transparent)]
      ...
      impl $pb$::EntityType for $name$ {
          type Type = $pb$::EnumEntity;
      }
