...
void UpbTypeConversions(Context& ctx, const Descriptor& msg) {
  if (ctx.opts().kernel == Kernel::kUpb) {
    ctx.Emit(
        {
            {"new_thunk", ThunkName(ctx, msg, "new")},
...
void GenerateRs(Context& ctx, const Descriptor& msg) {
  ...
  ctx.Emit(
      {{"Msg", RsSafeName(msg.name())},
       ...
       {"upb_type_conversions_impl", [&] { UpbTypeConversions(ctx, msg); }},
       {"unwrap_upb",
        [&] {
          if (ctx.is_upb()) {
      ...
      R"rs(
        ...
        impl<'msg> $pb$::EntityType for $Msg$View<'msg> {
            type Type = $pb$::MessageViewEntity;
        }
