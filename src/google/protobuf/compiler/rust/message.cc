        ...
        ctx.Emit(
            ...
            impl $pb$::ProxiedInMapValue<$key_t$> for $Msg$ {
                ...
                unsafe fn map_free(_private: $pbi$::Private, map: &mut $pb$::Map<$key_t$, Self>) {
                    use $pbr$::MapKey;
                    unsafe { $pbr$::proto2_rust_map_free(map.as_raw($pbi$::Private), $key_t$::CATEGORY, $pb$::View::<$Msg$>::default().raw_msg()); }
                }

                fn map_clear(mut map: $pb$::MapMut<$key_t$, Self>) {
                    use $pbr$::MapKey;
                    unsafe { $pbr$::proto2_rust_map_clear(map.as_raw($pbi$::Private), $key_t$::CATEGORY, $pb$::View::<$Msg$>::default().raw_msg()); }
                }
                    ...
                    unsafe {
                        iter.as_raw_mut($pbi$::Private).next_unchecked::<$key_t$, Self, _, _>(
                            |iter, key, value| { $pbr$::$map_iter_get$(
                                iter, $pb$::View::<$Msg$>::default().raw_msg(), key, value) },
