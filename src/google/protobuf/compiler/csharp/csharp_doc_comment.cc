...
// Escapes comment text taken from a .proto file so that it can be embedded in
// a C# delimited documentation comment ("/** ... */") as XML content.
//
// Note that the generated comments use delimited comments rather
// than single line ("///") ones to avoid edge case issues including where
// C# has a different definition for what characters are line terminators
// compared to .proto syntax.
static std::string EscapeXmlDocComment(absl::string_view input) {
  std::string result;
  ...
  for (const char c : input) {
    switch (c) {
      case '*':
        // Avoid "/*", which would start a nested comment.
        if (prev == '/') {
          result.append("&#42;");
        } else {
          result.push_back(c);
        ...
        break;
      case '/':
        // Avoid "*/", which would end the comment.
        if (prev == '*') {
          result.append("&#47;");
        } else {
          result.push_back(c);
        ...
        break;
      case '&':
        // XML escaping... no need for apostrophes etc as the whole text is
        // going to be a child node of a summary element, not part of an
        // attribute.
        result.append("&amp;");
        break;
      case '<':
...
void WriteDocCommentBodyImpl(io::Printer* printer, SourceLocation location) {
  ...
  comments = EscapeXmlDocComment(comments);
  ...
  // lines, either.)
  //
  // Every line is prefixed with " * ". The C# compiler strips that exterior
  // pattern (whitespace, an asterisk and the whitespace following it) back off
  // when it extracts the documentation, but only if the pattern is identical
  // on every line of the comment - hence the deliberate trailing space on
  // otherwise blank lines, without which the original indentation of each line
  // would be off by one. The space after the asterisk also stops a line
  // starting with '/' from ending the comment.
