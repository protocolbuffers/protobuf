...
bool CppGenerator::GenerateAll(const std::vector<const FileDescriptor*>& files,
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    succeeded =
        GenerateImpl(file, parameter, generator_context, error, file_options);
...
bool CppGenerator::Generate(const FileDescriptor* file,
                            ...
                            GeneratorContext* generator_context,
                            std::string* error) const {
  // Call CppGenerator::GenerateAll, not just GenerateAll. If a class inherits
  // from CppGenerator, we don't want to call the GenerateAll of the derived
  // class here.
  return CppGenerator::GenerateAll({file}, parameter, generator_context, error);
}
