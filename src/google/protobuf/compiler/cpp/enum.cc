...
EnumGenerator::EnumGenerator(const EnumDescriptor* descriptor,
  ...
  if (values_range < 16u || values_range < total_values * 2u) {
    ...
    dense_chunks_.push_back({limits_.min->number(), limits_.max->number()});
  } else if (!sorted_unique_values_.empty() && total_values <= 1024) {
    ...
    for (int val : sorted_unique_values_) {
      ...
      if (maybe_chunk_width >= 16u &&
          maybe_chunk_width >= maybe_chunk_size * 2u) {
...
void EnumGenerator::GenerateDefinition(io::Printer* p) {
  ...
  if (!dense_chunks_.empty() || !has_reflection_) {
    ...
    if (!dense_chunks_.empty()) {
      ...
      p->Emit(
          {
              // Using std::to_array would allow us to skip the chunk_count,
              // but it requires C++ 20.
              {"chunk_count", absl::StrCat(dense_chunks_.size())},
              {"chunks", absl::StrJoin(dense_chunks_, ", ",
                                       [](std::string* out,
                                          const internal::ChunkInfo& chunk) {
                                         absl::StrAppend(out, "{",
                                                         chunk.min_val, ", ",
                                                         chunk.max_val, "}");
                                       })},
          ...
          R"cc(
            //~ ADL Hook
            constexpr auto ProtobufInternalEnumChunks($Msg_Enum$) {
              return std::array<const $pbi$::ChunkInfo, $chunk_count$>({{$chunks$}});
            }

            template <>
            $nodiscard $inline $return_type$ $Msg_Enum$_Name($Msg_Enum$ value) {
              return $pbi$::NameOfFastEnum<$Msg_Enum$>(static_cast<int>(value));
            }
          )cc");
    }
  } else {
