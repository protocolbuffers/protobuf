...
bool IsExplicitNumeric8bit(const FieldDescriptor* f) {
  if (ShouldEmitNonDefaultCheck(f)) return false;

  return f->cpp_type() == FieldDescriptor::CPPTYPE_BOOL;
}

bool IsExplicitNumeric32bit(const FieldDescriptor* f) {
  if (ShouldEmitNonDefaultCheck(f)) return false;

  return f->cpp_type() == FieldDescriptor::CPPTYPE_INT32 ||
         f->cpp_type() == FieldDescriptor::CPPTYPE_UINT32 ||
         f->cpp_type() == FieldDescriptor::CPPTYPE_FLOAT ||
         f->cpp_type() == FieldDescriptor::CPPTYPE_ENUM;
}

bool IsExplicitNumeric64bit(const FieldDescriptor* f) {
  if (ShouldEmitNonDefaultCheck(f)) return false;

  return f->cpp_type() == FieldDescriptor::CPPTYPE_INT64 ||
         f->cpp_type() == FieldDescriptor::CPPTYPE_UINT64 ||
         f->cpp_type() == FieldDescriptor::CPPTYPE_DOUBLE;
}
      ...
      {{"handle_extension_set",
       ...
       {"handle_fields",
        [&] {
          ...
          while (it != end) {
            ABSL_CHECK(!it->fields.empty()) << descriptor_->full_name();

            // Group chunks on the same has word index (or none).
            auto next = FindNextUnequalChunk(
                it, end, [&](const FieldChunk& a, const FieldChunk& b) {
                  ABSL_CHECK(!a.fields.empty());
                  ABSL_CHECK(!b.fields.empty());

                  return a.has_hasbit == b.has_hasbit &&
                         a.should_split == b.should_split &&
                         HasWordIndex(a.fields.front()) ==
                             HasWordIndex(b.fields.front());
                });

            if (it->has_hasbit) {
              EmitUpdateByteSizeV2ForNumerics(
                  sizeof(bool), p, cached_has_word_index,
                  ExtractFields(it, next, [](const FieldDescriptor* f) {
                    return IsExplicitNumeric8bit(f);
                  }));

              EmitUpdateByteSizeV2ForNumerics(
                  sizeof(uint32_t), p, cached_has_word_index,
                  ExtractFields(it, next, [](const FieldDescriptor* f) {
                    return IsExplicitNumeric32bit(f);
                  }));

              EmitUpdateByteSizeV2ForNumerics(
                  sizeof(uint64_t), p, cached_has_word_index,
                  ExtractFields(it, next, [](const FieldDescriptor* f) {
                    return IsExplicitNumeric64bit(f);
                  }));
            }

            // Handle the rest in the previous way.
            bool has_haswords_check =
                MaybeEmitHaswordsCheck(it, next, options_, has_bit_indices_,
                                       cached_has_word_index, "this_.", p);
