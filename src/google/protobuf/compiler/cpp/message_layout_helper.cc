...
MessageLayoutHelper::ConsolidateAlignedFieldGroups(
  ...
  // Iterate from coldest to hottest partition, so that the padding of the
  // hotter partitions can be filled with colder fields.
  for (size_t h = 0; h < kMaxHotness; ++h) {
    ...
    // Don't promote fields into the repeated category, since all repeated
    // fields are 8-byte aligned and therefore promotion isn't possible.
    if (h != kRepeated) {
