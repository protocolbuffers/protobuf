...
auto FindIncompleteBlock(std::vector<FieldGroup>& aligned_to_8) {
  return absl::c_find_if(aligned_to_8, [](const FieldGroup& fg) {
    return fg.estimate_memory_size() <= 4;
  });
}
...
bool FieldGroup::UpdatePreferredLocationAndInsertOtherFields(
  ...
  estimate_memory_size_ += other.estimate_memory_size_;
...
void MessageLayoutHelper::DoOptimizeLayout(
  ...
  auto field_groups =
      ConsolidateFieldAlignmentGroups(std::move(field_alignment_groups));
  auto ordered_fields =
...
MessageLayoutHelper::ConsolidateFieldAlignmentGroups(
    FieldAlignmentGroups&& field_alignment_groups) {
...
MessageLayoutHelper::BuildFieldDescriptorOrder(
...
MessageLayoutHelper::ConsolidateAlignedFieldGroups(
  ...
  for (size_t h = kMaxHotness - 1; h < kMaxHotness; --h) {
    ...
    FillPaddingWithColderFields(partitions_aligned_to_target, h,
                                target_alignment);
  }
...
void MessageLayoutHelper::FillPaddingFromPartition(
  ...
  // ascending order, when searching for a group from src_partition that fits
  ...
  std::stable_sort(dst_partition.begin(), dst_partition.end(),
                   [](const FieldGroup& fg1, const FieldGroup& fg2) {
                     return fg1.estimate_memory_size() <
                            fg2.estimate_memory_size();
                   });
  ...
  for (auto dst_it = dst_partition.begin(), src_it = src_partition.begin();
    ...
    while (src_it != src_partition.end() &&
           dst_it->estimate_memory_size() + src_it->estimate_memory_size() >
               alignment) {
      ++src_it;
    }
  ...
  for (const auto& field_group : dst_partition) {
    ABSL_CHECK(field_group.estimate_memory_size() <= alignment)
        << "Field group of size " << field_group.estimate_memory_size()
        << " should have been merged into a group at most " << alignment
        << " bytes.";
  }
}

void MessageLayoutHelper::FillPaddingWithColderFields(
    std::array<std::vector<FieldGroup>, kMaxHotness>& partitions,
    size_t hotness, size_t alignment) {
  auto& target_partition = partitions[hotness];
  if (target_partition.empty()) {
    return;
  }

  for (size_t c = kCold; c > hotness; --c) {
    auto& colder_partition = partitions[c];
    FillPaddingFromPartition(target_partition, colder_partition, alignment);
  }
}
