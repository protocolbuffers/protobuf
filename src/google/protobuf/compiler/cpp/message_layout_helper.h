...
class FieldGroup {
 public:
  ...
  size_t estimate_memory_size() const {
    return static_cast<size_t>(estimate_memory_size_);
  }
 ...
 private:
  ...
  uint32_t estimate_memory_size_;
};
...
class MessageLayoutHelper {
 ...
 private:
  ...
  static FieldPartitionArray ConsolidateFieldAlignmentGroups(
      FieldAlignmentGroups&& field_alignment_groups);

  // Builds the final field order from the partitioned field groups. The order
  // is based first on `FieldHotness`, then on `FieldFamily` within each hotness
  // span. Repeated fields appear first, followed by hot fields of all other
  // types, then warm/cold/split fields. Within each hotness span, the order is
  // based on `FieldFamily`, arranged in a way to maximize contiguous spans of
  // zero-initializable fields.
  std::vector<const FieldDescriptor*> BuildFieldDescriptorOrder(
  ...
  // Promote fields into hotness level `hotness` if doing so would not increase
  // the size of the partition. Search for groups that fit into the extra
  // padding of existing groups in the `hotness` category. This will neither
  // increase the size of the partition nor break runs of field families.
  //
  // This must be called after all partitions colder than `hotness` in
  // `partitions` have been processed.
  static void FillPaddingWithColderFields(
      std::array<std::vector<FieldGroup>, kMaxHotness>& partitions,
      size_t hotness, size_t alignment);
