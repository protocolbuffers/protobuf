...
class UnsetFieldsMetadataTestUtil {
 public:
  static std::vector<const void*> GetRawAddresses(
    ...
    for (const auto p : metadata.addresses) {
      vals.push_back(p.field_pointer);
    }
    return vals;
  }
  static const void* GetAddress(const Message& message,
    ...
    return TextFormat::Parser::UnsetFieldsMetadata::
...
TEST_F(TextFormatTest, PopulatesNoOpFields) {
  ...
  {
    UnsetFieldsMetadataTestUtil::Clear(&no_op_fields);
    ...
    EXPECT_EQ(UnsetFieldsMetadataTestUtil::NumFields(no_op_fields), 1);
  }
  {
    UnsetFieldsMetadataTestUtil::Clear(&no_op_fields);
  ...
  }
  {
    ...
    EXPECT_TRUE(
        TextFormat::ParseFromString(nested_message_parse_string, &proto));
    EXPECT_TRUE(parser.ParseFromString(nested_message_parse_string, &proto));
    EXPECT_EQ(UnsetFieldsMetadataTestUtil::NumFields(no_op_fields), 0);
  }
  {
    UnsetFieldsMetadataTestUtil::Clear(&no_op_fields);
...
TEST_F(TextFormatTest, FieldsPopulatedCorrectly) {
  ...
  parser.OutputNoOpFields(&no_op_fields);
  {
    ...
    ASSERT_EQ(UnsetFieldsMetadataTestUtil::NumFields(no_op_fields), 2);
    std::vector<const void*> ptrs =
        UnsetFieldsMetadataTestUtil::GetRawAddresses(no_op_fields);
  ...
  }
  {
    ...
    EXPECT_TRUE(parser.ParseFromString(parse_string, &proto));
    ASSERT_EQ(UnsetFieldsMetadataTestUtil::NumFields(no_op_fields), 1);
    std::vector<const void*> ptrs =
        UnsetFieldsMetadataTestUtil::GetRawAddresses(no_op_fields);
  ...
  }
  {
    ...
    EXPECT_EQ(
        UnsetFieldsMetadataTestUtil::GetRawAddresses(no_op_fields)[0],
        UnsetFieldsMetadataTestUtil::GetAddress(proto, *reflection, *field));
  }
  {
    // The address returned by the field is a string_view, which is a separate
    // alocation. Check address directly.
