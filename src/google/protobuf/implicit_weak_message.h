...
class PROTOBUF_EXPORT ImplicitWeakMessage final : public MessageLite {
 public:
  ...
  explicit ImplicitWeakMessage(Arena* arena)
      : MessageLite(arena, _class_data_.base()), data_(new std::string) {}
 ...
 private:
  static const ClassDataLite<1> _class_data_;

  static void CheckTypeAndMergeFromImpl(MessageLite& msg,
                                        const MessageLite& other) {
    google::protobuf::DownCastToGenerated<ImplicitWeakMessage>(msg).CheckTypeAndMergeFrom(
        other);
  }
