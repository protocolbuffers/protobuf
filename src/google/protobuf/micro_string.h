...
//            This representation holds the size in the first byte (left shifted
//            to allow for tags) and the rest of the bytes are the data.
...
class PROTOBUF_EXPORT MicroString {
 ...
 protected:
  ...
  bool is_inline() const {
    return kHasInlineRep && !is_micro_rep() && !is_large_rep();
  }
...
class MicroStringExtraImpl : private MicroString {
  static_assert(N % 8 == 0, "Avoid padding");

 public:
  static constexpr size_t kInlineCapacity =
      kHasInlineRep ? N + MicroString::kInlineCapacity : 0;
...
using MicroStringExtra =
    std::conditional_t<N == 0 || !MicroString::kHasInlineRep, MicroString,
