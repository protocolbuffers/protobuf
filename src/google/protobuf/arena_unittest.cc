...
TEST(ArenaTest, SpaceReusePoisonsAndUnpoisonsMemory) {
  ...
  for (Arena* arena : {static_cast<Arena*>(nullptr), &arena_obj}) {
    for (size_t bytes = kMinBytes; bytes <= kMaxBytes;
         (bytes < 10) ? (bytes += 1) : (bytes += 10)) {
      SCOPED_TRACE(
          absl::Substitute("arena=$0 bytes=$1 align=$2", arena, bytes, kAlign));
      char* const arr = Arena::CreateArray<char>(arena, bytes);
      EXPECT_EQ(arr != nullptr, bytes != 0);
      EXPECT_EQ(reinterpret_cast<uintptr_t>(arr) % kAlign, 0)
          << "arr=" << (void*)arr;
...
#endif  // PROTOBUF_ASAN || PROTOBUF_MSAN
      if (arena == nullptr) {
        recycle_bin.emplace_back(arr);
      }
    }
  }
