...
TextFormat::Parser::UnsetFieldsMetadata::ExplicitlyUnsetProtoFieldAddress::
    ...
    : field_pointer(&reflection.GetRaw<char>(message, &fd)) {}
...
#define SET_FIELD(CPPTYPE, CPPTYPELCASE, VALUE)                   \
  if (field->is_repeated()) {                                     \
    ...
    if (no_op_fields_ && !field->has_presence() &&                \
      ...
      no_op_fields_->addresses.insert(                            \
          UnsetFieldsMetadata::ExplicitlyUnsetProtoFieldAddress(  \
