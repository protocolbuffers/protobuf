...
#undef SET_FIELD
  ...
  bool ConsumeAnyTypeUrlOrFullTypeName(std::string& full_type_name,
    ...
    std::string text;
    absl::optional<size_t> last_slash_pos;
    ...
    if (!last_slash_pos) {
      ...
      for (size_t i = 0; i < url_prefix.size(); ++i) {
        if (url_prefix[i] == '%' &&
            (i + 2 >= url_prefix.size() || !IsHexCharacter(url_prefix[i + 1]) ||
             !IsHexCharacter(url_prefix[i + 2]))) {
  ...
  bool TryConsumeUrlChar(char* url_char) {
    static constexpr absl::CharSet kUrlChars =
