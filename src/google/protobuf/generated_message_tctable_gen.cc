...
TailCallTableInfo::TailCallTableInfo(
  ...
  GeneratedMessageFastParseTableBuilder builder(message_options, field_entries,
                                                ordered_fields);
  auto fast_fields = builder.Build(builder.GetEndGroupTag(descriptor));
...
bool GeneratedMessageFastParseTableBuilder::IsFieldEligibleForFastParsing(
  ...
  switch (field->type()) {
    ...
    case FieldDescriptor::TYPE_ENUM: {
      uint8_t rmax_value;
      if (!message_options_.uses_codegen &&
          GetEnumRangeInfo(field, rmax_value) == EnumRangeInfo::kNone) {
        // We can't use fast parsing for these entries because we can't
