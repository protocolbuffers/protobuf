...
class KeyMapBase : public UntypedMapBase {
 ...
 protected:
  ...
  static size_type CalculateCapacityForSize(size_type size) {
    size_t capacity = absl::bit_ceil(size);
    if (size > CalculateHiCutoff(capacity) && capacity < max_size() / 2) {
      capacity *= 2;
    }
  ...
  // Merge the linked list at `head`, assuming `this->empty()` and that there
