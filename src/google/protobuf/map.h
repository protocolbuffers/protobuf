...
struct VariantKey {
  ...
  template <typename H>
  friend H AbslHashValue(H state, const VariantKey& variant_key) {
    return variant_key.data == nullptr
               ? H::combine(std::move(state), variant_key.integral)
               : H::combine(std::move(state),
                            absl::string_view(
                                variant_key.data,
                                static_cast<size_t>(variant_key.integral)));
  }
};
