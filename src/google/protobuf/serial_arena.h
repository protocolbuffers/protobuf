...
class PROTOBUF_EXPORT SerialArena {
 ...
 private:
  ...
  PROTOBUF_ALWAYS_INLINE
  static void MaybePrefetchImpl(const size_t prefetch_degree, const char* next,
                                const char* limit, const char*& prefetch_ptr) {
    if (PROTOBUF_PREDICT_TRUE(prefetch_ptr - next > prefetch_degree)) return;
    if (PROTOBUF_PREDICT_TRUE(prefetch_ptr < limit)) {
      const char* local_prefetch_ptr = std::max(next, prefetch_ptr);
      ABSL_DCHECK(local_prefetch_ptr != nullptr);
      const char* end = std::min(limit, local_prefetch_ptr + prefetch_degree);
      for (; local_prefetch_ptr < end;
           local_prefetch_ptr += ABSL_CACHELINE_SIZE) {
        absl::PrefetchToLocalCacheForWrite(local_prefetch_ptr);
      }
      prefetch_ptr = local_prefetch_ptr;
    }
  }
  PROTOBUF_ALWAYS_INLINE
  ...
  void MaybePrefetchCleanup() {
    ABSL_DCHECK(static_cast<const void*>(cleanup_list_.prefetch_ptr_) ==
                ...
                static_cast<const void*>(cleanup_list_.prefetch_ptr_) >=
                    cleanup_list_.head_);
    MaybePrefetchImpl(kPrefetchCleanupDegree,
                      reinterpret_cast<char*>(cleanup_list_.next_),
