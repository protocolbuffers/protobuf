...
class PROTOBUF_EXPORT SerialArena {
 public:
  ...
  void CleanupList(size_t* space_allocated) {
    cleanup_list_.Cleanup(*this, space_allocated);
  }
 ...
 private:
  ...
  // up to `limit_`, if `next` is within kPrefetchForwardsDegree bytes
  // of `prefetch_ptr_`.
  PROTOBUF_ALWAYS_INLINE
  void MaybePrefetchForwards(const char* next) {
    ...
    if (PROTOBUF_PREDICT_TRUE(prefetch_ptr_ - next > kPrefetchForwardsDegree))
      return;
    if (PROTOBUF_PREDICT_TRUE(prefetch_ptr_ < limit_)) {
      const char* prefetch_ptr = std::max(next, prefetch_ptr_);
      ABSL_DCHECK(prefetch_ptr != nullptr);
      const char* end =
          std::min<const char*>(limit_, prefetch_ptr + kPrefetchForwardsDegree);
