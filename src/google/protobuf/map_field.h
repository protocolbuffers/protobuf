...
inline MapKey::MapKey(MapKeyConstRef other) : type_() { CopyFrom(other); }
...
class PROTOBUF_EXPORT MapIterator {
 public:
  MapIterator(Message* message, const FieldDescriptor* field) {
    ...
    key_ref_.SetType(field->message_type()->map_key()->cpp_type());
  ...
  ABSL_DEPRECATED("Use GetKeyRef() instead.")
  const MapKey& GetKey() ABSL_ATTRIBUTE_LIFETIME_BOUND { return key_; }
 ...
 private:
  ...
  MapIterator(internal::MapFieldBase* map, const Descriptor* descriptor) {
    ...
    key_ref_.SetType(descriptor->map_key()->cpp_type());
