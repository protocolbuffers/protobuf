...
uint32_t FastParseTableBuilderBase::RecodeTagForFastParsing(uint32_t tag) {
...
uint32_t FastParseTableBuilderBase::GetRecodedTagForFastParsing(
    const FieldDescriptor* field) {
  return RecodeTagForFastParsing(internal::WireFormat::MakeTag(field));
}

absl::optional<uint32_t> FastParseTableBuilderBase::GetEndGroupTag(
...
uint32_t FastParseTableBuilderBase::TagToIdx(uint32_t tag,
