...
class PROTOBUF_EXPORT Tokenizer {
 public:
  ...
  enum TokenType {
    ...
    TYPE_CHARACTER,   // Any printable, non-whitespace character. Character
                      // tokens are always a single character and are only
                      // produced if single-character mode is enabled (see
                      // set_single_character_mode()).
  };
  ...
  // "25 foo7 +x" will be tokenized as 2, 5, f, o, o, 7 +, x (if whitespace is
