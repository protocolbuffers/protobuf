...
class MessageCreator {
 public:
  ...
  static constexpr MessageCreator Memcpy(uint32_t allocation_size,
                                         uintptr_t arena_bits = 0) {
    MessageCreator out;
    out.allocation_size_ = allocation_size;
    out.tag_ = kMemcpy;
  ...
  MessageLite* New(const MessageLite* prototype, Arena* arena) const;

  template <bool test_call = false, typename MessageLite>
  MessageLite* PlacementNew(const MessageLite* prototype, void* mem,
                            Arena* arena) const;
...
class PROTOBUF_EXPORT MessageLite {
 ...
 protected:
  ...
  struct ClassData {
    ...
    MessageLite* New(Arena* arena) const {
      return message_creator.New(prototype, arena);
    }

    MessageLite* PlacementNew(void* mem, Arena* arena) const {
      return message_creator.PlacementNew(prototype, mem, arena);
    }
...
PROTOBUF_ALWAYS_INLINE inline MessageLite* MessageCreator::PlacementNew(
    const MessageLite* prototype, void* mem, Arena* arena) const {
  ...
  const char* src = reinterpret_cast<const char*>(prototype);
...
PROTOBUF_ALWAYS_INLINE inline MessageLite* MessageCreator::New(
    const MessageLite* prototype, Arena* arena) const {
  return PlacementNew<test_call>(prototype,
                                 arena != nullptr
