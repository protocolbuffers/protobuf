...
void* RepeatedPtrFieldBase::AddOutOfLineHelper(ElementFactory factory) {
  if (tagged_rep_or_elem_ == nullptr) {
    current_size_ = 1;
    return tagged_rep_or_elem_ = factory(GetOwningArena());
  }
  if (using_sso()) {
  ...
  if (PROTOBUF_PREDICT_TRUE(current_size_ != total_size_)) {
    Rep* r = rep();
    if (current_size_ == r->allocated_size) {
      ++r->allocated_size;
      return r->elements[current_size_++] = factory(GetOwningArena());
    }
