...
// This exercises the corner case of swapping from a field with one element
// to a field with no elements: the destination field becomes an SOO field in
// the process.
TEST(RepeatedPtrFieldTest, SwapFromOneElemFieldToEmptyField) {
  Arena arena;
  SourceCodeInfo from;
  auto* loc = from.add_location();
  loc->add_path(3);
  loc->add_leading_detached_comments("Something");
  SourceCodeInfo* to = from.New(&arena);
  to->Swap(&from);
}
...
TEST(RepeatedPtrFieldTest, UnsafeArenaAddAllocatedReleaseLastOnBaseField) {
  ...
  ElemT* concrete_new_elem = dynamic_cast<ElemT*>(base_new_elem);
  ...
  ElemT* concrete_new_elem_roundtrip =
      dynamic_cast<ElemT*>(base_new_elem_roundtrip);
