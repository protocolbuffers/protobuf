...
// factors of two. Trivially destructible, but Cleanup() must be called before
// destruction.
class ChunkList {
 public:
  ...
  // before destruction.
  void Cleanup(const SerialArena& arena, size_t* space_allocated);

  // Returns the pointers to the to-be-cleaned objects. For testing.
  std::vector<void*> Peek();
 ...
 private:
  ...
  ABSL_ATTRIBUTE_ALWAYS_INLINE void AddFromExisting(void* elem,
                                                    void (*destructor)(void*)) {
    new (next_++) CleanupNode{elem, destructor};
  }
