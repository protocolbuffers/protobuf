...
class FieldMaskUtil:
  ...
  def MergeMessageTo(
    ...
    # Ensure the returned type is bytes, as expected from the C++ aux function
    if not isinstance(merged_bytes, bytes):
      raise TypeError(
          f"Expected bytes from CLIF call, got {type(merged_bytes)}"
      )
    return source.__class__().FromString(merged_bytes)
