...
MessageDifferencer::FieldDescriptorArray MessageDifferencer::RetrieveFields(
  ...
  static thread_local auto* tl_fields =
  ...
  if (descriptor->options().map_entry()) {
    if (this->scope_ == PARTIAL && base_message) {
      tl_fields->clear();
      reflection->ListFields(message, tl_fields);
      message_fields.assign(tl_fields->begin(), tl_fields->end());
    } else {
...
bool MessageDifferencer::CompareRepeatedRep(
  ...
  const bool already_matched_by_set = reporter_ == nullptr && smart_list &&
                                      !(count1 == 1 && count2 == 1) &&
...
bool MessageDifferencer::MatchRepeatedFieldIndices(
  ...
  if (reporter != nullptr) reporter_ = nullptr;
  ...
  if (reporter != nullptr) reporter_ = reporter;
