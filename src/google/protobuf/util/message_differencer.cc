    ...
    if (path_index == static_cast<int64_t>(key_field_path.size() - 1)) {
      if (field->is_map()) {
        return message_differencer_->CompareMapField(
            message1, message2, unpacked_any, field, mutable_parent_fields);
      ...
      mutable_parent_fields->push_back(specific_field);
      const bool result = IsMatchInternal(
          reflection1->GetMessage(message1, field),
          reflection2->GetMessage(message2, field), false /*key is never Any*/,
          *mutable_parent_fields, key_field_path, path_index + 1);
      mutable_parent_fields->pop_back();
      return result;
    }
  }
...
bool MessageDifferencer::MapEntryKeyComparator::IsMatch(
  ...
  return message_differencer_->CompareFieldValueUsingParentFields(
      message1, message2, unpacked_any, key, -1, -1, mutable_parent_fields);
}
...
MessageDifferencer::FieldDescriptorArray MessageDifferencer::RetrieveFields(
  ...
  static thread_local auto* const tmp_message_fields =
      new std::vector<const FieldDescriptor*>();

  const Reflection* reflection = message.GetReflection();
  if (descriptor->options().map_entry()) {
    if (this->scope_ == PARTIAL && base_message) {
      tmp_message_fields->clear();
...
bool MessageDifferencer::IsMatch(
  ...
  if (repeated_field->cpp_type() != FieldDescriptor::CPPTYPE_MESSAGE) {
    FieldContext field_context(mutable_parent_fields);
  ...
  if (key_comparator == nullptr) {
    ...
    mutable_parent_fields->push_back(specific_field);
    match = key_comparator->IsMatch(m1, m2, false, *mutable_parent_fields);
    mutable_parent_fields->pop_back();
  }
...
bool MessageDifferencer::CompareRepeatedRep(
  ...
  const bool already_matched_by_set =
      ...
      key_comparator == nullptr && IsTreatedAsSet(repeated_field) &&
...
bool MessageDifferencer::CompareFieldValueUsingParentFields(
  ...
  if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE &&
    ...
    if (parent_fields != nullptr &&
        (reporter_ != nullptr || !ignore_criteria_.empty() ||
         field_comparator_kind_ != kFCDefault ||
         field_comparator_.default_impl != &default_field_comparator_ ||
         !map_field_key_comparator_.empty())) {
      // Append currently compared field to the end of parent_fields.
      SpecificField& specific_field = PushSpecificField(parent_fields);
      specific_field.message1 = &message1;
      specific_field.message2 = &message2;
    ...
    } else if (parent_fields != nullptr) {
      return Compare(m1, m2, false, parent_fields);
    } else {
...
class MaximumMatcher {
 ...
 private:
  ...
  absl::InlinedVector<int, 64>* match_list1_;
  absl::InlinedVector<int, 64>* match_list2_;
};
...
bool MessageDifferencer::MatchRepeatedFieldIndices(
  ...
  if (scope_ == PARTIAL) {
    ...
    if (has_diagonal_pass && reporter == nullptr && !is_smart_list &&
      ...
      for (int i = diag + 1; i < count1 && i < count2; ++i) {
        if (IsMatch(repeated_field, key_comparator, &message1, &message2,
      ...
      if (has_diagonal_pass) {
        start_offset = is_smart_list ? std::min(count1, count2) : 0;
        for (int i = 0; i < count1 && i < count2; i++) {
          if (IsMatch(repeated_field, key_comparator, &message1, &message2,
            ...
            if (!is_smart_list && i == start_offset) {
              start_offset = i + 1;
            }
          } else if (is_smart_list) {
            start_offset = i;
            break;
          }
        }
    ...
    for (int i = start_offset; i < count1; ++i) {
      ...
      for (int j = start_offset; j < count2; j++) {
        if (has_diagonal_pass && !is_smart_list && !is_treated_as_smart_set &&
            i == j) {
          continue;
        }
        if ((*match_list2)[j] != -1) {
  ...
  if (IsTreatedAsSmartList(repeated_field)) {
    std::vector<int> vec1(match_list1->begin(), match_list1->end());
    std::vector<int> vec2(match_list2->begin(), match_list2->end());
    match_indices_for_smart_list_callback_(&vec1, &vec2);
    match_list1->assign(vec1.begin(), vec1.end());
    match_list2->assign(vec2.begin(), vec2.end());
  }
