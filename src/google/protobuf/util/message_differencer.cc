...
MessageDifferencer::MessageDifferencer()
  ...
  tmp_message_fields_.reserve(64);
}
...
MessageDifferencer::FieldDescriptorArray MessageDifferencer::RetrieveFields(
  ...
  if (descriptor->options().map_entry()) {
    if (this->scope_ == PARTIAL && base_message) {
      ...
      message_fields.assign(tmp_message_fields_.begin(),
...
bool MessageDifferencer::CompareRepeatedRep(
  ...
  const bool already_matched_by_set =
      ...
      ((repeated_field->cpp_type() != FieldDescriptor::CPPTYPE_MESSAGE &&
       ...
       reporter_ == nullptr ||
       (!report_matches_ && !report_moves_ && !report_ignores_ &&
        is_default_comparator));
...
bool MessageDifferencer::MatchRepeatedFieldIndices(
  ...
  if (scope_ == PARTIAL) {
    ...
    const bool fast_diagonal_pass = has_diagonal_pass && reporter == nullptr &&
                                    !is_smart_list && !is_treated_as_smart_set;
    ...
    for (int i = start_offset; i < count1; ++i) {
      ...
      for (int j = start_offset; j < count2; j++) {
        if (fast_diagonal_pass ? (i == start_offset && j == start_offset)
                               : (has_diagonal_pass && !is_smart_list &&
                                  !is_treated_as_smart_set && i == j)) {
          continue;
        }
        if ((*match_list2)[j] != -1) {
