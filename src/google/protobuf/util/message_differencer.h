...
class PROTOBUF_EXPORT MessageDifferencer {
 public:
  ...
  // If set, the addresses of fields in message1 that equal the addresses passed
  // here will be treated as required for comparison, even if they are absent.
  // Addresses should have valid pointers into message1 (though technically  no
  // UB will occur if they aren't, as the pointers are not dereferenced).
  void set_require_no_presence_addresses(
      const google::protobuf::TextFormat::Parser::UnsetFieldsMetadata& addresses) {
    require_no_presence_addresses_ = addresses;
  }
 ...
 private:
  ...
  google::protobuf::TextFormat::Parser::UnsetFieldsMetadata
      require_no_presence_addresses_;
