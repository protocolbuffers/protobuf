...
// Converts from protobuf message to JSON and appends it to |json_output|. This is a
// simple wrapper of BinaryToJsonStream(). It will use the DescriptorPool of the
// passed-in message to resolve Any types.
//
// Please note that non-OK statuses are not a stable output of this API and
// subject to change without notice.
PROTOBUF_EXPORT absl::Status MessageToJson(const Message& message, 
                                           io::ZeroCopyOutputStream* json_output,
                                           const PrintOptions& options);

inline absl::Status MessageToJson(const Message& message, 
                                  io::ZeroCopyOutputStream* json_output) {
  return MessageToJson(message, json_output, PrintOptions());
}
