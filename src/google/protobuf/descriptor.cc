      ...
      if (!insert_num_result.second) {
        ...
        auto get_custom_string = [](const EnumValueDescriptor* ev)
          ...
          if (ev->options().HasExtension(pb::enumvalue::json)) {
            return ev->options().GetExtension(pb::enumvalue::json).string();
          }
          return absl::nullopt;
        };
                   ...
                   [&]() -> std::string {
                     ...
                     std::string current_str =
                         current_custom.has_value()
                             ...
                             : "unset";
                     return absl::StrCat(
                         "Alias values for number ", value->number(),
                         " must have the same custom JSON string. \"",
                         value->name(), "\" has ", current_str,
                         " but primary alias \"", primary->name(), "\" has ",
                         primary_str, ".");
                   });
        }
      }
      ...
      if (!insert_result.second) {
        ...
        if (conflict != value && conflict->number() != value->number()) {
          ...
          AddError(result->full_name(), proto.value(i),
                   ...
                   [&]() -> std::string {
                     return absl::StrCat(
                         current_type == "custom" ? "Custom" : "Default",
                         " JSON string \"", name, "\" conflicts with ",
                         conflict_type, " JSON string of \"", conflict->name(),
                         "\".");
                   });
        }
      }
    ...
    if (absl::SimpleAtoi(custom_string, &parsed_val)) {
      if (parsed_val != enum_value->number()) {
        AddError(enum_value->full_name(), proto,
                 ...
                 [&]() -> std::string {
                   return absl::StrCat(
                       "Custom JSON string \"", custom_string,
                       "\" parses as integer ", parsed_val,
                       ", which does not match the enum value's number ",
                       enum_value->number(), ".");
                 });
      }
    }
