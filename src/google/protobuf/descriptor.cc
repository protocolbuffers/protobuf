...
template <typename DescriptorT>
const std::string& GetFullName(const DescriptorT& desc) {
  return desc.full_name();
}

const std::string& GetFullName(const FileDescriptor& desc) {
  return desc.name();
}
...
class DescriptorPool::DeferredValidation {
 public:
  ...
  FileDescriptorProto& CreateProto() {
    owned_protos_.emplace_back(std::make_unique<FileDescriptorProto>());
  ...
  bool Validate() {
    ...
    const Descriptor* feature_set =
        pool_->FindMessageTypeByName("google.protobuf.FeatureSet");
 ...
 private:
  ...
  absl::flat_hash_map<const FileDescriptor*, std::vector<LifetimesInfo>>
      lifetimes_info_map_;
  std::vector<std::unique_ptr<FileDescriptorProto>> owned_protos_;
};
