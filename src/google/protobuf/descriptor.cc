...
static void ReportCycleErrors(
    absl::Span<const FileDescriptorProto* const> cycle,
  ...
  std::string error_message = absl::StrCat(
      ...
      absl::StrJoin(cycle, " -> ",
                    [](std::string* out, const FileDescriptorProto* proto) {
                      absl::StrAppend(out, proto->name());
                    }));
  ...
  if (error_collector != nullptr) {
    ...
    const FileDescriptorProto* importer_proto = cycle[0];

    error_collector->RecordError(importer, imported, importer_proto,
    ...
    for (int i = static_cast<int>(cycle.size()) - 2; i >= 0; --i) {
      ...
      const FileDescriptorProto* edge_importer_proto = cycle[i];
      error_collector->RecordError(
          edge_importer, edge_imported, edge_importer_proto,
          DescriptorPool::ErrorCollector::IMPORT,
  ...
  for (const auto* proto : cycle) {
    known_bad_files.emplace(proto->name());
  }
}
...
DescriptorPool::CollectAndSortDependencies(absl::string_view name,
  ...
  std::vector<StackFrame> stack = {{name, false}};
  std::vector<const FileDescriptorProto*> current_path;
  bool has_cycle = false;
  std::vector<const FileDescriptorProto*> sorted_protos;

  while (!stack.empty()) {
    StackFrame frame = stack.back();
    stack.pop_back();

    auto& node = nodes[frame.name];

    if (frame.already_visited_children) {
      node.state = State::kVisited;
      if (node.proto != nullptr) {
        sorted_protos.push_back(node.proto);
      }
      current_path.pop_back();
      continue;
    }

    if (tables_->FindFile(frame.name) != nullptr ||
        (underlay_ != nullptr &&
         underlay_->FindFileByName(frame.name) != nullptr)) {
      continue;
    }
    if (tables_->known_bad_files_.contains(frame.name)) {
      continue;
    }
    ...
    if (node.state == State::kVisiting) {
      if (!has_cycle) {
        has_cycle = true;
        auto it =
            absl::c_find_if(current_path, [&](const FileDescriptorProto* p) {
              return p->name() == frame.name;
            });
        std::vector<const FileDescriptorProto*> cycle(it, current_path.end());
        cycle.push_back(*it);
        ReportCycleErrors(cycle, build_session.error_collector(),
                          tables_->known_bad_files_);
        for (const auto* cycle_node_proto : cycle) {
          nodes[cycle_node_proto->name()].proto = nullptr;
        }
      }
      continue;
    }
    ...
    if (!fallback_database_->FindFileByName(frame.name, proto)) {
      tables_->known_bad_files_.emplace(frame.name);
      node.proto = nullptr;
      node.state = State::kVisited;
      if (frame.name == name) return absl::nullopt;
      continue;
    }
    node.proto = proto;
    node.state = State::kVisiting;
    current_path.push_back(proto);
    stack.push_back({frame.name, true});

    // Push the dependencies in reverse order so they are processed in the same
    // order as they would be in a recursive DFS. This preserves the error
    // reporting order.
    for (int i = proto->option_dependency_size() - 1; i >= 0; --i) {
      stack.push_back({proto->option_dependency(i), false});
    }
    for (int i = proto->dependency_size() - 1; i >= 0; --i) {
      stack.push_back({proto->dependency(i), false});
    }
  }

  if (has_cycle) return absl::nullopt;
...
bool DescriptorPool::TryFindFileInFallbackDatabase(
  ...
  auto sorted_protos = CollectAndSortDependencies(name, build_session);
  if (!sorted_protos.has_value()) {
    return false;
  }

  bool success = true;
  for (const auto* proto : *sorted_protos) {
    if (BuildFileFromDatabase(*proto, build_session) == nullptr) {
      tables_->known_bad_files_.emplace(proto->name());
      success = false;
    }
  }
