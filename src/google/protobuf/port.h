...
PROTOBUF_ALWAYS_INLINE void PrefetchLine(const void* ptr, size_t line) {
  ...
  // In debug builds, guard against potential UB (ptr + offset overflow).This is
  // a best-effort check to catch obvious mistakes (e.g. a huge offsets or
  // num/unit mismatches). NOTE: Using `assert()`, not CHECKs, for lean
  // implementation.
#if !defined(NDEBUG)
  static constexpr uintptr_t kMaxPtr = std::numeric_limits<uintptr_t>::max();
  assert(kMaxPtr - offset >= reinterpret_cast<uintptr_t>(ptr));
#endif
  // However, overflows don't matter for prefetching, because the prefetch
  // instruction is just a no-op for invalid addresses. To formally avoid UB,
  // we perform the arithmetic in uintptr_t.
  const auto* prefetch_ptr =
