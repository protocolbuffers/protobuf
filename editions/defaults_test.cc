...
#include "editions/defaults_test_embedded_uint8_list.h"
#include "editions/defaults_test_embedded_xxd_i.h"
...
TEST(DefaultsTest, EmbeddedUInt8List) {
  FeatureSetDefaults defaults;
  ASSERT_TRUE(defaults.ParseFromArray(kDefaultTestEmbeddedUInt8List.data(),
                                      kDefaultTestEmbeddedUInt8List.size()))
      << "Could not parse embedded data";
  ASSERT_EQ(defaults.defaults().size(), 3);
  ASSERT_EQ(defaults.minimum_edition(), EDITION_2023);
  ASSERT_EQ(defaults.maximum_edition(), EDITION_2023);
...
TEST(DefaultsTest, EmbeddedXXD_I) {
  FeatureSetDefaults defaults;
  ASSERT_TRUE(defaults.ParseFromArray(kDefaultTestEmbeddedXXD_I.data(),
                                      kDefaultTestEmbeddedXXD_I.size()))
      << "Could not parse embedded data";
  ASSERT_EQ(defaults.defaults().size(), 3);
  ASSERT_EQ(defaults.minimum_edition(), EDITION_2023);
  ASSERT_EQ(defaults.maximum_edition(), EDITION_2023);
