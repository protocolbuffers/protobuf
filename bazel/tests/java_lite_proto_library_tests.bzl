...
def java_lite_proto_library_test_suite(name):
    ...
    test_suite(
        ...
        tests = [
            ...
            _test_same_version_compiler_arguments,
        ],
    )
...
# option_deps.
def _test_lite_binary_option_deps(name):
    util.helper_target(
        proto_library,
        name = name + "_baz",
        srcs = ["baz.proto"],
    ...
    analysis_test(
        name = name,
        target = ":" + name + "_pb2",
        impl = _test_lite_binary_option_deps_impl,
        provider_subject_factories = [java_info_subject_factory],
    )

def _test_lite_binary_option_deps_impl(env, target):
    java_info = env.expect.that_target(target).provider(JavaInfo)
    java_info.transitive_source_jars_in_package().contains_exactly([
        "{package}/test_lite_binary_option_deps_foo-lite-src.jar",
    ])
    java_info.transitive_source_jars().contains(
        "third_party/java_src/protobuf/current/java/com/google/protobuf/libprotobuf_lite-src.jar",
    )
    java_info.transitive_compile_time_jars_in_package().contains_exactly([
        "{package}/libtest_lite_binary_option_deps_foo-lite-hjar.jar",
    ])
    java_info.transitive_compile_time_jars().contains(
        "third_party/java_src/protobuf/current/java/com/google/protobuf/libprotobuf_lite-hjar.jar",
    )
...
def _test_lite_proto_library_builds_compiled_jar_impl(env, target):
    env.expect.that_target(target).default_outputs().contains(
        "{package}/libtest_lite_proto_library_builds_compiled_jar_foo-lite.jar",
    )
...
def _test_lite_java_proto2_compiler_args_impl(env, target):
    ...
    gen_proto.argv().contains_at_least([
        "--java_out=lite,immutable:{bindir}/{package}/test_lite_java_proto2_compiler_args_protolib-lite-src.jar",
        "-I.",
        "{package}/file.proto",
    ]).in_order()
...
def _test_lite_command_line_contains_target_label_impl(env, target):
    ...
    action.argv().contains_at_least([
        "--target_label",
        "//{package}:test_lite_command_line_contains_target_label_proto",
        "--injecting_rule_kind",
        "java_lite_proto_library",
    ])

def _test_same_version_compiler_arguments(name):
    util.helper_target(
        proto_library,
        name = name + "_protolib",
    )
    ...
    analysis_test(
        ...
        testing_aspect = _java_lite_proto_aspect_testing_aspect,
        impl = _test_same_version_compiler_arguments_impl,
        provider_subject_factories = [java_info_subject_factory],
    )

def _test_same_version_compiler_arguments_impl(env, target):
    proto_library = env.expect.that_target(target)
    gen_proto = proto_library.action_named("GenProto")
    gen_proto.argv().contains("{package}/file.proto")
    gen_proto.argv().contains_at_least([
        "--java_out=lite,immutable:{bindir}/{package}/test_same_version_compiler_arguments_protolib2-lite-src.jar",
        "-I.",
        "{package}/file.proto",
    ]).in_order()
    java_info = env.expect.that_target(target).provider(JavaInfo)
    java_info.transitive_runtime_jars().contains_at_least([
        "{package}/libtest_same_version_compiler_arguments_protolib2-lite.jar",
