...
def java_mutable_proto_library_test_suite(name):
    ...
    test_suite(
        ...
        tests = [
            ...
            _test_java_library_depends_on_java_mutable_proto_library,
        ],
    )
...
def _test_java_library_depends_on_java_mutable_proto_library(name):
    util.helper_target(
        proto_library,
        name = name + "_proto",
    )
    ...
    util.helper_target(
        ...
        name = name + "_java_lib",
        srcs = ["A.java"],
        deps = [":" + name + "_mutable_proto"],
    )
    analysis_test(
        name = name,
        target = ":" + name + "_java_lib",
        testing_aspect = _java_mutable_proto_aspect_testing_aspect,
        impl = _test_java_library_depends_on_java_mutable_proto_library_impl,
    )

def _test_java_library_depends_on_java_mutable_proto_library_impl(env, target):
    proto_library = env.expect.that_target(target)
    gen_proto = proto_library.action_named("Javac")
    gen_proto.argv().contains("//{package}:test_java_library_depends_on_java_mutable_proto_library_java_lib")
