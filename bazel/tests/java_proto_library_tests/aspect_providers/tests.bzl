"""Tests that java_proto_library aspect exposes a JavaInfo provider with the correct output jars."""
...
def _test_java_proto_library_aspect_providers(env, target):
    jars = target[JarInfo].jars.to_list()
    env.expect.that_collection(jars).has_size(1)
    java_output = jars[0]
    env.expect.that_str(java_output.class_jar.basename).equals("libfoo_proto-speed.jar")
...
TESTS = {
    ":foo_rule": [_test_java_proto_library_aspect_providers],
}
