"""Tests that a java_proto_library only provides direct jars corresponding on the
...
def _test_bar_correctly_defines_direct_jars(env, target):
    ...
    java_info.source_jars().contains_exactly([
        "{package}/bar_proto-speed-src.jar",
    ])

TESTS = {
    ":foo_java_proto": [_test_foo_correctly_defines_direct_jars],
    ":bar_java_proto": [_test_bar_correctly_defines_direct_jars],
}
