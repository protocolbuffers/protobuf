...
def _compile(
    ...
    # We use three separate args.add_all calls on proto_info.transitive_sources
    # to append the -I flags in strict protoc precedence order (virtual → external → main output)
    # without flattening the depset at analysis time or filtering twice.
