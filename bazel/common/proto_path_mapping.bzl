...
def import_external_repo_from_file(file):
    ...
    if root_path and root_path != ".":
        # Check if file.path has external repo prefix relative to root_path.
        # In Bazel, file.root.path is e.g. 'bazel-out/k8-fastbuild/bin', and
        # file.path is 'bazel-out/k8-fastbuild/bin/external/foo/bar.proto'.
        rel = file.path
        if rel.startswith(root_path + "/"):
            rel = rel[len(root_path) + 1:]
        if rel.startswith("external/") or rel.startswith("../"):
            parts = rel.split("/", 2)
            return "-I%s/%s/%s" % (root_path, parts[0], parts[1])
        if "external/" in root_path:
        ...
        if path.startswith("external/") or path.startswith("../"):
            ...
            # with 'external/repo_name/...' or '../repo_name/...'. We extract the first two
            # components as the import root, ignoring package subdirectories and filenames.
            parts = path.split("/", 2)
...
def output_directory_from_file(file):
    ...
    # Check if the generated file is in an external repository.
    root_path = file.root.path
    if root_path and root_path != ".":
        rel = path
        if rel.startswith(root_path + "/"):
        ...
        if rel.startswith("external/") or rel.startswith("../"):
            parts = rel.split("/", 2)
            return "%s/%s/%s" % (root_path, parts[0], parts[1])
