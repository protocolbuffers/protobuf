...
def bucket_transitive_sources(transitive_sources):
    """Sorts transitive_sources into protoc precedence order.

    Protoc requires include paths in order from most specific to least specific:
    1. Virtual imports (_virtual_imports/<target>)
    2. External repository roots (generated or source)
    3. Main workspace output root (bazel-out/.../bin)

    Args:
      transitive_sources: depset[File] of proto sources.

    Returns:
      A list of File objects ordered by protoc precedence.
    """
    virt = []
    ext = []
    main_out = []
    ...
    for f in transitive_sources.to_list():
        ...
        if "_virtual_imports/" in path:
            virt.append(f)
        elif "external/" in f.root.path or path.startswith("external/") or path.startswith("../"):
            ext.append(f)
        elif f.root.path and f.root.path != ".":
            main_out.append(f)

    return virt + ext + main_out

def import_proto_path_from_file(file):
    """Single map_each callback that returns -I<path> for any proto File object.

    Args:
      file: A File object from proto_info.transitive_sources.

    Returns:
      A string of the form '-I<path>' for the proto import root, or
      None if no flag is required.
    """
    res = import_virtual_from_file(file)
    if res != None:
        return res
    res = import_external_repo_from_file(file)
    if res != None:
        return res
    return import_main_output_from_file(file)
...
def import_main_output_from_file(file):
    ...
    if "external/" in root_path or "_virtual_imports/" in file.path:
        return None
...
proto_path_mapping = struct(
    bucket_transitive_sources = bucket_transitive_sources,
    import_proto_path_from_file = import_proto_path_from_file,
