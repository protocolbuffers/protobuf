...
_output_directory_from_file = proto_path_mapping.output_directory_from_file
...
def _import_virtual_from_file_test(ctx):
    ...
    # Virtual imports path with no trailing slash/filename after name → None.
    asserts.equals(
        env,
        None,
        _import_virtual(_make_file(
...
def _import_external_repo_from_file_test(ctx):
    ...
    # 'bazel-out/k8-fastbuild/bin/external/foo'
    # Generated file in external repo (Bazel-style where root.path is bin root).
    ext_gen_bazel = _make_file(
    ...
    asserts.equals(
        ...
        _import_external(ext_gen_bazel),
    )

    # Generated file in external repo (mock where root.path includes external/foo).
    ext_gen = _make_file(
        _ROOT_PATH + "/external/foo/bar.proto",
    ...
    # Generated file in external repo with sibling layout ('../foo').
    ext_gen_sibling = _make_file(
        _ROOT_PATH + "/../foo/bar.proto",
        _ROOT_PATH,
    )
    asserts.equals(
        env,
        "-I" + _ROOT_PATH + "/../foo",
        _import_external(ext_gen_sibling),
    )
    ...
    # 'external/foo' — source file in external repo with nested package path.
    asserts.equals(
        env,
        "-Iexternal/foo",
    ...
    asserts.equals(
        env,
        "-Iexternal/foo",
        _import_external(_make_file("external/foo/sub/pkg/bar.proto", "")),
    )

    # '../foo' — sibling layout external repo with nested package path.
    asserts.equals(
        env,
        "-I../foo",
    ...
    asserts.equals(
        env,
        "-I../foo",
        _import_external(_make_file("../foo/sub/pkg/bar.proto", "")),
    )
...
def _output_directory_from_file_test(ctx):
    """Test physical output directory path formatting from File objects.

    Tests all example forms:
      'bazel-out/k8-fastbuild/bin'
      'bazel-out/k8-fastbuild/bin/external/foo'
      'bazel-out/k8-fastbuild/bin/_virtual_imports/foo'
      'bazel-out/k8-fastbuild/bin/external/foo/e/_virtual_imports/e'
    """
    env = unittest.begin(ctx)

    # Standard generated output file in main workspace.
    # 'bazel-out/k8-fastbuild/bin'
    gen_file = _make_file(
        _ROOT_PATH + "/cloud/foo/bar_pb2.py",
        _ROOT_PATH,
    )
    asserts.equals(env, _ROOT_PATH, _output_directory_from_file(gen_file))

    # Generated output file in external repo (Bazel-style where root.path is bin root).
    # 'bazel-out/k8-fastbuild/bin/external/foo'
    ext_gen_bazel = _make_file(
        _ROOT_PATH + "/external/foo/cloud/bar_pb2.py",
        _ROOT_PATH,
    )
    asserts.equals(
        env,
        _ROOT_PATH + "/external/foo",
        _output_directory_from_file(ext_gen_bazel),
    )

    # Generated output file in external repo with sibling layout ('../foo').
    # 'bazel-out/k8-fastbuild/bin/../foo'
    ext_gen_sibling = _make_file(
        _ROOT_PATH + "/../foo/cloud/bar_pb2.py",
        _ROOT_PATH,
    )
    asserts.equals(
        env,
        _ROOT_PATH + "/../foo",
        _output_directory_from_file(ext_gen_sibling),
    )

    # Generated output file in external repo (mock where root.path includes external/foo).
    # 'bazel-out/k8-fastbuild/bin/external/foo'
    ext_gen = _make_file(
        _ROOT_PATH + "/external/foo/cloud/bar_pb2.py",
        _ROOT_PATH + "/external/foo",
    ...
    asserts.equals(
        env,
        _ROOT_PATH + "/external/foo",
        _output_directory_from_file(ext_gen),
    )

    # Virtual import generated output file in external repo (Bazel-style).
    # 'bazel-out/k8-fastbuild/bin/external/foo/e/_virtual_imports/e'
    virtual_ext_bazel = _make_file(
        _ROOT_PATH + "/external/foo/e/_virtual_imports/e/bar_pb2.py",
        _ROOT_PATH,
    )
    asserts.equals(
        env,
        _ROOT_PATH + "/external/foo/e/_virtual_imports/e",
        _output_directory_from_file(virtual_ext_bazel),
    )

    # Virtual import generated output file in main workspace.
    # 'bazel-out/k8-fastbuild/bin/_virtual_imports/foo'
    virtual_main = _make_file(
        _ROOT_PATH + "/_virtual_imports/foo/cloud/foo/bar_pb2.py",
        _ROOT_PATH,
    )
    asserts.equals(
        env,
        _ROOT_PATH + "/_virtual_imports/foo",
        _output_directory_from_file(virtual_main),
    )

    # Virtual import generated output file in external repo.
    # 'bazel-out/k8-fastbuild/bin/external/foo/e/_virtual_imports/e'
    ...
    asserts.equals(
        env,
        _ROOT_PATH + "/external/foo/e/_virtual_imports/e",
        _output_directory_from_file(virtual_ext),
    )

    # Virtual import in alternative bin root.
    # 'bazel-out/foo/k8-fastbuild/bin/e/_virtual_imports/e'
    virtual_alt = _make_file(
        _ALT_ROOT + "/e/_virtual_imports/e/bar_pb2.py",
        _ALT_ROOT + "/e",
    )
    asserts.equals(
        env,
        _ALT_ROOT + "/e/_virtual_imports/e",
        _output_directory_from_file(virtual_alt),
    )

    # Virtual import path with no trailing slash after virtual import name → returns file.root.path.
    asserts.equals(
        env,
        _ROOT_PATH,
        _output_directory_from_file(_make_file(
            _ROOT_PATH + "/_virtual_imports/foo",
            _ROOT_PATH,
        )),
    )
...
output_directory_test = unittest.make(_output_directory_from_file_test)
