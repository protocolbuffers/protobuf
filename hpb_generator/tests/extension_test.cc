...
using ::hpb_unittest::someotherpackage::protos::string_backslash_ext;
...
TEST(CppGeneratedCode, GetExtensionStringWithDefaultAndBackslash) {
  TestModel model;
  auto res = hpb::GetExtension(&model, string_backslash_ext);
  EXPECT_TRUE(res.ok());
  EXPECT_THAT(res, IsOkAndHolds("bse:der?bse\\der"));
}
