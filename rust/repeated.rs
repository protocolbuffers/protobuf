    ...
    T: ProxiedInRepeated + ?Sized + 'msg,
{
    ...
    /// Returns a mutator for the element at `index`.
    pub fn get_mut(&mut self, index: usize) -> Option<Mut<'_, T>> {
        ...
        // - The `&mut self` prevents an aliasing mut with any index
        Some(unsafe { T::repeated_get_unchecked_mut(self.as_mut(), index) })
    }

    /// Iterates over mutators of the elements in `self`
    ///
    /// This method has the same output values but with a smaller lifetime than
    /// `into_iter()`.
...
///   `repeated_len(x)`.
/// - It must be sound to call `repeated_get_unchecked_mut` multiple times with
///   overlapping borrows so long as the index's differ.
pub unsafe trait ProxiedInRepeated: Proxied {
    ...
    /// # Safety
    /// `index` must be less than `Self::repeated_len(repeated)`
    unsafe fn repeated_get_unchecked_mut(repeated: Mut<Repeated<Self>>, index: usize) -> Mut<Self>;

    /// # Safety
    /// `index` must be less than `Self::repeated_len(repeated)`
    unsafe fn repeated_set_unchecked(repeated: Mut<Repeated<Self>>, index: usize, val: View<Self>);

    /// Copies the values in the `src` repeated field into `dst`..
    fn repeated_copy_from(src: View<Repeated<Self>>, dst: Mut<Repeated<Self>>);
}
...
impl<'msg, T> iter::Iterator for RepeatedIterMut<'msg, T>
where
    T: ProxiedInRepeated + ?Sized + 'msg,
{
    type Item = Mut<'msg, T>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // SAFETY: The iterator guarantees that the same index is not
        // accessed multipled times for `'msg`, and index is in bounds.
        let elem = unsafe {
            T::repeated_get_unchecked_mut(
                RepeatedMut { inner: self.mutator.inner, _phantom: PhantomData },
                self.current_index,
            )
        };
        self.current_index += 1;
...
impl<'msg, T> iter::IntoIterator for RepeatedMut<'msg, T>
where
    T: ProxiedInRepeated + ?Sized + 'msg,
{
    type Item = Mut<'msg, T>;
    type IntoIter = RepeatedIterMut<'msg, T>;

    fn into_iter(self) -> Self::IntoIter {
        RepeatedIterMut { mutator: self, current_index: 0 }
    }
}
        ...
        macro_rules! primitive_repeated_tests {
            ($($t:ty => [$($vals:expr),* $(,)?]),* $(,)?) => {
                ...
                $(
                    ...
                    assert_that!(
                        r.get_mut(expected_len).map(|x| x.into_view()), eq(Some(val)));
                    expected_len += 1;
                ...
                assert_that!(
                    r.iter_mut().map(|x| x.into_view()).collect::<Vec<$t>>(),
                    elements_are![$(eq($vals)),*]);
                r.get_mut(0).expect("elem 0").set(<$t as Default>::default());
                assert_that!(r.get(0).expect("elem 0"), eq(<$t as Default>::default()));

                let mut iter = r.iter_mut();
                let mut e0 = iter.next().expect("elem 0");
                let e1 = iter.next().expect("elem 1");
                e0.set(e1.get());
                assert_that!(e0.get(), eq(e1.get()));
                r.clear();
