        ...
        $(
            impl<'a> RepeatedView<'a, $t> {
                pub fn len(&self) -> usize {
                    self.inner.map(|inner| inner.len()).unwrap_or(0)
                }
...
//TODO: revisit these derives. seems like we could get multiple mutable access
// easily by cloning
#[derive(Clone, Copy)]
...
impl<'a, T> std::ops::Deref for RepeatedMut<'a, T> {
    ...
    fn deref(&self) -> &Self::Target {
        // TODO: XXX: HACK: This impl Deref pattern is copied from std's
        // Path/PathBuf impls:
        // https://doc.rust-lang.org/1.72.1/src/std/path.rs.html#2040-2042.
        // Verify this is actually sound here (note the difference that RepeatedMut
        // holds a RepeatedField, while RepeatedView holds an Option<RepeatedField>).
