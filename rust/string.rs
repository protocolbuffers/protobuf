    ...
    fn proto_str_utf8_chunks() {
        macro_rules! assert_chunks {
            ($bytes:expr, $($chunks:expr),* $(,)?) => {
                ...
                let got_chunks : Vec<_> = s.utf8_chunks().collect();
                let expected_chunks : Vec<_> = chunks.iter().copied().collect();
                assert_that!(got_chunks, eq(expected_chunks), "{bytes:?} -> {chunks:?}");
            };
        }
    ...
    fn proto_str_chars() {
        macro_rules! assert_chars {
            ($bytes:expr, [$($chars:expr),* $(,)?]) => {
                ...
                let chars = [$($chars),*];
                let s = test_proto_str(bytes);
                let mut got_chars: Vec<char> = s.chars().collect();
                assert_that!(got_chars, elements_are![$(eq($chars)),*], "{bytes:?} -> {chars:?}");
            };
        }
