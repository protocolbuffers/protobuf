...
use crate::{
    Mut, PrimitiveMut, ProxiedInRepeated, Repeated, RepeatedMut, RepeatedView, View, ViewProxy,
};
use core::fmt::Debug;
use paste::paste;
use std::alloc;
...
///
/// Contains a `upb_Array*` as well as `&Arena`, most likely that of the
/// containing message. upb requires a `RawArena` to perform mutations on
/// a repeated field.
///
/// An owned `Repeated` stores a `InnerRepeatedMut<'static>` that manages the
/// contained `RawArena`.
#[derive(Clone, Copy, Debug)]
        ...
        $(
            unsafe impl ProxiedInRepeated for $t {
                ...
                fn repeated_copy_from(src: View<Repeated<$t>>, mut dst: Mut<Repeated<$t>>) {
                    ...
                    unsafe {
                        if (!upb_Array_Resize(dst.as_raw(Private), src.len(), dst.inner.arena)) {
                            panic!("upb_Array_Resize failed.");
                        }
                        ptr::copy_nonoverlapping(
                          upb_Array_DataPtr(src.as_raw(Private)).cast::<u8>(),
                          upb_Array_MutableDataPtr(dst.as_raw(Private)).cast::<u8>(),
                          size_of::<$t>() * src.len());
                    }
                }
            }
...
impl<'a, T: ?Sized> RepeatedMut<'a, T> {
    // Returns a `RawArena` which is live for at least `'a`
    #[doc(hidden)]
...
/// Returns a static thread-local empty RepeatedView.
pub fn empty_array<T: ?Sized + ProxiedInRepeated>() -> RepeatedView<'static, T> {
    // TODO: Consider creating a static empty array in C.
    thread_local! {
        // Use `i32` for a shared empty array for all repeated types on a thread.
        static REPEATED_FIELD: RepeatedView<'static, i32> =
            Box::leak(Box::new(Repeated::new())).as_mut().into_view();
    };

    // SAFETY:
    // - Because the array is never mutated, the array type is unused.
    // - The view is leaked for `'static`.
    REPEATED_FIELD.with(|v| unsafe { RepeatedView::from_raw(Private, v.as_raw(Private)) })
}
