...
/// upb requires an Arena to perform mutations on a repeated field.
...
impl<'msg, T: ?Sized> RepeatedField<'msg, T> {
    pub fn len(&self) -> usize {
        unsafe { upb_Array_Size(self.array) }
    }
    pub fn is_empty(&self) -> bool {
        self.len() == 0
    }
    pub fn from_inner(inner: RepeatedFieldInner<'msg>) -> Self {
        Self { array: inner.raw, arena: inner.arena, _phantom: PhantomData }
    }
}
    ...
    ($(($rs_type:ty, $union_field:ident, $upb_tag:expr)),*) => {
        $(
            impl<'msg> RepeatedField<'msg, $rs_type> {
                #[allow(dead_code)]
                fn new(arena: &'msg mut Arena) -> Self {
                    Self { array: unsafe { upb_Array_New(arena.raw, $upb_tag as std::ffi::c_int) }, arena, _phantom: PhantomData }
                }
                pub fn push(&mut self, val: $rs_type) {
                    unsafe { upb_Array_Append(self.array, upb_MessageValue { $union_field: val }, self.arena.raw) }
                }
                pub fn get(&self, i: usize) -> Option<$rs_type> {
                    if i >= self.len() { None } else { unsafe { Some(upb_Array_Get(self.array, i).$union_field) } }
                }
                pub fn set(&self, i: usize, val: $rs_type) {
                    if i >= self.len() {
                        return;
                    }
                    unsafe { upb_Array_Set(self.array, i, upb_MessageValue { $union_field: val }) }
                }
            }
        )*
    }
...
mod tests {
    ...
    fn i32_array() {
        ...
        let mut arr = RepeatedField::<i32>::new(&mut arena);
        assert_eq!(arr.len(), 0);
        arr.push(1);
