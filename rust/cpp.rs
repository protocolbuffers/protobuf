...
use crate::{
    EntityType, IntoProxied, Map, MapIter, MapMut, MapView, Mut, ProtoBytes, ProtoStr, ProtoString,
...
// contains just enough information to indicate the value type of the map. We
// need this on the C++ side to be able to determine size and offset information
// about the map entry. For messages, the prototype is the message default
// instance. For all other types, it is just a MapValue with the appropriate
// tag.
trait Prototype {
...
macro_rules! impl_prototype_for_numeric_types {
    ($($t:ty),* $(,)?) => {
        $(
...
impl_prototype_for_numeric_types!(u32, i32, u64, i64, f32, f64, bool);
...
impl<'a> Prototype for &'a ProtoStr {
    fn get_prototype() -> MapValue {
        MapValue { tag: MapValueTag::String, val: MapValueUnion { s: None } }
    }
}

impl<'a> Prototype for &'a [u8] {
    fn get_prototype() -> MapValue {
        MapValue { tag: MapValueTag::String, val: MapValueUnion { s: None } }
    }
}
...
where
    ...
    for<'a> View<'a, Value>: From<MapValue>,
{
    ...
    fn map_get<'a>(map: MapView<'a, Key, Self>, key: View<'_, Key>) -> Option<View<'a, Self>> {
        ...
        unsafe { Some(<MapValue as Into<View<'a, Self>>>::into(value.assume_init())) }
    }
    ...
    fn map_iter_next<'a>(
        ...
        unsafe {
            iter.as_raw_mut(Private).next_unchecked::<Key, Self, _, _>(
                ...
                <MapValue as Into<View<'a, Self>>>::into,
            )
        }
    }
}
...
impl From<ProtoString> for MapValue {
    fn from(value: ProtoString) -> Self {
        MapValue::make_string(protostr_into_cppstdstring(value))
    }
}

impl From<ProtoBytes> for MapValue {
    fn from(value: ProtoBytes) -> Self {
        MapValue::make_string(protobytes_into_cppstdstring(value))
    }
}

impl<'a> From<&'a ProtoStr> for MapValue {
    fn from(value: &'a ProtoStr) -> Self {
        MapValue::make_string(protostr_into_cppstdstring(value.into_proxied(Private)))
    }
}

impl<'a> From<&'a [u8]> for MapValue {
    fn from(value: &'a [u8]) -> Self {
        MapValue::make_string(protobytes_into_cppstdstring(value.into_proxied(Private)))
    }
}

impl From<MapValue> for u32 {
    fn from(value: MapValue) -> Self {
        unsafe { value.val.u }
    }
}

impl From<MapValue> for i32 {
    fn from(value: MapValue) -> Self {
        unsafe { value.val.u as i32 }
    }
}

impl From<MapValue> for u64 {
    fn from(value: MapValue) -> Self {
        unsafe { value.val.uu }
    }
}

impl From<MapValue> for i64 {
    fn from(value: MapValue) -> Self {
        unsafe { value.val.uu as i64 }
    }
}

impl From<MapValue> for f32 {
    fn from(value: MapValue) -> Self {
        f32::from_bits(unsafe { value.val.u })
    }
}

impl From<MapValue> for f64 {
    fn from(value: MapValue) -> Self {
        f64::from_bits(unsafe { value.val.uu })
    }
}

impl From<MapValue> for bool {
    fn from(value: MapValue) -> Self {
        unsafe { value.val.b }
    }
}

impl<'msg> From<MapValue> for &'msg ProtoStr {
    fn from(value: MapValue) -> Self {
        unsafe {
...
impl<'msg> From<MapValue> for &'msg [u8] {
    fn from(value: MapValue) -> Self {
