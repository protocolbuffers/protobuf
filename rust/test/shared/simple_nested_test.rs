...
fn test_nested_muts() {
    ...
    // TODO if we don't `clear` here, test_nested_views can fail!
    // simple_nested_cpp passes after cl/582978261, but simple_nested_upb still
    // fails
    /*
        inner_msg.double_mut().clear();
        inner_msg.float_mut().clear();
        inner_msg.int32_mut().clear();
        inner_msg.int64_mut().clear();
        inner_msg.uint32_mut().clear();
        inner_msg.uint64_mut().clear();
        inner_msg.sint32_mut().clear();
        inner_msg.sint64_mut().clear();
        inner_msg.fixed32_mut().clear();
        inner_msg.fixed64_mut().clear();
        inner_msg.bool_mut().clear();
    */
