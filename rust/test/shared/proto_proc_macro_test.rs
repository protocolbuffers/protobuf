...
fn test_setting_literals() {
    ...
    let msg = proto_proc!(TestAllTypes {
        optional_int32: 101,
        optional_int64: 102,
        optional_uint32: 103,
...
fn single_nested_message() {
    let msg = proto_proc!(TestAllTypes { optional_nested_message: NestedMessage { bb: 42 } });
    ...
    let msg = proto_proc!(::unittest_rust_proto::TestAllTypes {
    ...
    let msg = proto_proc!(::unittest_rust_proto::TestAllTypes { optional_nested_message: __ {} });
    assert_that!(msg.has_optional_nested_message(), eq(true));
}

#[gtest]
fn test_recursive_msg() {
    let msg = proto_proc!(NestedTestAllTypes {
...
fn test_spread_msg() {
    let msg = proto_proc!(TestAllTypes { optional_nested_message: NestedMessage { bb: 42 } });
    let msg2 = proto_proc!(TestAllTypes { ..msg.as_view() });
    assert_that!(msg2.optional_nested_message().bb(), eq(42));
    let msg3 = proto_proc!(TestAllTypes { optional_int32: 1, ..msg.as_view() });
...
fn test_spread_nested_msg() {
    ...
    let msg2 = proto_proc!(NestedTestAllTypes {
...
fn test_repeated_i32() {
    let msg = proto_proc!(TestAllTypes { repeated_int32: [1, 1 + 1, 3] });
...
fn test_repeated_msg() {
    let msg2 = proto_proc!(NestedTestAllTypes { payload: TestAllTypes { optional_int32: 1 } });
    let msg = proto_proc!(NestedTestAllTypes {
        child: NestedTestAllTypes {
            repeated_child: [
...
fn test_string_maps() {
    let msg = proto_proc!(TestMap {
        map_string_string: [("foo", "bar"), ("baz", "qux"), ("quux", "quuz")]
    });
