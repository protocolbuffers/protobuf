...
fn test_generated_descriptors() {
    let descriptor = FileDescriptorProto::parse(
        unittest_rust_proto::THIRD_PARTY_PROTOBUF_RUST_TEST_UNITTEST_DESCRIPTOR_INFO.descriptor,
    )
    .unwrap();
    ...
    assert_that!(
        unittest_rust_proto::THIRD_PARTY_PROTOBUF_RUST_TEST_UNITTEST_DESCRIPTOR_INFO.deps.len(),
        eq(1)
    );
    let dep = FileDescriptorProto::parse(
        unittest_rust_proto::THIRD_PARTY_PROTOBUF_RUST_TEST_UNITTEST_DESCRIPTOR_INFO.deps[0]
            .descriptor,
    )
    .unwrap();
