...
unsafe extern "C" {
    ...
    pub fn proto2_rust_map_iter(m: RawMap, iter: *mut UntypedMapIterator);
}
    ...
    Value: Singular + CppMapTypeConversions,
{
    ...
    fn map_iter<Key: MapKey>(_private: Private, map: MapView<Key, Self>) -> MapIter<Key, Self> {
        ...
        let mut iter = std::mem::MaybeUninit::<UntypedMapIterator>::uninit();
        // SAFETY:
        ...
        unsafe {
            proto2_rust_map_iter(map.as_raw(Private), iter.as_mut_ptr());
