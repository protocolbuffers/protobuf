...
pub fn get_dependency(crate_name: &str) -> protobuf_codegen::Dependency {
    protobuf_codegen::Dependency {
        crate_name: crate_name.to_string(),
        import_paths: vec!(Path::new(env!("CARGO_MANIFEST_DIR")).join("proto")),
        include_paths: vec!(Path::new(env!("OUT_DIR")).join("protobuf_generated")),
        proto_files: vec!(
